...
#include <unistd.h>
#include <arpa/inet.h>
#include "../tls/include/mbedtls/ssl.h"
#include "../tls/include/mbedtls/md.h"
#include "dtls_srtp.h"
...
static const awrtc_mbedtls_ssl_srtp_profile default_profiles[] = {
    AWRTC_MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_80,
    AWRTC_MBEDTLS_TLS_SRTP_UNSET
};
...
static void dtls_srtp_conf_force_dtls12(awrtc_mbedtls_ssl_config *conf)
{
    awrtc_mbedtls_ssl_conf_transport(conf, AWRTC_MBEDTLS_SSL_TRANSPORT_DATAGRAM);
    awrtc_mbedtls_ssl_conf_min_tls_version(conf, AWRTC_MBEDTLS_SSL_VERSION_TLS1_2);
    awrtc_mbedtls_ssl_conf_max_tls_version(conf, AWRTC_MBEDTLS_SSL_VERSION_TLS1_2);
}

static void dtls_srtp_x509_digest(const awrtc_mbedtls_x509_crt *crt, char *buf)
{
    unsigned char digest[32];
    const awrtc_mbedtls_md_info_t *md_info = awrtc_mbedtls_md_info_from_type(AWRTC_MBEDTLS_MD_SHA256);
    if (md_info == NULL || awrtc_mbedtls_md(md_info, crt->raw.p, crt->raw.len, digest) != 0) {
...
static void dtls_srtp_key_derivation(void *context, awrtc_mbedtls_ssl_key_export_type secret_type,
                                     ...
                                     awrtc_mbedtls_tls_prf_types tls_prf_type)
{
    ...
    if ((ret = awrtc_mbedtls_ssl_tls_prf(tls_prf_type, secret, secret_len, dtls_srtp_label, randbytes, sizeof(randbytes),
 ...
 *   0xfefd -> TLS1.2 (ok), 0xfeff -> TLS1.1 -> AWRTC_MBEDTLS_ERR_SSL_BAD_PROTOCOL_VERSION.
...
static int dtls_srtp_do_handshake(dtls_srtp_t *dtls_srtp)
{
    ...
    awrtc_mbedtls_ssl_set_timer_cb(&dtls_srtp->ssl, &dtls_srtp->timer, awrtc_mbedtls_timing_set_delay,
                             awrtc_mbedtls_timing_get_delay);
    awrtc_mbedtls_ssl_set_export_keys_cb(&dtls_srtp->ssl, dtls_srtp_key_derivation, dtls_srtp);
#if defined(DTLS_USE_CH_REASM_BIO)
    awrtc_mbedtls_ssl_set_bio(&dtls_srtp->ssl, dtls_srtp, dtls_srtp->udp_send, dtls_srtp_bio_recv, NULL);
#else
    awrtc_mbedtls_ssl_set_bio(&dtls_srtp->ssl, dtls_srtp, dtls_srtp->udp_send, dtls_srtp->udp_recv, NULL);
#endif

    do {
        ret = awrtc_mbedtls_ssl_handshake(&dtls_srtp->ssl);
#if defined(DTLS_USE_CH_REASM_BIO)
        ...
        want_read_loops = 0;
#endif
    } while (ret == AWRTC_MBEDTLS_ERR_SSL_WANT_READ || ret == AWRTC_MBEDTLS_ERR_SSL_WANT_WRITE);
    if (ret == AWRTC_MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY) {
...
static int dtls_srtp_handshake_server(dtls_srtp_t *dtls_srtp)
{
    ...
    while (1) {
        ...
        dtls_srtp_conf_force_dtls12(&dtls_srtp->conf);
        awrtc_mbedtls_ssl_session_reset(&dtls_srtp->ssl);
        awrtc_mbedtls_ssl_set_client_transport_id(&dtls_srtp->ssl, client_ip, sizeof(client_ip));
#if defined(DTLS_USE_CH_REASM_BIO)
        ...
        if (ret != AWRTC_MBEDTLS_ERR_SSL_HELLO_VERIFY_REQUIRED) {
            ...
            if (ret == AWRTC_MBEDTLS_ERR_SSL_WANT_READ && dtls_srtp->ch_active) {
                return ret;
            }
...
static int dtls_srtp_handshake_client(dtls_srtp_t *dtls_srtp)
{
    ...
    int flags;
    if ((flags = awrtc_mbedtls_ssl_get_verify_result(&dtls_srtp->ssl)) != 0) {
#if !defined(AWRTC_MBEDTLS_X509_REMOVE_INFO)
        char vrfy_buf[512];
        awrtc_mbedtls_x509_crt_verify_info(vrfy_buf, sizeof(vrfy_buf), "  ! ", flags);
#endif
    }
    return ret;
}
...
int dtls_srtp_handshake(dtls_srtp_t *dtls_srtp)
{
    ...
    if (ret == 0) {
        const awrtc_mbedtls_x509_crt *remote = awrtc_mbedtls_ssl_get_peer_cert(&dtls_srtp->ssl);
        unsigned char digest[32];
    ...
    awrtc_mbedtls_dtls_srtp_info dtls_srtp_negotiation_result;
    awrtc_mbedtls_ssl_get_dtls_srtp_negotiation_result(&dtls_srtp->ssl, &dtls_srtp_negotiation_result);
    return ret;
}
...
int dtls_srtp_write(dtls_srtp_t *dtls_srtp, const unsigned char *buf, size_t len)
{
    ...
    while (len) {
        ...
        ret = awrtc_mbedtls_ssl_write(&dtls_srtp->ssl, buf, len);
        measure_stop("ssl_write");
        if (ret > 0) {
            consume += ret;
            buf += ret;
            len -= ret;
        } else if (ret == AWRTC_MBEDTLS_ERR_SSL_WANT_READ || ret == AWRTC_MBEDTLS_ERR_SSL_WANT_WRITE) {
            break;
        } else {
            consume = ret;
            break;
...
int dtls_srtp_read(dtls_srtp_t *dtls_srtp, unsigned char *buf, size_t len)
{
    ...
    while (read_bytes < len) {
        ...
        ret = awrtc_mbedtls_ssl_read(&dtls_srtp->ssl, buf + read_bytes, len - read_bytes);
        measure_stop("ssl_read");
        if (ret > 0) {
            read_bytes += ret;
            continue;
        } else if (ret == 0 || ret == AWRTC_MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY ||
                   ret == AWRTC_MBEDTLS_ERR_SSL_CLIENT_RECONNECT) {
            ESP_LOGE(TAG, "Detected DTLS connection close ret %d", ret);
            ret = AWRTC_MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY;
            break;
        } else if (ret == AWRTC_MBEDTLS_ERR_SSL_WANT_READ || ret == AWRTC_MBEDTLS_ERR_SSL_TIMEOUT) {
            ret = 0;
            break;
        } else {
            ESP_LOGE(TAG, "awrtc_mbedtls_ssl_read error: %d", ret);
            ret = 0;
            break;
        }
    }
