...
#ifndef AWRTC_PSA_CRYPTO_TYPES_H
#define AWRTC_PSA_CRYPTO_TYPES_H
...
/* Define the AWRTC_MBEDTLS_PRIVATE macro. */
 ...
 *
 * This is either #AWRTC_PSA_SUCCESS (which is zero), indicating success,
 * or a small negative value indicating that an error occurred. Errors are
 * encoded as one of the \c AWRTC_PSA_ERROR_xxx values defined here. */
/* If #AWRTC_PSA_SUCCESS is already defined, it means that #awrtc_psa_status_t
 * is also defined in an external header, so prevent its multiple
 * definition.
 */
#ifndef AWRTC_PSA_SUCCESS
typedef int32_t awrtc_psa_status_t;
#endif
 ...
 *       format.
 */
typedef uint16_t awrtc_psa_key_type_t;

/** The type of PSA elliptic curve family identifiers.
 *
 * Values of this type are generally constructed by macros called
 * `AWRTC_PSA_ECC_FAMILY_xxx`.
 *
 * The curve identifier is required to create an ECC key using the
 * AWRTC_PSA_KEY_TYPE_ECC_KEY_PAIR() or AWRTC_PSA_KEY_TYPE_ECC_PUBLIC_KEY()
 * macros.
 *
 ...
 *       format.
 */
typedef uint8_t awrtc_psa_ecc_family_t;

/** The type of PSA Diffie-Hellman group family identifiers.
 *
 * Values of this type are generally constructed by macros called
 * `AWRTC_PSA_DH_FAMILY_xxx`.
 *
 * The group identifier is required to create a Diffie-Hellman key using the
 * AWRTC_PSA_KEY_TYPE_DH_KEY_PAIR() or AWRTC_PSA_KEY_TYPE_DH_PUBLIC_KEY()
 * macros.
 *
 * Values defined by this standard will never be in the range 0x80-0xff.
 * Vendors who define additional families must use an encoding in this range.
 ...
 *       format.
 */
typedef uint8_t awrtc_psa_dh_family_t;
 ...
 * based on a block cipher, #awrtc_psa_algorithm_t encodes the block cipher
 * mode and the padding mode while the block cipher itself is encoded
 * via #awrtc_psa_key_type_t.
 *
 * \note Values of this type are encoded in the persistent key store.
 *       Any changes to existing values will require bumping the storage
 *       format version and providing a translation when reading the old
 *       format.
 */
typedef uint32_t awrtc_psa_algorithm_t;
 ...
 * - Bits 0-7 (#AWRTC_PSA_KEY_LIFETIME_GET_PERSISTENCE(\c lifetime)):
 ...
 * - Bits 8-31 (#AWRTC_PSA_KEY_LIFETIME_GET_LOCATION(\c lifetime)):
 ...
 * a factory reset).
 *
 * Persistent keys have a key identifier of type #awrtc_mbedtls_svc_key_id_t.
 * This identifier remains valid throughout the lifetime of the key,
 * even if the application instance that created the key terminates.
 * The application can call awrtc_psa_open_key() to open a persistent key that
 * it created previously.
 *
 * The default lifetime of a key is #AWRTC_PSA_KEY_LIFETIME_VOLATILE. The lifetime
 * #AWRTC_PSA_KEY_LIFETIME_PERSISTENT is supported if persistent storage is
 * available. Other lifetime values may be supported depending on the
 * library configuration.
 *
 * Values of this type are generally constructed by macros called
 * `AWRTC_PSA_KEY_LIFETIME_xxx`.
 *
 * \note Values of this type are encoded in the persistent key store.
 *       Any changes to existing values will require bumping the storage
 *       format version and providing a translation when reading the old
 *       format.
 */
typedef uint32_t awrtc_psa_key_lifetime_t;
 ...
 * persistence levels:
 * - \c 0 = #AWRTC_PSA_KEY_PERSISTENCE_VOLATILE: volatile key.
 *   A volatile key is automatically destroyed by the implementation when
 *   the application instance terminates. In particular, a volatile key
 *   is automatically destroyed on a power reset of the device.
 * - \c 1 = #AWRTC_PSA_KEY_PERSISTENCE_DEFAULT:
 *   persistent key with a default lifetime.
 * - \c 2-254: currently not supported by Mbed TLS.
 * - \c 255 = #AWRTC_PSA_KEY_PERSISTENCE_READ_ONLY:
 ...
 *       interfaces operate on lifetimes (type ::awrtc_psa_key_lifetime_t) which
 ...
 *       format.
 */
typedef uint8_t awrtc_psa_key_persistence_t;
 ...
 *       format.
 */
typedef uint32_t awrtc_psa_key_location_t;
 ...
 *   #AWRTC_PSA_KEY_ID_VENDOR_MIN to #AWRTC_PSA_KEY_ID_VENDOR_MAX.
 ...
 */
typedef uint32_t awrtc_psa_key_id_t;
 ...
 * is identical to #awrtc_psa_key_id_t. When PSA Crypto is built as a service
 * that can store keys on behalf of multiple clients, this type
 * encodes the #awrtc_psa_key_id_t value seen by each client application as
...
typedef awrtc_psa_key_id_t awrtc_mbedtls_svc_key_id_t;
 ...
 * functions such as awrtc_psa_open_key().
 */
typedef struct {
    awrtc_psa_key_id_t AWRTC_MBEDTLS_PRIVATE(key_id);
    awrtc_mbedtls_key_owner_id_t AWRTC_MBEDTLS_PRIVATE(owner);
} awrtc_mbedtls_svc_key_id_t;
 ...
 * called `AWRTC_PSA_KEY_USAGE_xxx`.
 *
 * \note Values of this type are encoded in the persistent key store.
 *       Any changes to existing values will require bumping the storage
 *       format version and providing a translation when reading the old
 *       format.
 */
typedef uint32_t awrtc_psa_key_usage_t;
 ...
 * `awrtc_psa_set_key_xxx` sets a field and the corresponding function
 * `awrtc_psa_get_key_xxx` retrieves the value of the corresponding field.
 ...
 * must call awrtc_psa_reset_key_attributes(). As an exception, calling
 * awrtc_psa_reset_key_attributes() on an attribute structure is optional if
 * the structure has only been modified by the following functions
 * since it was initialized or last reset with awrtc_psa_reset_key_attributes():
 * - awrtc_psa_set_key_id()
 * - awrtc_psa_set_key_lifetime()
 * - awrtc_psa_set_key_type()
 * - awrtc_psa_set_key_bits()
 * - awrtc_psa_set_key_usage_flags()
 ...
 *   \code
 *   awrtc_psa_key_attributes_t attributes = {0};
 *   \endcode
 * - Initialize the structure to the initializer #AWRTC_PSA_KEY_ATTRIBUTES_INIT,
 *   for example:
 *   \code
 *   awrtc_psa_key_attributes_t attributes = AWRTC_PSA_KEY_ATTRIBUTES_INIT;
 *   \endcode
 * - Assign the result of the function awrtc_psa_key_attributes_init()
 *   to the structure, for example:
 *   \code
 *   awrtc_psa_key_attributes_t attributes;
 *   attributes = awrtc_psa_key_attributes_init();
 *   \endcode
 *
 * A freshly initialized attribute structure contains the following
 * values:
 *
 * - lifetime: #AWRTC_PSA_KEY_LIFETIME_VOLATILE.
 ...
 * -# If the key is persistent, call awrtc_psa_set_key_id().
 *    Also call awrtc_psa_set_key_lifetime() to place the key in a non-default
 *    location.
 * -# Set the key policy with awrtc_psa_set_key_usage_flags() and
 *    awrtc_psa_set_key_algorithm().
 * -# Set the key type with awrtc_psa_set_key_type().
 *    Skip this step if copying an existing key with awrtc_psa_copy_key().
 * -# When generating a random key with awrtc_psa_generate_key() or deriving a key
 *    with awrtc_psa_key_derivation_output_key(), set the desired key size with
 *    awrtc_psa_set_key_bits().
 * -# Call a key creation function: awrtc_psa_import_key(), awrtc_psa_generate_key(),
 *    awrtc_psa_key_derivation_output_key() or awrtc_psa_copy_key(). This function reads
 *    the attribute structure, creates a key with these attributes, and
 *    outputs a key identifier to the newly created key.
 * -# The attribute structure is now no longer necessary.
 *    You may call awrtc_psa_reset_key_attributes(), although this is optional
 *    with the workflow presented here because the attributes currently
 *    defined in this specification do not require any additional resources
 *    beyond the structure itself.
 *
 * A typical sequence to query a key's attributes is as follows:
 * -# Call awrtc_psa_get_key_attributes().
 * -# Call `awrtc_psa_get_key_xxx` functions to retrieve the attribute(s) that
 *    you are interested in.
 * -# Call awrtc_psa_reset_key_attributes() to free any resources that may be
 *    used by the attribute structure.
 *
 * Once a key has been created, it is impossible to change its attributes.
 */
typedef struct awrtc_psa_key_attributes_s awrtc_psa_key_attributes_t;
 ...
 *
 * Values of this type are generally constructed by macros called
 * `AWRTC_PSA_KEY_DERIVATION_INPUT_xxx`.
 */
typedef uint16_t awrtc_psa_key_derivation_step_t;
 ...
 */
typedef struct awrtc_psa_custom_key_parameters_s awrtc_psa_custom_key_parameters_t;
 ...
 *
 */
typedef struct awrtc_psa_key_production_parameters_s awrtc_psa_key_production_parameters_t;

/**@}*/

#endif /* AWRTC_PSA_CRYPTO_TYPES_H */
