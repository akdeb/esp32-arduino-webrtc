...
#ifndef AWRTC_PSA_CRYPTO_COMPAT_H
#define AWRTC_PSA_CRYPTO_COMPAT_H
...
/*
 * To support both openless APIs and awrtc_psa_open_key() temporarily, define
 * awrtc_psa_key_handle_t to be equal to awrtc_mbedtls_svc_key_id_t. Do not mark the
 * type and its utility macros and functions deprecated yet. This will be done
 * in a subsequent phase.
 */
typedef awrtc_mbedtls_svc_key_id_t awrtc_psa_key_handle_t;

#define AWRTC_PSA_KEY_HANDLE_INIT AWRTC_MBEDTLS_SVC_KEY_ID_INIT
 ...
 */
static inline int awrtc_psa_key_handle_is_null(awrtc_psa_key_handle_t handle)
{
    return awrtc_mbedtls_svc_key_id_is_null(handle);
}

/** Open a handle to an existing persistent key.
 *
 * Open a handle to a persistent key. A key is persistent if it was created
 * with a lifetime other than #AWRTC_PSA_KEY_LIFETIME_VOLATILE. A persistent key
 * always has a nonzero key identifier, set with awrtc_psa_set_key_id() when
 * creating the key. Implementations may provide additional pre-provisioned
 * keys that can be opened with awrtc_psa_open_key(). Such keys have an application
 * key identifier in the vendor range, as documented in the description of
 * #awrtc_psa_key_id_t.
 *
 * The application must eventually close the handle with awrtc_psa_close_key() or
 * awrtc_psa_destroy_key() to release associated resources. If the application dies
 * without calling one of these functions, the implementation should perform
 * the equivalent of a call to awrtc_psa_close_key().
 *
 * Some implementations permit an application to open the same key multiple
 * times. If this is successful, each call to awrtc_psa_open_key() will return a
 ...
 */
awrtc_psa_status_t awrtc_psa_open_key(awrtc_mbedtls_svc_key_id_t key,
                          awrtc_psa_key_handle_t *handle);

/** Close a key handle.
 *
 * If the handle designates a volatile key, this will destroy the key material
 * and free all associated resources, just like awrtc_psa_destroy_key().
 *
 * If this is the last open handle to a persistent key, then closing the handle
 * will free all resources associated with the key in volatile memory. The key
 * data in persistent storage is not affected and can be opened again later
 * with a call to awrtc_psa_open_key().
 *
 ...
 *                      If this is \c 0, do nothing and return \c AWRTC_PSA_SUCCESS.
 *
 * \retval #AWRTC_PSA_SUCCESS
 ...
 */
#define AWRTC_PSA_DH_FAMILY_CUSTOM                                            \
    ((awrtc_psa_dh_family_t) AWRTC_MBEDTLS_DEPRECATED_NUMERIC_CONSTANT(0x7e))
 ...
 *              awrtc_psa_set_key_type() and will be removed in a future version
 *              of the library.
 *
 * \param[in,out] attributes    Attribute structure where \p type will be set.
 * \param type                  Key type (a \c AWRTC_PSA_KEY_TYPE_XXX value).
 ...
 */
static inline awrtc_psa_status_t AWRTC_MBEDTLS_DEPRECATED awrtc_psa_set_key_domain_parameters(
    ...
    awrtc_psa_key_type_t type, const uint8_t *data, size_t data_length)
{
    ...
    awrtc_psa_set_key_type(attributes, type);
 ...
 * \retval #AWRTC_PSA_SUCCESS \emptydescription
 */
static inline awrtc_psa_status_t AWRTC_MBEDTLS_DEPRECATED awrtc_psa_get_key_domain_parameters(
...
/** Safe output buffer size for awrtc_psa_get_key_domain_parameters().
 *
 */
#define AWRTC_PSA_KEY_DOMAIN_PARAMETERS_SIZE(key_type, key_bits)      \
    AWRTC_MBEDTLS_DEPRECATED_NUMERIC_CONSTANT(1u)
...
#endif /* AWRTC_PSA_CRYPTO_COMPAT_H */
