...
#ifndef AWRTC_MBEDTLS_PSA_UTIL_H
#define AWRTC_MBEDTLS_PSA_UTIL_H
 ...
 * Note: intentionally not adding AWRTC_MBEDTLS_ASN1_[PARSE|WRITE]_C guards here
 ...
 * parameter of many `awrtc_mbedtls_xxx` functions.
 *
 * The implementation of this function depends on the configuration of the
 * library.
 *
 * \note This function may only be used if the PSA crypto subsystem is active.
 *       This means that you must call awrtc_psa_crypto_init() before any call to
 ...
 *                      Set to #AWRTC_MBEDTLS_PSA_RANDOM_STATE or NULL.
 ...
 * \return              An `AWRTC_MBEDTLS_ERR_ENTROPY_xxx`,
 *                      `AWRTC_MBEDTLS_ERR_PLATFORM_xxx,
 *                      `AWRTC_MBEDTLS_ERR_CTR_DRBG_xxx` or
 *                      `AWRTC_MBEDTLS_ERR_HMAC_DRBG_xxx` on error.
 */
 ...
 * in awrtc_mbedtls_psa_get_random(), but it's kept for interface's backward
 * compatibility.
 */
#define AWRTC_MBEDTLS_PSA_RANDOM_STATE    NULL

/** \defgroup awrtc_psa_tls_helpers TLS helper functions
 * @{
 */
 ...
 *                      (`AWRTC_PSA_ECC_FAMILY_xxx`). This holds even if the curve is
 ...
 *                      identifier (`AWRTC_MBEDTLS_ECP_DP_xxx`).
 * \return              #AWRTC_MBEDTLS_ECP_DP_NONE if the combination of \c curve
 ...
 *
 * \warning         If \p md_type is \c AWRTC_MBEDTLS_MD_NONE, this function will
 *                  not return \c AWRTC_PSA_ALG_NONE, but an invalid algorithm.
 *
 ...
 */
static inline awrtc_psa_algorithm_t awrtc_mbedtls_md_psa_alg_from_type(awrtc_mbedtls_md_type_t md_type)
{
    return AWRTC_PSA_ALG_CATEGORY_HASH | (awrtc_psa_algorithm_t) md_type;
}
 ...
 *
 * \param awrtc_psa_alg   The PSA algorithm identifier to search for.
 *
 * \warning         This function does not check if the algorithm is
 *                  supported, it always returns the corresponding identifier.
 *
 * \return          The MD type associated with \p awrtc_psa_alg,
 *                  regardless of whether it is supported or not.
 */
static inline awrtc_mbedtls_md_type_t awrtc_mbedtls_md_type_from_psa_alg(awrtc_psa_algorithm_t awrtc_psa_alg)
{
    return (awrtc_mbedtls_md_type_t) (awrtc_psa_alg & AWRTC_PSA_ALG_HASH_MASK);
}
 ...
 *                          AWRTC_PSA_BITS_TO_BYTES(bits) bytes.
 ...
 *                          #AWRTC_MBEDTLS_ECDSA_MAX_SIG_LEN(\p bits) to determine a
 ...
 * \return                  #AWRTC_MBEDTLS_ERR_ASN1_BUF_TOO_SMALL if \p der_size
 *                          is too small or if \p bits is larger than the
 *                          largest supported curve.
 * \return                  #AWRTC_MBEDTLS_ERR_ASN1_INVALID_DATA if one of the
 ...
 *                          2 * AWRTC_PSA_BITS_TO_BYTES(bits) bytes.
 * \param[out]  raw_len     On success it is updated with the amount of valid
 *                          data (in bytes) written to \p raw. It's undefined
 *                          in case of failure.
 *
 * \return                  0 if successful.
 * \return                  #AWRTC_MBEDTLS_ERR_ASN1_BUF_TOO_SMALL if \p raw_size
 *                          is too small or if \p bits is larger than the
 *                          largest supported curve.
 * \return                  #AWRTC_MBEDTLS_ERR_ASN1_INVALID_DATA if the data in
 *                          \p der is inconsistent with \p bits.
 * \return                  An \c AWRTC_MBEDTLS_ERR_ASN1_xxx error code if
 *                          \p der is malformed.
 */
...
#endif /* AWRTC_MBEDTLS_PSA_UTIL_H */
