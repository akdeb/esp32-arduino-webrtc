 ...
 */
#ifndef AWRTC_MBEDTLS_SSL_CACHE_H
#define AWRTC_MBEDTLS_SSL_CACHE_H
...
#if !defined(AWRTC_MBEDTLS_SSL_CACHE_DEFAULT_TIMEOUT)
#define AWRTC_MBEDTLS_SSL_CACHE_DEFAULT_TIMEOUT       86400   /*!< 1 day  */
#endif

#if !defined(AWRTC_MBEDTLS_SSL_CACHE_DEFAULT_MAX_ENTRIES)
#define AWRTC_MBEDTLS_SSL_CACHE_DEFAULT_MAX_ENTRIES      50   /*!< Maximum entries in cache */
#endif
...
typedef struct awrtc_mbedtls_ssl_cache_context awrtc_mbedtls_ssl_cache_context;
typedef struct awrtc_mbedtls_ssl_cache_entry awrtc_mbedtls_ssl_cache_entry;

/**
 * \brief   This structure is used for storing cache entries
 */
struct awrtc_mbedtls_ssl_cache_entry {
#if defined(AWRTC_MBEDTLS_HAVE_TIME)
    awrtc_mbedtls_time_t AWRTC_MBEDTLS_PRIVATE(timestamp);           /*!< entry timestamp    */
#endif

    unsigned char AWRTC_MBEDTLS_PRIVATE(session_id)[32];       /*!< session ID         */
    size_t AWRTC_MBEDTLS_PRIVATE(session_id_len);

    unsigned char *AWRTC_MBEDTLS_PRIVATE(session);             /*!< serialized session */
    size_t AWRTC_MBEDTLS_PRIVATE(session_len);

    awrtc_mbedtls_ssl_cache_entry *AWRTC_MBEDTLS_PRIVATE(next);      /*!< chain pointer      */
};

/**
 * \brief Cache context
 */
struct awrtc_mbedtls_ssl_cache_context {
    awrtc_mbedtls_ssl_cache_entry *AWRTC_MBEDTLS_PRIVATE(chain);     /*!< start of the chain     */
    int AWRTC_MBEDTLS_PRIVATE(timeout);                /*!< cache entry timeout    */
    int AWRTC_MBEDTLS_PRIVATE(max_entries);            /*!< maximum entries        */
#if defined(AWRTC_MBEDTLS_THREADING_C)
 ...
 * \return                #AWRTC_MBEDTLS_ERR_SSL_CACHE_ENTRY_NOT_FOUND if there is
 ...
 *                 (Default: AWRTC_MBEDTLS_SSL_CACHE_DEFAULT_TIMEOUT (1 day))
 *
 ...
 */
static inline int awrtc_mbedtls_ssl_cache_get_timeout(awrtc_mbedtls_ssl_cache_context *cache)
{
    return cache->AWRTC_MBEDTLS_PRIVATE(timeout);
}
#endif /* AWRTC_MBEDTLS_HAVE_TIME */

/**
 * \brief          Set the maximum number of cache entries
 *                 (Default: AWRTC_MBEDTLS_SSL_CACHE_DEFAULT_MAX_ENTRIES (50))
 *
 * \param cache    SSL cache context
