 ...
 *        AWRTC_MBEDTLS_LMS_SHA256_M32_H10 in order to reduce complexity. This is one
 ...
 */
#ifndef AWRTC_MBEDTLS_LMS_H
#define AWRTC_MBEDTLS_LMS_H
...
#define AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA   -0x0011 /**< Bad data has been input to an LMS function */
#define AWRTC_MBEDTLS_ERR_LMS_OUT_OF_PRIVATE_KEYS -0x0013 /**< Specified LMS key has utilised all of its private keys */
#define AWRTC_MBEDTLS_ERR_LMS_VERIFY_FAILED    -0x0015 /**< LMS signature verification failed */
#define AWRTC_MBEDTLS_ERR_LMS_ALLOC_FAILED     -0x0017 /**< LMS failed to allocate space for a private key */
#define AWRTC_MBEDTLS_ERR_LMS_BUFFER_TOO_SMALL -0x0019 /**< Input/output buffer is too small to contain requited data */

/* Currently only defined for SHA256, 32 is the max hash output size */
#define AWRTC_MBEDTLS_LMOTS_N_HASH_LEN_MAX           (32u)
#define AWRTC_MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT_MAX    (34u)
#define AWRTC_MBEDTLS_LMOTS_N_HASH_LEN(type)         ((type) == AWRTC_MBEDTLS_LMOTS_SHA256_N32_W8 ? 32u : 0)
#define AWRTC_MBEDTLS_LMOTS_I_KEY_ID_LEN             (16u)
#define AWRTC_MBEDTLS_LMOTS_Q_LEAF_ID_LEN            (4u)
#define AWRTC_MBEDTLS_LMOTS_TYPE_LEN                 (4u)
#define AWRTC_MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT(type)  ((type) == AWRTC_MBEDTLS_LMOTS_SHA256_N32_W8 ? 34u : 0)
#define AWRTC_MBEDTLS_LMOTS_C_RANDOM_VALUE_LEN(type) (AWRTC_MBEDTLS_LMOTS_N_HASH_LEN(type))

#define AWRTC_MBEDTLS_LMOTS_SIG_LEN(type) (AWRTC_MBEDTLS_LMOTS_TYPE_LEN + \
                                     AWRTC_MBEDTLS_LMOTS_C_RANDOM_VALUE_LEN(type) + \
                                     (AWRTC_MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT(type) * \
                                      AWRTC_MBEDTLS_LMOTS_N_HASH_LEN(type)))


#define AWRTC_MBEDTLS_LMS_TYPE_LEN            (4)
#define AWRTC_MBEDTLS_LMS_H_TREE_HEIGHT(type) ((type) == AWRTC_MBEDTLS_LMS_SHA256_M32_H10 ? 10u : 0)

/* The length of a hash output, Currently only implemented for SHA256.
 * Max is 32 bytes.
 */
#define AWRTC_MBEDTLS_LMS_M_NODE_BYTES(type) ((type) == AWRTC_MBEDTLS_LMS_SHA256_M32_H10 ? 32 : 0)
#define AWRTC_MBEDTLS_LMS_M_NODE_BYTES_MAX 32

#define AWRTC_MBEDTLS_LMS_SIG_LEN(type, otstype) (AWRTC_MBEDTLS_LMOTS_Q_LEAF_ID_LEN + \
                                            AWRTC_MBEDTLS_LMOTS_SIG_LEN(otstype) + \
                                            AWRTC_MBEDTLS_LMS_TYPE_LEN + \
                                            (AWRTC_MBEDTLS_LMS_H_TREE_HEIGHT(type) * \
                                             AWRTC_MBEDTLS_LMS_M_NODE_BYTES(type)))

#define AWRTC_MBEDTLS_LMS_PUBLIC_KEY_LEN(type) (AWRTC_MBEDTLS_LMS_TYPE_LEN + \
 ...
 */
typedef enum {
    AWRTC_MBEDTLS_LMS_SHA256_M32_H10 = 0x6,
} awrtc_mbedtls_lms_algorithm_type_t;
 ...
 */
typedef enum {
    AWRTC_MBEDTLS_LMOTS_SHA256_N32_W8 = 4
} awrtc_mbedtls_lmots_algorithm_type_t;
 ...
 */
typedef struct {
    unsigned char AWRTC_MBEDTLS_PRIVATE(I_key_identifier[AWRTC_MBEDTLS_LMOTS_I_KEY_ID_LEN]); /*!< The key
                                                                                    identifier. */
    unsigned char AWRTC_MBEDTLS_PRIVATE(q_leaf_identifier[AWRTC_MBEDTLS_LMOTS_Q_LEAF_ID_LEN]); /*!< Which
                                                                                      leaf of the LMS key this is.
                                                                                      0 if the key is not part of an LMS key. */
    awrtc_mbedtls_lmots_algorithm_type_t AWRTC_MBEDTLS_PRIVATE(type); /*!< The LM-OTS key type identifier as
                                                               per IANA. Only SHA256_N32_W8 is
                                                               currently supported. */
} awrtc_mbedtls_lmots_parameters_t;
 ...
 */
typedef struct {
    awrtc_mbedtls_lmots_parameters_t AWRTC_MBEDTLS_PRIVATE(params);
    unsigned char AWRTC_MBEDTLS_PRIVATE(public_key)[AWRTC_MBEDTLS_LMOTS_N_HASH_LEN_MAX];
    unsigned char AWRTC_MBEDTLS_PRIVATE(have_public_key); /*!< Whether the context contains a public key.
                                                       Boolean values only. */
} awrtc_mbedtls_lmots_public_t;
...
typedef struct {
    awrtc_mbedtls_lmots_parameters_t AWRTC_MBEDTLS_PRIVATE(params);
    unsigned char AWRTC_MBEDTLS_PRIVATE(private_key)[AWRTC_MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT_MAX][
        AWRTC_MBEDTLS_LMOTS_N_HASH_LEN_MAX];
    unsigned char AWRTC_MBEDTLS_PRIVATE(have_private_key); /*!< Whether the context contains a private key.
                                                        Boolean values only. */
} awrtc_mbedtls_lmots_private_t;
...
typedef struct {
    unsigned char AWRTC_MBEDTLS_PRIVATE(I_key_identifier[AWRTC_MBEDTLS_LMOTS_I_KEY_ID_LEN]); /*!< The key
                                                                                    identifier. */
    awrtc_mbedtls_lmots_algorithm_type_t AWRTC_MBEDTLS_PRIVATE(otstype); /*!< The LM-OTS key type identifier as
                                                                per IANA. Only SHA256_N32_W8 is
                                                                currently supported. */
    awrtc_mbedtls_lms_algorithm_type_t AWRTC_MBEDTLS_PRIVATE(type); /*!< The LMS key type identifier as per
                                                             IANA. Only SHA256_M32_H10 is currently
                                                             supported. */
} awrtc_mbedtls_lms_parameters_t;
 ...
 */
typedef struct {
    awrtc_mbedtls_lms_parameters_t AWRTC_MBEDTLS_PRIVATE(params);
    unsigned char AWRTC_MBEDTLS_PRIVATE(T_1_pub_key)[AWRTC_MBEDTLS_LMS_M_NODE_BYTES_MAX]; /*!< The public key, in
                                                                                 the form of the Merkle tree root node. */
    unsigned char AWRTC_MBEDTLS_PRIVATE(have_public_key); /*!< Whether the context contains a public key.
                                                       Boolean values only. */
} awrtc_mbedtls_lms_public_t;
...
typedef struct {
    awrtc_mbedtls_lms_parameters_t AWRTC_MBEDTLS_PRIVATE(params);
    uint32_t AWRTC_MBEDTLS_PRIVATE(q_next_usable_key); /*!< The index of the next OTS key that has not
                                                      been used. */
    awrtc_mbedtls_lmots_private_t *AWRTC_MBEDTLS_PRIVATE(ots_private_keys); /*!< The private key material. One OTS key
                                                                   for each leaf node in the Merkle tree. NULL
                                                                   when have_private_key is 0 and non-NULL otherwise.
                                                                   is 2^AWRTC_MBEDTLS_LMS_H_TREE_HEIGHT(type) in length. */
    awrtc_mbedtls_lmots_public_t *AWRTC_MBEDTLS_PRIVATE(ots_public_keys); /*!< The OTS key public keys, used to
                                                                   build the Merkle tree. NULL
                                                                   when have_private_key is 0 and
                                                                   non-NULL otherwise.
                                                                   Is 2^AWRTC_MBEDTLS_LMS_H_TREE_HEIGHT(type)
                                                                   in length. */
    unsigned char AWRTC_MBEDTLS_PRIVATE(have_private_key); /*!< Whether the context contains a private key.
                                                        Boolean values only. */
} awrtc_mbedtls_lms_private_t;
#endif /* defined(AWRTC_MBEDTLS_LMS_PRIVATE) */
 ...
 *                           #AWRTC_MBEDTLS_LMS_PUBLIC_KEY_LEN bytes will be read from
 *                           this.
 ...
 *                           be at least #AWRTC_MBEDTLS_LMS_PUBLIC_KEY_LEN in size.
 ...
 *                           #AWRTC_MBEDTLS_LMS_SIG_LEN bytes will be read from
 *                           this.
 ...
 *                           Must be at least #AWRTC_MBEDTLS_LMS_SIG_LEN in size.
