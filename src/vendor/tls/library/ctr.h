...
#ifndef AWRTC_MBEDTLS_CTR_H
#define AWRTC_MBEDTLS_CTR_H
 ...
 */
static inline void awrtc_mbedtls_ctr_increment_counter(uint8_t n[16])
{
    ...
    for (int i = 3;; i--) {
        uint32_t x = AWRTC_MBEDTLS_GET_UINT32_BE(n, i << 2);
        x += 1;
        AWRTC_MBEDTLS_PUT_UINT32_BE(x, n, i << 2);
        if (x != 0 || i == 0) {
            break;
        }
    }
