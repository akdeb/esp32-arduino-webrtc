...
#define AWRTC_MBEDTLS_PLATFORM_HAS_EXPLICIT_BZERO 1
 ...
 *
 * This implementation for awrtc_mbedtls_platform_zeroize() was inspired from Colin
 ...
 * option AWRTC_MBEDTLS_PLATFORM_ZEROIZE_ALT, which allows users to configure
 * awrtc_mbedtls_platform_zeroize() to use a suitable implementation for their
 * platform and needs.
 */
#if !defined(AWRTC_MBEDTLS_PLATFORM_HAS_EXPLICIT_BZERO) && !(defined(__STDC_LIB_EXT1__) && \
...
{
    if (len > 0) {
#if defined(AWRTC_MBEDTLS_PLATFORM_HAS_EXPLICIT_BZERO)
...
#elif defined(AWRTC_MBEDTLS_COMPILER_IS_GCC)
...
{
    if (buf != NULL) {
        awrtc_mbedtls_platform_zeroize(buf, len);
    }
                                     ...
                                     struct tm *tm_buf)
{
    ...
    if (awrtc_mbedtls_mutex_lock(&awrtc_mbedtls_threading_gmtime_mutex) != 0) {
        return NULL;
    }
...
#if defined(AWRTC_MBEDTLS_THREADING_C)
    if (awrtc_mbedtls_mutex_unlock(&awrtc_mbedtls_threading_gmtime_mutex) != 0) {
        return NULL;
    }
#endif /* AWRTC_MBEDTLS_THREADING_C */
...
void (*awrtc_mbedtls_test_hook_test_fail)(const char *, int, const char *);
#endif /* AWRTC_MBEDTLS_TEST_HOOKS */

#if defined(AWRTC_MBEDTLS_HAVE_TIME) && !defined(AWRTC_MBEDTLS_PLATFORM_MS_TIME_ALT)
...
awrtc_mbedtls_ms_time_t awrtc_mbedtls_ms_time(void)
{
    int ret;
    struct timespec tv;
    awrtc_mbedtls_ms_time_t current_ms;
...
awrtc_mbedtls_ms_time_t awrtc_mbedtls_ms_time(void)
{
    ...
    current_ms = ((awrtc_mbedtls_ms_time_t) ct.dwLowDateTime +
                  ((awrtc_mbedtls_ms_time_t) (ct.dwHighDateTime) << 32LL))/10000;
    return current_ms;
}
#else
#error "No awrtc_mbedtls_ms_time available"
#endif
#endif /* AWRTC_MBEDTLS_HAVE_TIME && !AWRTC_MBEDTLS_PLATFORM_MS_TIME_ALT */
