...
#if defined(AWRTC_MBEDTLS_SSL_CLI_C) && defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_2)
...
static int ssl_write_renegotiation_ext(awrtc_mbedtls_ssl_context *ssl,
                                       unsigned char *buf,
                                       ...
                                       size_t *olen)
{
    ...
    *olen = 0;
    ...
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 5 + ssl->verify_data_len);
    ...
    *p++ = 0x00;
    *p++ = AWRTC_MBEDTLS_BYTE_0(ssl->verify_data_len + 1);
    *p++ = AWRTC_MBEDTLS_BYTE_0(ssl->verify_data_len);
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_supported_point_formats_ext(awrtc_mbedtls_ssl_context *ssl,
                                                 unsigned char *buf,
                                                 const unsigned char *end,
                                                 size_t *olen)
{
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_ecjpake_kkpp_ext(awrtc_mbedtls_ssl_context *ssl,
                                      unsigned char *buf,
                                      const unsigned char *end,
                                      size_t *olen)
{
    ...
    *olen = 0;
    ...
    if (ssl->handshake->ecjpake_cache == NULL ||
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("generating new ecjpake parameters"));
        ...
        if (ret != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1,
                                  "awrtc_mbedtls_ecjpake_write_round_one", ret);
            return ret;
        }
#endif /* AWRTC_MBEDTLS_USE_PSA_CRYPTO */

        ssl->handshake->ecjpake_cache = awrtc_mbedtls_calloc(1, kkpp_len);
        if (ssl->handshake->ecjpake_cache == NULL) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("allocation failed"));
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("re-using cached ecjpake parameters"));

        kkpp_len = ssl->handshake->ecjpake_cache_len;
        AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p + 2, end, kkpp_len);
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_cid_ext(awrtc_mbedtls_ssl_context *ssl,
                             unsigned char *buf,
                             const unsigned char *end,
                             size_t *olen)
{
    ...
    *olen = 0;
    if (ssl->conf->transport != AWRTC_MBEDTLS_SSL_TRANSPORT_DATAGRAM ||
        ssl->negotiate_cid == AWRTC_MBEDTLS_SSL_CID_DISABLED) {
        return 0;
    }
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, adding CID extension"));

    /* ssl->own_cid_len is at most AWRTC_MBEDTLS_SSL_CID_IN_LEN_MAX
     * which is at most 255, so the increment cannot overflow. */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, (unsigned) (ssl->own_cid_len + 5));
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_max_fragment_length_ext(awrtc_mbedtls_ssl_context *ssl,
                                             unsigned char *buf,
                                             const unsigned char *end,
                                             size_t *olen)
{
    ...
    *olen = 0;
    ...
    if (ssl->conf->mfl_code == AWRTC_MBEDTLS_SSL_MAX_FRAG_LEN_NONE) {
        return 0;
    }
    ...
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 5);
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_encrypt_then_mac_ext(awrtc_mbedtls_ssl_context *ssl,
                                          unsigned char *buf,
                                          const unsigned char *end,
                                          size_t *olen)
{
    ...
    *olen = 0;
    ...
    if (ssl->conf->encrypt_then_mac == AWRTC_MBEDTLS_SSL_ETM_DISABLED) {
        return 0;
    }
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_extended_ms_ext(awrtc_mbedtls_ssl_context *ssl,
                                     unsigned char *buf,
                                     const unsigned char *end,
                                     size_t *olen)
{
    ...
    *olen = 0;
    ...
    if (ssl->conf->extended_ms == AWRTC_MBEDTLS_SSL_EXTENDED_MS_DISABLED) {
        return 0;
    }
...
#if defined(AWRTC_MBEDTLS_SSL_SESSION_TICKETS)
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_session_ticket_ext(awrtc_mbedtls_ssl_context *ssl,
                                        unsigned char *buf,
                                        const unsigned char *end,
                                        size_t *olen)
{
    ...
    *olen = 0;
    ...
    if (awrtc_mbedtls_ssl_conf_get_session_tickets(ssl->conf) ==
    ...
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 4 + tlen);

    AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_TLS_EXT_SESSION_TICKET, p, 0);
    p += 2;

    AWRTC_MBEDTLS_PUT_UINT16_BE(tlen, p, 0);
    p += 2;
    ...
    *olen = 4;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3,
                          ("sending session ticket of length %" AWRTC_MBEDTLS_PRINTF_SIZET, tlen));
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_use_srtp_ext(awrtc_mbedtls_ssl_context *ssl,
                                  unsigned char *buf,
                                  const unsigned char *end,
                                  size_t *olen)
{
    ...
    *olen = 0;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, adding use_srtp extension"));

    /* Check there is room in the buffer for the extension + 4 bytes
     * - the extension tag (2 bytes)
     * - the extension length (2 bytes)
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, ext_len + 4);

    AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_TLS_EXT_USE_SRTP, p, 0);
    p += 2;
    ...
    /* micro-optimization:
     * the list size is limited to AWRTC_MBEDTLS_TLS_SRTP_MAX_PROFILE_LIST_LENGTH
     ...
     */
    *p++ = 0;
    *p++ = AWRTC_MBEDTLS_BYTE_0(2 * ssl->conf->dtls_srtp_profile_list_len);
    ...
    for (protection_profiles_index = 0;
        ...
        if (profile_value != AWRTC_MBEDTLS_TLS_SRTP_UNSET) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("ssl_write_use_srtp_ext, add profile: %04x",
                                      profile_value));
            AWRTC_MBEDTLS_PUT_UINT16_BE(profile_value, p, 0);
            p += 2;
        } else {
            /*
             * Note: we shall never arrive here as protection profiles
             * is checked by awrtc_mbedtls_ssl_conf_dtls_srtp_protection_profiles function
             */
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(3,
                                  ("client hello, "
    ...
    *p++ = mki_len & 0xFF;
    ...
    if (mki_len != 0) {
         ...
         */
        p += mki_len;
        AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "sending mki",  ssl->dtls_srtp_info.mki_value,
                                              ...
                                              size_t *out_len)
{
    ...
    *out_len = 0;
     ...
     * even if AWRTC_MBEDTLS_SSL_RENEGOTIATION is not defined. */
#if defined(AWRTC_MBEDTLS_SSL_RENEGOTIATION)
    if ((ret = ssl_write_renegotiation_ext(ssl, p, end, &ext_len)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_renegotiation_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
    ...
    if (uses_ec) {
        if ((ret = ssl_write_supported_point_formats_ext(ssl, p, end,
                                                         &ext_len)) != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_supported_point_formats_ext", ret);
            return ret;
        }
        p += ext_len;
    }
    ...
    if ((ret = ssl_write_ecjpake_kkpp_ext(ssl, p, end, &ext_len)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_ecjpake_kkpp_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
    ...
    if ((ret = ssl_write_cid_ext(ssl, p, end, &ext_len)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_cid_ext", ret);
        return ret;
    }
    p += ext_len;
#endif /* AWRTC_MBEDTLS_SSL_DTLS_CONNECTION_ID */
    ...
    if ((ret = ssl_write_max_fragment_length_ext(ssl, p, end,
                                                 &ext_len)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_max_fragment_length_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
    ...
    if ((ret = ssl_write_encrypt_then_mac_ext(ssl, p, end, &ext_len)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_encrypt_then_mac_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
    ...
    if ((ret = ssl_write_extended_ms_ext(ssl, p, end, &ext_len)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_extended_ms_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
    ...
    if ((ret = ssl_write_use_srtp_ext(ssl, p, end, &ext_len)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_use_srtp_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
    ...
    if ((ret = ssl_write_session_ticket_ext(ssl, p, end, &ext_len)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_session_ticket_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
                                        ...
                                        size_t len)
{
    ...
    if (ssl->renego_status != AWRTC_MBEDTLS_SSL_INITIAL_HANDSHAKE) {
        ...
        if (len    != 1 + ssl->verify_data_len * 2 ||
            ...
            awrtc_mbedtls_ct_memcmp(buf + 1,
                              ssl->own_verify_data, ssl->verify_data_len) != 0 ||
            awrtc_mbedtls_ct_memcmp(buf + 1 + ssl->verify_data_len,
                             ...
                             size_t len)
{
    ...
    if ( /* CID extension only makes sense in DTLS */
        ...
        ssl->negotiate_cid == AWRTC_MBEDTLS_SSL_CID_DISABLED) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("CID extension unexpected"));
    ...
    if (len == 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("CID extension invalid"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "Server CID", buf, peer_cid_len);
                                        ...
                                        size_t len)
{
    ...
    defined(AWRTC_MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_supported_point_formats_ext(awrtc_mbedtls_ssl_context *ssl,
                                                 const unsigned char *buf,
                                                 size_t len)
{
    size_t list_size;
    ...
    if (len == 0 || (size_t) (buf[0] + 1) != len) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
    }
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("no point format in common"));
    awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                   AWRTC_MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE);
    return AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
}
                                  ...
                                  size_t len)
{
    ...
    awrtc_mbedtls_free(ssl->handshake->ecjpake_cache);
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_alpn_ext(awrtc_mbedtls_ssl_context *ssl,
                              const unsigned char *buf, size_t len)
{
    ...
    if (ssl->conf->alpn_list == NULL) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("non-matching ALPN extension"));
        awrtc_mbedtls_ssl_send_alert_message(
            ssl,
            AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
            AWRTC_MBEDTLS_SSL_ALERT_MSG_UNSUPPORTED_EXT);
    ...
    list_len = AWRTC_MBEDTLS_GET_UINT16_BE(buf, 0);
    if (list_len != len - 2) {
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("ALPN extension: no matching protocol"));
    awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                   AWRTC_MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE);
    return AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
}
...
static int ssl_parse_use_srtp_ext(awrtc_mbedtls_ssl_context *ssl,
                                  ...
                                  size_t len)
{
    awrtc_mbedtls_ssl_srtp_profile server_protection = AWRTC_MBEDTLS_TLS_SRTP_UNSET;
    size_t i, mki_len = 0;
    ...
    server_protection = awrtc_mbedtls_ssl_check_srtp_profile_value(
        server_protection_profile_value);
    if (server_protection != AWRTC_MBEDTLS_TLS_SRTP_UNSET) {
    ...
    if (len > 5) {
        AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "received mki", ssl->dtls_srtp_info.mki_value,
                              ssl->dtls_srtp_info.mki_len);
    }
#endif
    return 0;
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_hello_verify_request(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
    const unsigned char *p = ssl->in_msg + awrtc_mbedtls_ssl_hs_hdr_len(ssl);
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> parse hello verify request"));

    /* Check that there is enough room for:
     * - 2 bytes of version
     * - 1 byte of cookie_len
     */
    if (awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 3 > ssl->in_msglen) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1,
    ...
    dtls_legacy_version = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
    p += 2;
    ...
    if (dtls_legacy_version != 0xfefd && dtls_legacy_version != 0xfeff) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server version"));
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "cookie", p, cookie_len);

    awrtc_mbedtls_free(ssl->handshake->cookie);

    ssl->handshake->cookie = awrtc_mbedtls_calloc(1, cookie_len);
    if (ssl->handshake->cookie  == NULL) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("alloc failed (%d bytes)", cookie_len));
        return AWRTC_MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_server_hello(awrtc_mbedtls_ssl_context *ssl)
{
    int ret, i;
    size_t n;
    size_t ext_len;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> parse server hello"));
    ...
    if (ssl->in_msgtype != AWRTC_MBEDTLS_SSL_MSG_HANDSHAKE) {
        ...
        if (ssl->renego_status == AWRTC_MBEDTLS_SSL_RENEGOTIATION_IN_PROGRESS) {
            ...
            return AWRTC_MBEDTLS_ERR_SSL_WAITING_SERVER_HELLO_RENEGO;
        }
#endif /* AWRTC_MBEDTLS_SSL_RENEGOTIATION */
...
#if defined(AWRTC_MBEDTLS_SSL_PROTO_DTLS)
    if (ssl->conf->transport == AWRTC_MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        if (buf[0] == AWRTC_MBEDTLS_SSL_HS_HELLO_VERIFY_REQUEST) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("received hello verify request"));
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("<= parse server hello"));
    ...
    if (ssl->in_hslen < 38 + awrtc_mbedtls_ssl_hs_hdr_len(ssl) ||
        buf[0] != AWRTC_MBEDTLS_SSL_HS_SERVER_HELLO) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "server hello, version", buf, 2);
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, current time: %lu",
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3,   "server hello, random bytes", buf + 2, 32);
    ...
    if (ssl->in_hslen > awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 39 + n) {
        ext_len = AWRTC_MBEDTLS_GET_UINT16_BE(buf, 38 + n);

        if ((ext_len > 0 && ext_len < 4) ||
            ssl->in_hslen != awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 40 + n + ext_len) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
            awrtc_mbedtls_ssl_send_alert_message(
                ssl,
                AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
    ...
    } else if (ssl->in_hslen == awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 38 + n) {
        ext_len = 0;
    } else {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
    ...
    i = (int) AWRTC_MBEDTLS_GET_UINT16_BE(buf, n + 35);
    ...
    if (comp != AWRTC_MBEDTLS_SSL_COMPRESS_NULL) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1,
     ...
     */
    ssl->handshake->ciphersuite_info = awrtc_mbedtls_ssl_ciphersuite_from_id(i);
    ...
    awrtc_mbedtls_ssl_optimize_checksum(ssl, ssl->handshake->ciphersuite_info);

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, session id len.: %" AWRTC_MBEDTLS_PRINTF_SIZET, n));
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3,   "server hello, session id", buf + 35, n);
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, chosen ciphersuite: %04x", (unsigned) i));
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, compress alg.: %d",
                              buf[37 + n]));
    ...
    suite_info = awrtc_mbedtls_ssl_ciphersuite_from_id(
        ssl->session_negotiate->ciphersuite);
    if (awrtc_mbedtls_ssl_validate_ciphersuite(ssl, suite_info, ssl->tls_version,
                                         ssl->tls_version) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
        awrtc_mbedtls_ssl_send_alert_message(
            ssl,
    ...
    if (suite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA &&
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2,
                          ("server hello, total extension length: %" AWRTC_MBEDTLS_PRINTF_SIZET,
                           ext_len));

    while (ext_len) {
        unsigned int ext_id   = AWRTC_MBEDTLS_GET_UINT16_BE(ext, 0);
        unsigned int ext_size = AWRTC_MBEDTLS_GET_UINT16_BE(ext, 2);

        if (ext_size + 4 > ext_len) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
            awrtc_mbedtls_ssl_send_alert_message(
                ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
            return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
        }
        ...
        switch (ext_id) {
            ...
            case AWRTC_MBEDTLS_TLS_EXT_ENCRYPT_THEN_MAC:
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("found encrypt_then_mac extension"));
            ...
            case AWRTC_MBEDTLS_TLS_EXT_SESSION_TICKET:
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("found session_ticket extension"));
          ...
          AWRTC_MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED */
            ...
            case AWRTC_MBEDTLS_TLS_EXT_ECJPAKE_KKPP:
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("found ecjpake_kkpp extension"));
    ...
    /*
     * awrtc_mbedtls_ssl_derive_keys() has to be called after the parsing of the
     ...
     */
    if (ssl->secure_renegotiation == AWRTC_MBEDTLS_SSL_LEGACY_RENEGOTIATION &&
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_server_dh_params(awrtc_mbedtls_ssl_context *ssl,
                                      unsigned char **p,
                                      unsigned char *end)
{
    int ret = AWRTC_MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
     ...
     * } ServerDHParams;
     */
    if ((ret = awrtc_mbedtls_dhm_read_params(&ssl->handshake->dhm_ctx,
                                       p, end)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(2, ("awrtc_mbedtls_dhm_read_params"), ret);
        return ret;
    }

    dhm_actual_bitlen = awrtc_mbedtls_dhm_get_bitlen(&ssl->handshake->dhm_ctx);
    if (dhm_actual_bitlen < ssl->conf->dhm_min_bitlen) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("DHM prime too short: %" AWRTC_MBEDTLS_PRINTF_SIZET " < %u",
                                  dhm_actual_bitlen,
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_server_ecdh_params(awrtc_mbedtls_ssl_context *ssl,
                                        unsigned char **p,
                                        unsigned char *end)
{
    uint16_t tls_id;
    ...
    if (*(*p)++ != AWRTC_MBEDTLS_ECP_TLS_NAMED_CURVE) {
        return AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }

    /* Next two bytes are the namedcurve value */
    tls_id = AWRTC_MBEDTLS_GET_UINT16_BE(*p, 0);
    *p += 2;

    /* Check it's a curve we offered */
    if (awrtc_mbedtls_ssl_check_curve_tls_id(ssl, tls_id) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2,
    ...
    if (awrtc_mbedtls_ssl_get_psa_curve_info_from_tls_id(tls_id, &key_type,
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_check_server_ecdh_params(const awrtc_mbedtls_ssl_context *ssl)
{
    uint16_t tls_id;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("ECDH curve: %s",
                              awrtc_mbedtls_ssl_get_curve_name_from_tls_id(tls_id)));

    if (awrtc_mbedtls_ssl_check_curve(ssl, grp_id) != 0) {
        return -1;
    }
                                        ...
                                        unsigned char *end)
{
     ...
     * } ServerECDHParams;
     */
    if ((ret = awrtc_mbedtls_ecdh_read_params(&ssl->handshake->ecdh_ctx,
                                        (const unsigned char **) p, end)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, ("awrtc_mbedtls_ecdh_read_params"), ret);
#if defined(AWRTC_MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
        if (ret == AWRTC_MBEDTLS_ERR_ECP_IN_PROGRESS) {
            ret = AWRTC_MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
        }
#endif
        return ret;
    }
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_server_psk_hint(awrtc_mbedtls_ssl_context *ssl,
                                     unsigned char **p,
                                     unsigned char *end)
{
    int ret = AWRTC_MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
    ...
    len = AWRTC_MBEDTLS_GET_UINT16_BE(*p, 0);
    *p += 2;
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_encrypted_pms(awrtc_mbedtls_ssl_context *ssl,
                                   ...
                                   size_t pms_offset)
{
    ...
    if (offset + len_bytes > AWRTC_MBEDTLS_SSL_OUT_CONTENT_LEN) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("buffer too small for encrypted pms"));
     ...
     *  } PreMasterSecret;
     */
    awrtc_mbedtls_ssl_write_version(p, ssl->conf->transport,
     ...
     */
    if (!awrtc_mbedtls_pk_can_do(peer_pk, AWRTC_MBEDTLS_PK_RSA)) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("certificate key type mismatch"));
        return AWRTC_MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH;
    }

    if ((ret = awrtc_mbedtls_pk_encrypt(peer_pk,
                                  p, ssl->handshake->pmslen,
                                  ssl->out_msg + offset + len_bytes, olen,
                                  AWRTC_MBEDTLS_SSL_OUT_CONTENT_LEN - offset - len_bytes,
                                  ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "awrtc_mbedtls_pk_encrypt", ret);
        return ret;
    }

    if (len_bytes == 2) {
        AWRTC_MBEDTLS_PUT_UINT16_BE(*olen, ssl->out_msg, offset);
        *olen += 2;
    }

#if !defined(AWRTC_MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
    /* We don't need the peer's public key anymore. Free it. */
    awrtc_mbedtls_pk_free(peer_pk);
#endif /* !AWRTC_MBEDTLS_SSL_KEEP_PEER_CERTIFICATE */
    return 0;
}
...
static int ssl_get_ecdh_params_from_cert(awrtc_mbedtls_ssl_context *ssl)
{
    ...
    if (!awrtc_mbedtls_pk_can_do(peer_pk, AWRTC_MBEDTLS_PK_ECKEY)) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("server key not ECDH capable"));
        return AWRTC_MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH;
    }

#if !defined(AWRTC_MBEDTLS_PK_USE_PSA_EC_DATA)
    const awrtc_mbedtls_ecp_keypair *peer_key = awrtc_mbedtls_pk_ec_ro(*peer_pk);
#endif /* !defined(AWRTC_MBEDTLS_PK_USE_PSA_EC_DATA) */

#if defined(AWRTC_MBEDTLS_USE_PSA_CRYPTO)
    uint16_t tls_id = 0;
    awrtc_psa_key_type_t key_type = AWRTC_PSA_KEY_TYPE_NONE;
    awrtc_mbedtls_ecp_group_id grp_id = awrtc_mbedtls_pk_get_ec_group_id(peer_pk);

    if (awrtc_mbedtls_ssl_check_curve(ssl, grp_id) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server certificate (ECDH curve)"));
        return AWRTC_MBEDTLS_ERR_SSL_BAD_CERTIFICATE;
    }

    tls_id = awrtc_mbedtls_ssl_get_tls_id_from_ecp_group_id(grp_id);
    if (tls_id == 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("ECC group %u not supported",
                                  grp_id));
        return AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
    }
    ...
    size_t olen = 0;
    ret = awrtc_mbedtls_ecp_point_write_binary(&peer_key->grp, &peer_key->Q,
                                         AWRTC_MBEDTLS_ECP_PF_UNCOMPRESSED, &olen,
                                         ssl->handshake->xxdh_psa_peerkey,
                                         sizeof(ssl->handshake->xxdh_psa_peerkey));

    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, ("awrtc_mbedtls_ecp_point_write_binary"), ret);
        return ret;
    }
    ssl->handshake->xxdh_psa_peerkey_len = olen;
#endif /* AWRTC_MBEDTLS_PK_USE_PSA_EC_DATA */
#else /* AWRTC_MBEDTLS_USE_PSA_CRYPTO */
    if ((ret = awrtc_mbedtls_ecdh_get_params(&ssl->handshake->ecdh_ctx, peer_key,
                                       AWRTC_MBEDTLS_ECDH_THEIRS)) != 0) {
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_signature_algorithm(awrtc_mbedtls_ssl_context *ssl,
                                         uint16_t sig_alg,
                                         awrtc_mbedtls_md_type_t *md_alg,
                                         awrtc_mbedtls_pk_type_t *pk_alg)
{
    if (awrtc_mbedtls_ssl_get_pk_type_and_md_alg_from_sig_alg(sig_alg, pk_alg, md_alg) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1,
                              ("Server used unsupported value in SigAlg extension 0x%04x",
                               sig_alg));
        return AWRTC_MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER;
    }

    /*
     * awrtc_mbedtls_ssl_get_pk_sigalg_and_md_alg_from_sig_alg() understands sig_alg code points across
    ...
    if (!awrtc_mbedtls_ssl_sig_alg_is_offered(ssl, sig_alg)) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("Server used SigAlg value 0x%04x that was not offered", sig_alg));
        return AWRTC_MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER;
    }

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("Server used SignatureAlgorithm %d", sig_alg & 0x00FF));
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("Server used HashAlgorithm %d", sig_alg >> 8));
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_server_key_exchange(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> parse server key exchange"));

#if defined(AWRTC_MBEDTLS_KEY_EXCHANGE_RSA_ENABLED)
    if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_RSA) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("<= skip parse server key exchange"));
        awrtc_mbedtls_ssl_handshake_increment_state(ssl);
        return 0;
    }
    ((void) p);
...
#if defined(AWRTC_MBEDTLS_KEY_EXCHANGE_ECDH_RSA_ENABLED) || \
    defined(AWRTC_MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED)
    if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_ECDH_RSA ||
    ...
    if (ssl->in_msgtype != AWRTC_MBEDTLS_SSL_MSG_HANDSHAKE) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server key exchange message"));
        awrtc_mbedtls_ssl_send_alert_message(
            ssl,
            AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
            AWRTC_MBEDTLS_SSL_ALERT_MSG_UNEXPECTED_MESSAGE);
     ...
     */
    if (ssl->in_msg[0] != AWRTC_MBEDTLS_SSL_HS_SERVER_KEY_EXCHANGE) {
        if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_PSK ||
            ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_RSA_PSK) {
...
start_processing:
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3,   "server key exchange", p, (size_t) (end - p));

#if defined(AWRTC_MBEDTLS_KEY_EXCHANGE_SOME_PSK_ENABLED)
    if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_PSK ||
        ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_RSA_PSK ||
        ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_DHE_PSK ||
    ...
    if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
         ...
         * that TLS ID here
         */
        uint16_t read_tls_id = AWRTC_MBEDTLS_GET_UINT16_BE(p, 1);
        uint16_t exp_tls_id = awrtc_mbedtls_ssl_get_tls_id_from_ecp_group_id(
        ...
        if ((ret = awrtc_mbedtls_psa_ecjpake_read_round(
                 &ssl->handshake->awrtc_psa_pake_ctx, p, end - p,
    ...
    if (awrtc_mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
        ...
        unsigned char *params = ssl->in_msg + awrtc_mbedtls_ssl_hs_hdr_len(ssl);
        ...
        uint16_t sig_alg = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
        ...
        sig_len = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
        p += 2;
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "signature", p, sig_len);
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_certificate_request(awrtc_mbedtls_ssl_context *ssl)
{
    const awrtc_mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
    ...
    if (!awrtc_mbedtls_ssl_ciphersuite_cert_req_allowed(ciphersuite_info)) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("<= skip parse certificate request"));
        awrtc_mbedtls_ssl_handshake_increment_state(ssl);
        return 0;
    }
...
static int ssl_parse_certificate_request(awrtc_mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->in_msgtype != AWRTC_MBEDTLS_SSL_MSG_HANDSHAKE) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad certificate request message"));
        awrtc_mbedtls_ssl_send_alert_message(
            ssl,
            AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
            AWRTC_MBEDTLS_SSL_ALERT_MSG_UNEXPECTED_MESSAGE);
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("got %s certificate request",
    ...
    cert_type_len = buf[awrtc_mbedtls_ssl_hs_hdr_len(ssl)];
    n = cert_type_len;
     ...
     */
    if (ssl->in_hslen <= awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 2 + n) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad certificate request message"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
    ...
    sig_alg_len = AWRTC_MBEDTLS_GET_UINT16_BE(buf, awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 1 + n);
     ...
     */
    if (ssl->in_hslen <= awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 3 + n + sig_alg_len) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad certificate request message"));
        awrtc_mbedtls_ssl_send_alert_message(
            ssl,
            AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
...
#if defined(AWRTC_MBEDTLS_DEBUG_C)
    sig_alg = buf + awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 3 + n;
    ...
    dn_len = AWRTC_MBEDTLS_GET_UINT16_BE(buf, awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 1 + n);

    n += dn_len;
    if (ssl->in_hslen != awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 3 + n) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad certificate request message"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
...
#if defined(AWRTC_MBEDTLS_DEBUG_C)
    dn = buf + awrtc_mbedtls_ssl_hs_hdr_len(ssl) + 3 + n - dn_len;
    for (size_t i = 0, dni_len = 0; i < dn_len; i += 2 + dni_len) {
        unsigned char *p = dn + i + 2;
        awrtc_mbedtls_x509_name name;
        size_t asn1_len;
        char s[AWRTC_MBEDTLS_X509_MAX_DN_NAME_SIZE];
        memset(&name, 0, sizeof(name));
        dni_len = AWRTC_MBEDTLS_GET_UINT16_BE(dn + i, 0);
        if (dni_len > dn_len - i - 2 ||
            awrtc_mbedtls_asn1_get_tag(&p, p + dni_len, &asn1_len,
                                 AWRTC_MBEDTLS_ASN1_CONSTRUCTED | AWRTC_MBEDTLS_ASN1_SEQUENCE) != 0 ||
            awrtc_mbedtls_x509_get_name(&p, p + asn1_len, &name) != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad certificate request message"));
            awrtc_mbedtls_ssl_send_alert_message(
                ssl,
                AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3,
                              ("DN hint: %.*s",
                               awrtc_mbedtls_x509_dn_gets(s, sizeof(s), &name), s));
        awrtc_mbedtls_asn1_free_named_data_list_shallow(name.next);
    }
#endif
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_server_hello_done(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> parse server hello done"));
    ...
    if (ssl->in_msgtype != AWRTC_MBEDTLS_SSL_MSG_HANDSHAKE) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello done message"));
        return AWRTC_MBEDTLS_ERR_SSL_UNEXPECTED_MESSAGE;
    }

    if (ssl->in_hslen  != awrtc_mbedtls_ssl_hs_hdr_len(ssl) ||
        ssl->in_msg[0] != AWRTC_MBEDTLS_SSL_HS_SERVER_HELLO_DONE) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello done message"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_client_key_exchange(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> write client key exchange"));
    ...
    if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_DHE_RSA) {
         ...
         */
        content_len = awrtc_mbedtls_dhm_get_len(&ssl->handshake->dhm_ctx);

        AWRTC_MBEDTLS_PUT_UINT16_BE(content_len, ssl->out_msg, 4);
        header_len = 6;

        ret = awrtc_mbedtls_dhm_make_public(&ssl->handshake->dhm_ctx,
                                      (int) awrtc_mbedtls_dhm_get_len(&ssl->handshake->dhm_ctx),
                                      &ssl->out_msg[header_len], content_len,
                                      ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "awrtc_mbedtls_dhm_make_public", ret);
            return ret;
        }
    ...
    if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
        ...
        unsigned char *end = ssl->out_msg + AWRTC_MBEDTLS_SSL_OUT_CONTENT_LEN;
        ...
        if (status != AWRTC_PSA_SUCCESS || destruction_status != AWRTC_PSA_SUCCESS) {
            return AWRTC_MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
        }
#else
        /*
        ...
        if (ssl->handshake->ecrs_enabled) {
            ...
            awrtc_mbedtls_ecdh_enable_restart(&ssl->handshake->ecdh_ctx);
        }
#endif

        ret = awrtc_mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx,
                                       &content_len,
                                       &ssl->out_msg[header_len], 1000,
                                       ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "awrtc_mbedtls_ecdh_make_public", ret);
#if defined(AWRTC_MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
            if (ret == AWRTC_MBEDTLS_ERR_ECP_IN_PROGRESS) {
                ret = AWRTC_MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
            }
...
ecdh_calc_secret:
    ...
    if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
         ...
         */
        if (awrtc_mbedtls_ssl_conf_has_static_psk(ssl->conf) == 0) {
        ...
        *p++ = AWRTC_MBEDTLS_BYTE_1(ssl->conf->psk_identity_len);
        *p++ = AWRTC_MBEDTLS_BYTE_0(ssl->conf->psk_identity_len);
        ...
        *p = (unsigned char) own_pubkey_len;
        ...
        AWRTC_MBEDTLS_PUT_UINT16_BE(zlen, pms, 0);
        pms += zlen_size + zlen;
    } else
#endif /* AWRTC_MBEDTLS_USE_PSA_CRYPTO &&
          AWRTC_MBEDTLS_KEY_EXCHANGE_ECDHE_PSK_ENABLED */
#if defined(AWRTC_MBEDTLS_KEY_EXCHANGE_SOME_PSK_ENABLED)
    if (awrtc_mbedtls_ssl_ciphersuite_uses_psk(ciphersuite_info)) {
        /*
         * opaque psk_identity<0..2^16-1>;
         */
        if (awrtc_mbedtls_ssl_conf_has_static_psk(ssl->conf) == 0) {
        ...
        if (header_len + 2 + content_len > AWRTC_MBEDTLS_SSL_OUT_CONTENT_LEN) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(1,
                                  ("psk identity too long or SSL buffer too short"));
            return AWRTC_MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL;
        }

        ssl->out_msg[header_len++] = AWRTC_MBEDTLS_BYTE_1(content_len);
        ssl->out_msg[header_len++] = AWRTC_MBEDTLS_BYTE_0(content_len);
        ...
        if (ciphersuite_info->key_exchange == AWRTC_MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
            ...
            ret = awrtc_mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx,
                                           ...
                                           AWRTC_MBEDTLS_SSL_OUT_CONTENT_LEN - header_len,
                                           ssl->conf->f_rng, ssl->conf->p_rng);
            if (ret != 0) {
                AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "awrtc_mbedtls_ecdh_make_public", ret);
                return ret;
...
#endif /* !AWRTC_MBEDTLS_USE_PSA_CRYPTO && AWRTC_MBEDTLS_KEY_EXCHANGE_ECDHE_PSK_ENABLED */
        {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
            return AWRTC_MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        }
        ...
        if ((ret = awrtc_mbedtls_ssl_psk_derive_premaster(ssl,
                                                    ...
                                                    key_exchange)) != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1,
                                  "awrtc_mbedtls_ssl_psk_derive_premaster", ret);
            return ret;
        }
#endif /* !AWRTC_MBEDTLS_USE_PSA_CRYPTO */
    } else
...
#else
        ret = awrtc_mbedtls_ecjpake_write_round_two(&ssl->handshake->ecjpake_ctx,
                                              ssl->out_msg + header_len,
                                              AWRTC_MBEDTLS_SSL_OUT_CONTENT_LEN - header_len,
                                              &content_len,
                                              ssl->conf->f_rng, ssl->conf->p_rng);
    ...
    ssl->out_msg[0]  = AWRTC_MBEDTLS_SSL_HS_CLIENT_KEY_EXCHANGE;
...
#if !defined(AWRTC_MBEDTLS_KEY_EXCHANGE_CERT_REQ_ALLOWED_ENABLED)
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_certificate_verify(awrtc_mbedtls_ssl_context *ssl)
{
    const awrtc_mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
static int ssl_write_certificate_verify(awrtc_mbedtls_ssl_context *ssl)
{
    ...
    if (awrtc_mbedtls_ssl_own_key(ssl) == NULL) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("got no private key for certificate"));
...
sign:
    ...
    if (ssl->handshake->ciphersuite_info->mac == AWRTC_MBEDTLS_MD_SHA384) {
        ...
        ssl->out_msg[4] = AWRTC_MBEDTLS_SSL_HASH_SHA384;
    } else {
        md_alg = AWRTC_MBEDTLS_MD_SHA256;
        ssl->out_msg[4] = AWRTC_MBEDTLS_SSL_HASH_SHA256;
    }
    ssl->out_msg[5] = awrtc_mbedtls_ssl_sig_from_pk(awrtc_mbedtls_ssl_own_key(ssl));
    ...
    if ((ret = awrtc_mbedtls_pk_sign_restartable(awrtc_mbedtls_ssl_own_key(ssl),
    ...
    AWRTC_MBEDTLS_PUT_UINT16_BE(n, ssl->out_msg, offset + 4);

    ssl->out_msglen  = 6 + n + offset;
    ssl->out_msgtype = AWRTC_MBEDTLS_SSL_MSG_HANDSHAKE;
    ssl->out_msg[0]  = AWRTC_MBEDTLS_SSL_HS_CERTIFICATE_VERIFY;
...
#if defined(AWRTC_MBEDTLS_SSL_SESSION_TICKETS)
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_new_session_ticket(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    uint32_t lifetime;
    size_t ticket_len;
    ...
    if (ssl->in_msgtype != AWRTC_MBEDTLS_SSL_MSG_HANDSHAKE) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad new session ticket message"));
        awrtc_mbedtls_ssl_send_alert_message(
            ssl,
            AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
            AWRTC_MBEDTLS_SSL_ALERT_MSG_UNEXPECTED_MESSAGE);
     ...
     */
    if (ssl->in_msg[0] != AWRTC_MBEDTLS_SSL_HS_NEW_SESSION_TICKET ||
        ssl->in_hslen < 6 + awrtc_mbedtls_ssl_hs_hdr_len(ssl)) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad new session ticket message"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
    ...
    msg = ssl->in_msg + awrtc_mbedtls_ssl_hs_hdr_len(ssl);

    lifetime = AWRTC_MBEDTLS_GET_UINT32_BE(msg, 0);

    ticket_len = AWRTC_MBEDTLS_GET_UINT16_BE(msg, 4);

    if (ticket_len + 6 + awrtc_mbedtls_ssl_hs_hdr_len(ssl) != ssl->in_hslen) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad new session ticket message"));
        awrtc_mbedtls_ssl_send_alert_message(ssl, AWRTC_MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       AWRTC_MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return AWRTC_MBEDTLS_ERR_SSL_DECODE_ERROR;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("ticket length: %" AWRTC_MBEDTLS_PRINTF_SIZET, ticket_len));
    ...
    if (ssl->session != NULL && ssl->session->ticket != NULL) {
        awrtc_mbedtls_zeroize_and_free(ssl->session->ticket,
                                 ssl->session->ticket_len);
        ssl->session->ticket = NULL;
        ssl->session->ticket_len = 0;
    }

    awrtc_mbedtls_zeroize_and_free(ssl->session_negotiate->ticket,
     ...
     */
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("ticket in use, discarding session id"));
...
int awrtc_mbedtls_ssl_handshake_client_step(awrtc_mbedtls_ssl_context *ssl)
{
    ...
    /* Change state now, so that it is right in awrtc_mbedtls_ssl_read_record(), used
     * by DTLS for dropping out-of-sequence ChangeCipherSpec records */
#if defined(AWRTC_MBEDTLS_SSL_SESSION_TICKETS)
    if (ssl->state == AWRTC_MBEDTLS_SSL_SERVER_CHANGE_CIPHER_SPEC &&
        ssl->handshake->new_session_ticket != 0) {
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_NEW_SESSION_TICKET);
    }
#endif
...
#endif /* AWRTC_MBEDTLS_SSL_CLI_C && AWRTC_MBEDTLS_SSL_PROTO_TLS1_2 */
