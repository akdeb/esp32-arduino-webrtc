...
#endif /* x86 gnuc */
#endif /* AWRTC_MBEDTLS_HAVE_ASM && GNUC */
#if !defined(ARIA_P1)
 ...
 */
#define ARIA_P3(x) AWRTC_MBEDTLS_BSWAP32(x)
...
{
    *a = ((uint32_t) sa[AWRTC_MBEDTLS_BYTE_0(*a)]) ^
         (((uint32_t) sb[AWRTC_MBEDTLS_BYTE_1(*a)]) <<  8) ^
         (((uint32_t) sc[AWRTC_MBEDTLS_BYTE_2(*a)]) << 16) ^
         (((uint32_t) sd[AWRTC_MBEDTLS_BYTE_3(*a)]) << 24);
    *b = ((uint32_t) sa[AWRTC_MBEDTLS_BYTE_0(*b)]) ^
         (((uint32_t) sb[AWRTC_MBEDTLS_BYTE_1(*b)]) <<  8) ^
         (((uint32_t) sc[AWRTC_MBEDTLS_BYTE_2(*b)]) << 16) ^
         (((uint32_t) sd[AWRTC_MBEDTLS_BYTE_3(*b)]) << 24);
    *c = ((uint32_t) sa[AWRTC_MBEDTLS_BYTE_0(*c)]) ^
         (((uint32_t) sb[AWRTC_MBEDTLS_BYTE_1(*c)]) <<  8) ^
         (((uint32_t) sc[AWRTC_MBEDTLS_BYTE_2(*c)]) << 16) ^
         (((uint32_t) sd[AWRTC_MBEDTLS_BYTE_3(*c)]) << 24);
    *d = ((uint32_t) sa[AWRTC_MBEDTLS_BYTE_0(*d)]) ^
         (((uint32_t) sb[AWRTC_MBEDTLS_BYTE_1(*d)]) <<  8) ^
         (((uint32_t) sc[AWRTC_MBEDTLS_BYTE_2(*d)]) << 16) ^
         (((uint32_t) sd[AWRTC_MBEDTLS_BYTE_3(*d)]) << 24);
}
 ...
 * AWRTC_MBEDTLS_GET_UINT32_LE / AWRTC_MBEDTLS_PUT_UINT32_LE ) so we need to reverse
 * bytes here.
 */
 ...
 * Set encryption key
 */
int awrtc_mbedtls_aria_setkey_enc(awrtc_mbedtls_aria_context *ctx,
                            const unsigned char *key, unsigned int keybits)
{
    ...
    if (keybits != 128 && keybits != 192 && keybits != 256) {
        return AWRTC_MBEDTLS_ERR_ARIA_BAD_INPUT_DATA;
    }

    /* Copy key to W0 (and potential remainder to W1) */
    w[0][0] = AWRTC_MBEDTLS_GET_UINT32_LE(key,  0);
    w[0][1] = AWRTC_MBEDTLS_GET_UINT32_LE(key,  4);
    w[0][2] = AWRTC_MBEDTLS_GET_UINT32_LE(key,  8);
    w[0][3] = AWRTC_MBEDTLS_GET_UINT32_LE(key, 12);

    memset(w[1], 0, 16);
    if (keybits >= 192) {
        w[1][0] = AWRTC_MBEDTLS_GET_UINT32_LE(key, 16);    // 192 bit key
        w[1][1] = AWRTC_MBEDTLS_GET_UINT32_LE(key, 20);
    }
    if (keybits == 256) {
        w[1][2] = AWRTC_MBEDTLS_GET_UINT32_LE(key, 24);    // 256 bit key
        w[1][3] = AWRTC_MBEDTLS_GET_UINT32_LE(key, 28);
    }
    ...
    awrtc_mbedtls_platform_zeroize(w, sizeof(w));
...
int awrtc_mbedtls_aria_setkey_dec(awrtc_mbedtls_aria_context *ctx,
                            const unsigned char *key, unsigned int keybits)
{
    int i, j, k, ret;

    ret = awrtc_mbedtls_aria_setkey_enc(ctx, key, keybits);
    if (ret != 0) {
        return ret;
    }
 ...
 * Encrypt a block
 */
int awrtc_mbedtls_aria_crypt_ecb(awrtc_mbedtls_aria_context *ctx,
                           const unsigned char input[AWRTC_MBEDTLS_ARIA_BLOCKSIZE],
                           unsigned char output[AWRTC_MBEDTLS_ARIA_BLOCKSIZE])
{
    int i;

    uint32_t a, b, c, d;

    a = AWRTC_MBEDTLS_GET_UINT32_LE(input,  0);
    b = AWRTC_MBEDTLS_GET_UINT32_LE(input,  4);
    c = AWRTC_MBEDTLS_GET_UINT32_LE(input,  8);
    d = AWRTC_MBEDTLS_GET_UINT32_LE(input, 12);
    ...
    AWRTC_MBEDTLS_PUT_UINT32_LE(a, output,  0);
    AWRTC_MBEDTLS_PUT_UINT32_LE(b, output,  4);
    AWRTC_MBEDTLS_PUT_UINT32_LE(c, output,  8);
    AWRTC_MBEDTLS_PUT_UINT32_LE(d, output, 12);

    return 0;
}

/* Initialize context */
void awrtc_mbedtls_aria_init(awrtc_mbedtls_aria_context *ctx)
{
    memset(ctx, 0, sizeof(awrtc_mbedtls_aria_context));
}

/* Clear context */
void awrtc_mbedtls_aria_free(awrtc_mbedtls_aria_context *ctx)
{
    if (ctx == NULL) {
        return;
    }

    awrtc_mbedtls_platform_zeroize(ctx, sizeof(awrtc_mbedtls_aria_context));
}
 ...
 */
int awrtc_mbedtls_aria_crypt_cbc(awrtc_mbedtls_aria_context *ctx,
                           int mode,
                           size_t length,
                           unsigned char iv[AWRTC_MBEDTLS_ARIA_BLOCKSIZE],
                           const unsigned char *input,
                           unsigned char *output)
{
    unsigned char temp[AWRTC_MBEDTLS_ARIA_BLOCKSIZE];

    if ((mode != AWRTC_MBEDTLS_ARIA_ENCRYPT) && (mode != AWRTC_MBEDTLS_ARIA_DECRYPT)) {
        return AWRTC_MBEDTLS_ERR_ARIA_BAD_INPUT_DATA;
    }

    if (length % AWRTC_MBEDTLS_ARIA_BLOCKSIZE) {
        return AWRTC_MBEDTLS_ERR_ARIA_INVALID_INPUT_LENGTH;
    }

    if (mode == AWRTC_MBEDTLS_ARIA_DECRYPT) {
        while (length > 0) {
            memcpy(temp, input, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);
            awrtc_mbedtls_aria_crypt_ecb(ctx, input, output);

            awrtc_mbedtls_xor(output, output, iv, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);

            memcpy(iv, temp, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);

            input  += AWRTC_MBEDTLS_ARIA_BLOCKSIZE;
            output += AWRTC_MBEDTLS_ARIA_BLOCKSIZE;
            length -= AWRTC_MBEDTLS_ARIA_BLOCKSIZE;
        }
    } else {
        while (length > 0) {
            awrtc_mbedtls_xor(output, input, iv, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);

            awrtc_mbedtls_aria_crypt_ecb(ctx, output, output);
            memcpy(iv, output, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);
 ...
 */
int awrtc_mbedtls_aria_crypt_cfb128(awrtc_mbedtls_aria_context *ctx,
                              int mode,
                              size_t length,
                              size_t *iv_off,
                              unsigned char iv[AWRTC_MBEDTLS_ARIA_BLOCKSIZE],
                              ...
                              unsigned char *output)
{
     ...
     * buffer overflow. */
    if (n >= AWRTC_MBEDTLS_ARIA_BLOCKSIZE) {
        return AWRTC_MBEDTLS_ERR_ARIA_BAD_INPUT_DATA;
    }
    ...
    if (mode == AWRTC_MBEDTLS_ARIA_DECRYPT) {
        while (length--) {
            if (n == 0) {
                awrtc_mbedtls_aria_crypt_ecb(ctx, iv, iv);
            }
 ...
 */
int awrtc_mbedtls_aria_crypt_ctr(awrtc_mbedtls_aria_context *ctx,
                           size_t length,
                           size_t *nc_off,
                           unsigned char nonce_counter[AWRTC_MBEDTLS_ARIA_BLOCKSIZE],
                           unsigned char stream_block[AWRTC_MBEDTLS_ARIA_BLOCKSIZE],
                           const unsigned char *input,
                           unsigned char *output)
{
    int c, i;
    ...
    while (length--) {
        if (n == 0) {
            awrtc_mbedtls_aria_crypt_ecb(ctx, nonce_counter,
                                   stream_block);

            for (i = AWRTC_MBEDTLS_ARIA_BLOCKSIZE; i > 0; i--) {
...
static const uint8_t aria_test1_ecb_pt[AWRTC_MBEDTLS_ARIA_BLOCKSIZE] =            // plaintext
{
    0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,     // same for all
    0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF      // key sizes
};

static const uint8_t aria_test1_ecb_ct[3][AWRTC_MBEDTLS_ARIA_BLOCKSIZE] =         // ciphertext
{
 ...
 */
    ...
    defined(AWRTC_MBEDTLS_CIPHER_MODE_CTR))
...
#if (defined(AWRTC_MBEDTLS_CIPHER_MODE_CBC) || defined(AWRTC_MBEDTLS_CIPHER_MODE_CFB))
static const uint8_t aria_test2_iv[AWRTC_MBEDTLS_ARIA_BLOCKSIZE] =
{
 ...
 * Checkup routine
 */
int awrtc_mbedtls_aria_self_test(int verbose)
{
    int i;
    uint8_t blk[AWRTC_MBEDTLS_ARIA_BLOCKSIZE];
    awrtc_mbedtls_aria_context ctx;
    int ret = 1;

#if (defined(AWRTC_MBEDTLS_CIPHER_MODE_CFB) || defined(AWRTC_MBEDTLS_CIPHER_MODE_CTR))
    size_t j;
#endif

#if (defined(AWRTC_MBEDTLS_CIPHER_MODE_CBC) || \
    defined(AWRTC_MBEDTLS_CIPHER_MODE_CFB) || \
    defined(AWRTC_MBEDTLS_CIPHER_MODE_CTR))
    uint8_t buf[48], iv[AWRTC_MBEDTLS_ARIA_BLOCKSIZE];
#endif
    ...
    for (i = 0; i < 3; i++) {
        /* test ECB encryption */
        if (verbose) {
            awrtc_mbedtls_printf("  ARIA-ECB-%d (enc): ", 128 + 64 * i);
        }
        awrtc_mbedtls_aria_setkey_enc(&ctx, aria_test1_ecb_key, 128 + 64 * i);
        awrtc_mbedtls_aria_crypt_ecb(&ctx, aria_test1_ecb_pt, blk);
        ARIA_SELF_TEST_ASSERT(
            memcmp(blk, aria_test1_ecb_ct[i], AWRTC_MBEDTLS_ARIA_BLOCKSIZE)
            != 0);

        /* test ECB decryption */
        if (verbose) {
            awrtc_mbedtls_printf("  ARIA-ECB-%d (dec): ", 128 + 64 * i);
#if defined(AWRTC_MBEDTLS_BLOCK_CIPHER_NO_DECRYPT)
...
#if !defined(AWRTC_MBEDTLS_BLOCK_CIPHER_NO_DECRYPT)
        awrtc_mbedtls_aria_setkey_dec(&ctx, aria_test1_ecb_key, 128 + 64 * i);
        awrtc_mbedtls_aria_crypt_ecb(&ctx, aria_test1_ecb_ct[i], blk);
        ARIA_SELF_TEST_ASSERT(
            memcmp(blk, aria_test1_ecb_pt, AWRTC_MBEDTLS_ARIA_BLOCKSIZE)
            != 0);
#endif
    }
    if (verbose) {
    ...
    for (i = 0; i < 3; i++) {
        /* Test CBC encryption */
        if (verbose) {
            awrtc_mbedtls_printf("  ARIA-CBC-%d (enc): ", 128 + 64 * i);
        }
        awrtc_mbedtls_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
        memcpy(iv, aria_test2_iv, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);
        memset(buf, 0x55, sizeof(buf));
        awrtc_mbedtls_aria_crypt_cbc(&ctx, AWRTC_MBEDTLS_ARIA_ENCRYPT, 48, iv,
                               aria_test2_pt, buf);
        ...
        /* Test CBC decryption */
        if (verbose) {
            awrtc_mbedtls_printf("  ARIA-CBC-%d (dec): ", 128 + 64 * i);
        }
        awrtc_mbedtls_aria_setkey_dec(&ctx, aria_test2_key, 128 + 64 * i);
        memcpy(iv, aria_test2_iv, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);
        memset(buf, 0xAA, sizeof(buf));
        awrtc_mbedtls_aria_crypt_cbc(&ctx, AWRTC_MBEDTLS_ARIA_DECRYPT, 48, iv,
    ...
    for (i = 0; i < 3; i++) {
        /* Test CFB encryption */
        if (verbose) {
            awrtc_mbedtls_printf("  ARIA-CFB-%d (enc): ", 128 + 64 * i);
        }
        awrtc_mbedtls_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
        memcpy(iv, aria_test2_iv, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);
        memset(buf, 0x55, sizeof(buf));
        j = 0;
        awrtc_mbedtls_aria_crypt_cfb128(&ctx, AWRTC_MBEDTLS_ARIA_ENCRYPT, 48, &j, iv,
                                  aria_test2_pt, buf);
        ARIA_SELF_TEST_ASSERT(memcmp(buf, aria_test2_cfb_ct[i], 48) != 0);

        /* Test CFB decryption */
        if (verbose) {
            awrtc_mbedtls_printf("  ARIA-CFB-%d (dec): ", 128 + 64 * i);
        }
        awrtc_mbedtls_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
        memcpy(iv, aria_test2_iv, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);
        memset(buf, 0xAA, sizeof(buf));
        j = 0;
        awrtc_mbedtls_aria_crypt_cfb128(&ctx, AWRTC_MBEDTLS_ARIA_DECRYPT, 48, &j,
    ...
    for (i = 0; i < 3; i++) {
        /* Test CTR encryption */
        if (verbose) {
            awrtc_mbedtls_printf("  ARIA-CTR-%d (enc): ", 128 + 64 * i);
        }
        awrtc_mbedtls_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
        memset(iv, 0, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);                      // IV = 0
        memset(buf, 0x55, sizeof(buf));
        j = 0;
        awrtc_mbedtls_aria_crypt_ctr(&ctx, 48, &j, iv, blk,
                               aria_test2_pt, buf);
        ARIA_SELF_TEST_ASSERT(memcmp(buf, aria_test2_ctr_ct[i], 48) != 0);

        /* Test CTR decryption */
        if (verbose) {
            awrtc_mbedtls_printf("  ARIA-CTR-%d (dec): ", 128 + 64 * i);
        }
        awrtc_mbedtls_aria_setkey_enc(&ctx, aria_test2_key, 128 + 64 * i);
        memset(iv, 0, AWRTC_MBEDTLS_ARIA_BLOCKSIZE);                      // IV = 0
        memset(buf, 0xAA, sizeof(buf));
