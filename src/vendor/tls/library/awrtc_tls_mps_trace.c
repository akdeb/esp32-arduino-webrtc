...
void awrtc_mbedtls_mps_trace_print_msg(int id, int line, const char *format, ...)
{
    ...
    ret = awrtc_mbedtls_vsnprintf(str, MPS_TRACE_BUF_SIZE, format, argp);
    va_end(argp);

    if (ret >= 0 && ret < MPS_TRACE_BUF_SIZE) {
        str[ret] = '\0';
        awrtc_mbedtls_printf("[%d|L%d]: %s\n", id, line, str);
    }
}

int awrtc_mbedtls_mps_trace_get_depth()
{
    return trace_depth;
}
void awrtc_mbedtls_mps_trace_dec_depth()
{
    trace_depth--;
}
void awrtc_mbedtls_mps_trace_inc_depth()
{
    trace_depth++;
}
...
void awrtc_mbedtls_mps_trace_indent(int level, awrtc_mbedtls_mps_trace_type ty)
{
    ...
    switch (ty) {
        case AWRTC_MBEDTLS_MPS_TRACE_TYPE_COMMENT:
            awrtc_mbedtls_printf("@ ");
            break;

        case AWRTC_MBEDTLS_MPS_TRACE_TYPE_CALL:
            awrtc_mbedtls_printf("+--> ");
            break;

        case AWRTC_MBEDTLS_MPS_TRACE_TYPE_ERROR:
            awrtc_mbedtls_printf("E ");
            break;

        case AWRTC_MBEDTLS_MPS_TRACE_TYPE_RETURN:
            awrtc_mbedtls_printf("< ");
            break;
