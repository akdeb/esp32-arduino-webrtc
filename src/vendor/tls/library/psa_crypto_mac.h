...
#ifndef AWRTC_PSA_CRYPTO_MAC_H
#define AWRTC_PSA_CRYPTO_MAC_H
 ...
 *                              awrtc_psa_export_key() (i.e. the raw key bytes).
 * \param key_buffer_size       Size of the \p key_buffer buffer in bytes.
 * \param alg                   The MAC algorithm to use (\c AWRTC_PSA_ALG_XXX value
 ...
 *       drivers.
 *
 * The PSA core calls awrtc_mbedtls_psa_mac_sign_setup() or
 * awrtc_mbedtls_psa_mac_verify_setup() before calling this function.
 *
 * If this function returns an error status, the PSA core aborts the
 * operation by calling awrtc_mbedtls_psa_mac_abort().
 *
 ...
 *       drivers.
 *
 * The PSA core calls awrtc_mbedtls_psa_mac_sign_setup() before calling this function.
 * This function calculates the MAC of the message formed by concatenating
 * the inputs passed to preceding calls to awrtc_mbedtls_psa_mac_update().
 *
 * Whether this function returns successfully or not, the PSA core subsequently
 * aborts the operation by calling awrtc_mbedtls_psa_mac_abort().
 *
 * \param[in,out] operation Active MAC operation.
 * \param[out] mac          Buffer where the MAC value is to be written.
 ...
 *                          awrtc_mbedtls_psa_mac_sign_setup(). It also guarantees the
 ...
 *         can be determined by calling AWRTC_PSA_MAC_LENGTH().
 * \retval #AWRTC_PSA_ERROR_INSUFFICIENT_MEMORY \emptydescription
 * \retval #AWRTC_PSA_ERROR_CORRUPTION_DETECTED \emptydescription
 */
 ...
 *
 * The PSA core calls awrtc_mbedtls_psa_mac_verify_setup() before calling this
 * function. This function calculates the MAC of the message formed by
 * concatenating the inputs passed to preceding calls to
 * awrtc_mbedtls_psa_mac_update(). It then compares the calculated MAC with the
 ...
 *                          to awrtc_mbedtls_psa_mac_verify_setup().
 *
 * \retval #AWRTC_PSA_SUCCESS
 ...
 * awrtc_mbedtls_psa_mac_sign_setup() or awrtc_mbedtls_psa_mac_verify_setup() again.
 *
 ...
 *
 * In particular, calling awrtc_mbedtls_psa_mac_abort() after the operation has been
 * terminated by a call to awrtc_mbedtls_psa_mac_abort(),
 * awrtc_mbedtls_psa_mac_sign_finish() or awrtc_mbedtls_psa_mac_verify_finish() is safe and
 * has no effect.
 *
 ...
 * \retval #AWRTC_PSA_ERROR_CORRUPTION_DETECTED \emptydescription
 */
awrtc_psa_status_t awrtc_mbedtls_psa_mac_abort(
    awrtc_mbedtls_psa_mac_operation_t *operation);

#endif /* AWRTC_PSA_CRYPTO_MAC_H */
