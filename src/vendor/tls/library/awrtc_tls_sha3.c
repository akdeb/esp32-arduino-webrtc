 ...
 * x86-64.
 */
#if !defined(AWRTC_MBEDTLS_SHA3_THETA_UNROLL)
    #define AWRTC_MBEDTLS_SHA3_THETA_UNROLL 0 //no-check-names
#endif
#if !defined(AWRTC_MBEDTLS_SHA3_CHI_UNROLL)
    #if defined(__OPTIMIZE_SIZE__)
        #define AWRTC_MBEDTLS_SHA3_CHI_UNROLL 0 //no-check-names
    #else
        #define AWRTC_MBEDTLS_SHA3_CHI_UNROLL 1 //no-check-names
    #endif
#endif
#if !defined(AWRTC_MBEDTLS_SHA3_PI_UNROLL)
    #define AWRTC_MBEDTLS_SHA3_PI_UNROLL 1 //no-check-names
#endif
#if !defined(AWRTC_MBEDTLS_SHA3_RHO_UNROLL)
    #define AWRTC_MBEDTLS_SHA3_RHO_UNROLL 1 //no-check-names
#endif
...
static void keccak_f1600(awrtc_mbedtls_sha3_context *ctx)
{
    uint64_t lane[5];
    ...
    for (int round = 0; round < 24; round++) {
        ...
        /* Theta */
#if AWRTC_MBEDTLS_SHA3_THETA_UNROLL == 0 //no-check-names
        for (i = 0; i < 5; i++) {
...
#if AWRTC_MBEDTLS_SHA3_RHO_UNROLL == 0
            for (int j = i; j < i + 4; j++) {
...
#else
            s[i + 0] = ROTR64(s[i + 0], AWRTC_MBEDTLS_BYTE_3(r));
            s[i + 1] = ROTR64(s[i + 1], AWRTC_MBEDTLS_BYTE_2(r));
            s[i + 2] = ROTR64(s[i + 2], AWRTC_MBEDTLS_BYTE_1(r));
            s[i + 3] = ROTR64(s[i + 3], AWRTC_MBEDTLS_BYTE_0(r));
#endif
        }

        /* Pi */
        t = s[1];
#if AWRTC_MBEDTLS_SHA3_PI_UNROLL == 0
        for (i = 0; i < 24; i += 4) {
            uint32_t p = pi[i >> 2];
...
#else
        uint32_t p = pi[0];
        SWAP(s[AWRTC_MBEDTLS_BYTE_0(p)], t); SWAP(s[AWRTC_MBEDTLS_BYTE_1(p)], t);
        SWAP(s[AWRTC_MBEDTLS_BYTE_2(p)], t); SWAP(s[AWRTC_MBEDTLS_BYTE_3(p)], t);
        p = pi[1];
        SWAP(s[AWRTC_MBEDTLS_BYTE_0(p)], t); SWAP(s[AWRTC_MBEDTLS_BYTE_1(p)], t);
        SWAP(s[AWRTC_MBEDTLS_BYTE_2(p)], t); SWAP(s[AWRTC_MBEDTLS_BYTE_3(p)], t);
        p = pi[2];
        ...
        /* Chi */
#if AWRTC_MBEDTLS_SHA3_CHI_UNROLL == 0 //no-check-names
        for (i = 0; i <= 20; i += 5) {
...
{
    memset(ctx, 0, sizeof(awrtc_mbedtls_sha3_context));
}
...
void awrtc_mbedtls_sha3_free(awrtc_mbedtls_sha3_context *ctx)
{
    ...
    awrtc_mbedtls_platform_zeroize(ctx, sizeof(awrtc_mbedtls_sha3_context));
}
...
{
    switch (id) {
        case AWRTC_MBEDTLS_SHA3_224:
            ctx->olen = 224 / 8;
            ctx->max_block_size = 1152 / 8;
            break;
        case AWRTC_MBEDTLS_SHA3_256:
            ctx->olen = 256 / 8;
            ctx->max_block_size = 1088 / 8;
            break;
        case AWRTC_MBEDTLS_SHA3_384:
            ctx->olen = 384 / 8;
            ctx->max_block_size = 832 / 8;
            break;
        case AWRTC_MBEDTLS_SHA3_512:
            ctx->olen = 512 / 8;
            ctx->max_block_size = 576 / 8;
            break;
        default:
            return AWRTC_MBEDTLS_ERR_SHA3_BAD_INPUT_DATA;
    }
                        ...
                        size_t ilen)
{
    if (ilen >= 8) {
        ...
        while (ilen >= 8) {
            ABSORB(ctx, ctx->index, AWRTC_MBEDTLS_GET_UINT64_LE(input, 0));
            input += 8;
            ilen -= 8;
                        ...
                        uint8_t *output, size_t olen)
{
    ...
    if (ctx->olen > 0) {
        if (ctx->olen > olen) {
            ret = AWRTC_MBEDTLS_ERR_SHA3_BAD_INPUT_DATA;
            goto exit;
        }
        olen = ctx->olen;
    }
...
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    awrtc_mbedtls_sha3_context ctx;

    awrtc_mbedtls_sha3_init(&ctx);

    /* Sanity checks are performed in every awrtc_mbedtls_sha3_xxx() */
    if ((ret = awrtc_mbedtls_sha3_starts(&ctx, id)) != 0) {
        goto exit;
    }

    if ((ret = awrtc_mbedtls_sha3_update(&ctx, input, ilen)) != 0) {
        goto exit;
    }

    if ((ret = awrtc_mbedtls_sha3_finish(&ctx, output, olen)) != 0) {
        goto exit;
    }
...
static int awrtc_mbedtls_sha3_kat_test(int verbose,
                                 const char *type_name,
                                 awrtc_mbedtls_sha3_id id,
                                 int test_num)
{
    uint8_t hash[64];
    int result;

    result = awrtc_mbedtls_sha3(id,
                          test_data[test_num], test_data_len[test_num],
                          hash, sizeof(hash));
    if (result != 0) {
        if (verbose != 0) {
            awrtc_mbedtls_printf("  %s test %d error code: %d\n",
    ...
    if (0 != result) {
        if (verbose != 0) {
            awrtc_mbedtls_printf("  %s test %d failed\n", type_name, test_num);
        }
    ...
    if (verbose != 0) {
        awrtc_mbedtls_printf("  %s test %d passed\n", type_name, test_num);
    }

    return 0;
}

static int awrtc_mbedtls_sha3_long_kat_test(int verbose,
                                      const char *type_name,
                                      awrtc_mbedtls_sha3_id id)
{
    awrtc_mbedtls_sha3_context ctx;
    ...
    if (verbose != 0) {
        awrtc_mbedtls_printf("  %s long KAT test ", type_name);
    }

    awrtc_mbedtls_sha3_init(&ctx);

    result = awrtc_mbedtls_sha3_starts(&ctx, id);
    if (result != 0) {
        if (verbose != 0) {
            awrtc_mbedtls_printf("setup failed\n ");
        }
    }

    /* Process 1,000,000 (one million) 'a' characters */
    for (int i = 0; i < 1000; i++) {
        result = awrtc_mbedtls_sha3_update(&ctx, buffer, 1000);
        if (result != 0) {
            if (verbose != 0) {
                awrtc_mbedtls_printf("update error code: %i\n", result);
            }
    ...
    result = awrtc_mbedtls_sha3_finish(&ctx, hash, sizeof(hash));
    if (result != 0) {
        if (verbose != 0) {
            awrtc_mbedtls_printf("finish error code: %d\n", result);
        }
...
int awrtc_mbedtls_sha3_self_test(int verbose)
{
    ...
    for (i = 0; i < 2; i++) {
        if (0 != awrtc_mbedtls_sha3_kat_test(verbose,
                                       "SHA3-224", AWRTC_MBEDTLS_SHA3_224, i)) {
            return 1;
        }

        if (0 != awrtc_mbedtls_sha3_kat_test(verbose,
                                       "SHA3-256", AWRTC_MBEDTLS_SHA3_256, i)) {
            return 1;
        }

        if (0 != awrtc_mbedtls_sha3_kat_test(verbose,
                                       "SHA3-384", AWRTC_MBEDTLS_SHA3_384, i)) {
            return 1;
        }

        if (0 != awrtc_mbedtls_sha3_kat_test(verbose,
                                       "SHA3-512", AWRTC_MBEDTLS_SHA3_512, i)) {
            return 1;
        }
    }

    /* SHA-3 long KAT tests */
    if (0 != awrtc_mbedtls_sha3_long_kat_test(verbose,
                                        "SHA3-224", AWRTC_MBEDTLS_SHA3_224)) {
        return 1;
    }

    if (0 != awrtc_mbedtls_sha3_long_kat_test(verbose,
                                        "SHA3-256", AWRTC_MBEDTLS_SHA3_256)) {
        return 1;
    }

    if (0 != awrtc_mbedtls_sha3_long_kat_test(verbose,
                                        "SHA3-384", AWRTC_MBEDTLS_SHA3_384)) {
        return 1;
    }

    if (0 != awrtc_mbedtls_sha3_long_kat_test(verbose,
                                        "SHA3-512", AWRTC_MBEDTLS_SHA3_512)) {
        return 1;
    }
