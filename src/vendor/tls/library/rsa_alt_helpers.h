 ...
 *      - awrtc_mbedtls_rsa_deduce_primes
 *      - awrtc_mbedtls_rsa_deduce_private_exponent
 *      - awrtc_mbedtls_rsa_deduce_crt
 *       Each of these functions takes a set of core RSA parameters and
 *       generates some other, or CRT related parameters.
 *
 *  (2) Parameter-checking helpers. These are:
 *      - awrtc_mbedtls_rsa_validate_params
 *      - awrtc_mbedtls_rsa_validate_crt
 ...
 */
#ifndef AWRTC_MBEDTLS_RSA_ALT_HELPERS_H
#define AWRTC_MBEDTLS_RSA_ALT_HELPERS_H
 ...
 *                 use the helper function \c awrtc_mbedtls_rsa_validate_params.
 *
 */
int awrtc_mbedtls_rsa_deduce_primes(awrtc_mbedtls_mpi const *N, awrtc_mbedtls_mpi const *E,
                              awrtc_mbedtls_mpi const *D,
 ...
 * \return         #AWRTC_MBEDTLS_ERR_MPI_NOT_ACCEPTABLE if E is not coprime to P-1
 *                 and Q-1, that is, if GCD( E, (P-1)*(Q-1) ) != 1.
 * \return         #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if inputs are otherwise
 *                 invalid.
 *
 ...
 *                   (%) This is only done if AWRTC_MBEDTLS_GENPRIME is defined.
 *                 - A non-zero error code otherwise.
 *
 ...
 *                 - \c AWRTC_MBEDTLS_ERR_RSA_KEY_CHECK_FAILED if check failed,
 *                   potentially including \c AWRTC_MBEDTLS_ERR_MPI_XXX if some
 *                   MPI calculations failed.
 *                 - \c AWRTC_MBEDTLS_ERR_RSA_BAD_INPUT_DATA if insufficient
