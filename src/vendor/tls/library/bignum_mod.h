 ...
 *   of type #awrtc_mbedtls_mpi_mod_modulus. The structure must be set up with an
 ...
 *   of type \c const #awrtc_mbedtls_mpi_mod_modulus* must not modify its value.
 * - **Bignum parameters**: Bignums are passed as pointers to an array of
 *   limbs or to a #awrtc_mbedtls_mpi_mod_residue structure. A limb has the type
 *   #awrtc_mbedtls_mpi_uint. Residues must be initialized before use, and must be
 ...
 *       type \c const #awrtc_mbedtls_mpi_mod_residue* must not modify its value.
 ...
 *   #awrtc_mbedtls_mpi_mod_modulus and #awrtc_mbedtls_mpi_mod_residue have a \c limbs
...
#ifndef AWRTC_MBEDTLS_BIGNUM_MOD_H
#define AWRTC_MBEDTLS_BIGNUM_MOD_H
 ...
 * what their contents are (see #awrtc_mbedtls_mpi_mod_modulus).
 */
typedef enum {
    /** Representation not chosen (makes the modulus structure invalid). */
    AWRTC_MBEDTLS_MPI_MOD_REP_INVALID    = 0,
    /* Skip 1 as it is slightly easier to accidentally pass to functions. */
    /** Montgomery representation. */
    AWRTC_MBEDTLS_MPI_MOD_REP_MONTGOMERY = 2,
    /* Optimised reduction available. This indicates a coordinate modulus (P)
     * and one or more of the following have been configured:
     * - A nist curve (AWRTC_MBEDTLS_ECP_DP_SECPXXXR1_ENABLED) & AWRTC_MBEDTLS_ECP_NIST_OPTIM.
     * - A Kobliz Curve.
     * - A Fast Reduction Curve CURVE25519 or CURVE448. */
    AWRTC_MBEDTLS_MPI_MOD_REP_OPT_RED,
} awrtc_mbedtls_mpi_mod_rep_selector;

/* Make awrtc_mbedtls_mpi_mod_rep_selector and awrtc_mbedtls_mpi_mod_ext_rep disjoint to
 * make it easier to catch when they are accidentally swapped. */
typedef enum {
    AWRTC_MBEDTLS_MPI_MOD_EXT_REP_INVALID = 0,
    AWRTC_MBEDTLS_MPI_MOD_EXT_REP_LE      = 8,
    AWRTC_MBEDTLS_MPI_MOD_EXT_REP_BE
} awrtc_mbedtls_mpi_mod_ext_rep;

typedef struct {
    awrtc_mbedtls_mpi_uint *p;
    size_t limbs;
} awrtc_mbedtls_mpi_mod_residue;

typedef struct {
    awrtc_mbedtls_mpi_uint const *rr;  /* The residue for 2^{2*n*biL} mod N */
    awrtc_mbedtls_mpi_uint mm;         /* Montgomery const for -N^{-1} mod 2^{ciL} */
} awrtc_mbedtls_mpi_mont_struct;

typedef int (*awrtc_mbedtls_mpi_modp_fn)(awrtc_mbedtls_mpi_uint *X, size_t X_limbs);

typedef struct {
    awrtc_mbedtls_mpi_modp_fn modp;    /* The optimised reduction function pointer */
} awrtc_mbedtls_mpi_opt_red_struct;

typedef struct {
    const awrtc_mbedtls_mpi_uint *p;
    size_t limbs;                            // number of limbs
    size_t bits;                             // bitlen of p
    awrtc_mbedtls_mpi_mod_rep_selector int_rep;    // selector to signal the active member of the union
    union rep {
        /* if int_rep == #AWRTC_MBEDTLS_MPI_MOD_REP_MONTGOMERY */
        awrtc_mbedtls_mpi_mont_struct mont;
        /* if int_rep == #AWRTC_MBEDTLS_MPI_MOD_REP_OPT_RED */
        awrtc_mbedtls_mpi_opt_red_struct ored;
    } rep;
} awrtc_mbedtls_mpi_mod_modulus;
 ...
 *                  awrtc_mbedtls_mpi_mod_residue_release() is called. The data
 ...
 * \return      #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if \p p_limbs is less than the
 ...
 * awrtc_mbedtls_mpi_mod_residue_setup() to make it safe to free or use again.
 *
 * This function invalidates \p r and it must not be used until after
 * awrtc_mbedtls_mpi_mod_residue_setup() is called on it again.
 *
 ...
 *                  not be modified in any way until after
 *                  awrtc_mbedtls_mpi_mod_modulus_free() is called.
 ...
 *
 * This function frees any memory allocated by awrtc_mbedtls_mpi_mod_modulus_setup().
 *
 * \warning This function does not free the limb array passed to
 *          awrtc_mbedtls_mpi_mod_modulus_setup() only removes the reference to it,
 ...
 * AWRTC_MBEDTLS_MPI_MOD_REP_MONTGOMERY.
 *
 ...
 *
 * \return      \c 0 if successful.
 * \return      #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if all the parameters do not
 ...
 *       awrtc_mbedtls_mpi_mod_residue_setup().
 *
 ...
 *
 * \return          \c 0 if successful.
 * \return          #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if the given MPIs do not
 ...
 *
 * \return         \c 0 if successful.
 * \return         #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if \p A and \p N do not
 *                 have the same number of limbs.
 * \return         #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if \p A is zero.
 * \return         #AWRTC_MBEDTLS_ERR_MPI_ALLOC_FAILED if couldn't allocate enough
 ...
 *
 * \return       \c 0 if successful.
 * \return       #AWRTC_MBEDTLS_ERR_MPI_BUFFER_TOO_SMALL if \p r isn't
 *               large enough to hold the value in \p buf.
 * \return       #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if \p ext_rep
 ...
 * awrtc_mbedtls_mpi_mod_residue_setup() and awrtc_mbedtls_mpi_mod_read()).
 *
 ...
 *               zeroes).
 * \return       #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if \p ext_rep is invalid.
 * \return       #AWRTC_MBEDTLS_ERR_MPI_ALLOC_FAILED if couldn't allocate enough
...
#endif /* AWRTC_MBEDTLS_BIGNUM_MOD_H */
