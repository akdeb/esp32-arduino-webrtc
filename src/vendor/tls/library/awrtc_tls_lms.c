...
#define SIG_TYPE_OFFSET(otstype) (SIG_OTS_SIG_OFFSET   + \
                                  AWRTC_MBEDTLS_LMOTS_SIG_LEN(otstype))
#define SIG_PATH_OFFSET(otstype) (SIG_TYPE_OFFSET(otstype) + \
                                  AWRTC_MBEDTLS_LMS_TYPE_LEN)
...
#define MERKLE_TREE_NODE_AM(type)          ((size_t) 1 << (AWRTC_MBEDTLS_LMS_H_TREE_HEIGHT(type) + 1u))
#define MERKLE_TREE_LEAF_NODE_AM(type)     ((size_t) 1 << AWRTC_MBEDTLS_LMS_H_TREE_HEIGHT(type))
#define MERKLE_TREE_INTERNAL_NODE_AM(type) ((unsigned int) \
                                            (1u << AWRTC_MBEDTLS_LMS_H_TREE_HEIGHT(type)))
 ...
 */
static int create_merkle_leaf_value(const awrtc_mbedtls_lms_parameters_t *params,
                                    unsigned char *pub_key,
                                    unsigned int r_node_idx,
                                    unsigned char *out)
{
    awrtc_psa_hash_operation_t op;
    ...
    memset(out, 0, AWRTC_MBEDTLS_LMS_M_NODE_BYTES(params->type));

    op = awrtc_psa_hash_operation_init();
    ...
    AWRTC_MBEDTLS_PUT_UINT32_BE(r_node_idx, r_node_idx_bytes, 0);
    status = awrtc_psa_hash_update(&op, r_node_idx_bytes, 4);
    if (status != AWRTC_PSA_SUCCESS) {
        goto exit;
    }

    status = awrtc_psa_hash_update(&op, D_LEAF_CONSTANT_BYTES, D_CONST_LEN);
    if (status != AWRTC_PSA_SUCCESS) {
        goto exit;
    }

    status = awrtc_psa_hash_update(&op, pub_key,
                             AWRTC_MBEDTLS_LMOTS_N_HASH_LEN(params->otstype));
    if (status != AWRTC_PSA_SUCCESS) {
        goto exit;
    }

    status = awrtc_psa_hash_finish(&op, out, AWRTC_MBEDTLS_LMS_M_NODE_BYTES(params->type),
                             &output_hash_len);
    if (status != AWRTC_PSA_SUCCESS) {
        goto exit;
    }
 ...
 *  out                 The output node value, which is a hash output.
 */
static int create_merkle_internal_value(const awrtc_mbedtls_lms_parameters_t *params,
                                        ...
                                        unsigned char *out)
{
    ...
    status = awrtc_psa_hash_update(&op, D_INTR_CONSTANT_BYTES, D_CONST_LEN);
    if (status != AWRTC_PSA_SUCCESS) {
        goto exit;
    }

    status = awrtc_psa_hash_update(&op, left_node,
                             AWRTC_MBEDTLS_LMS_M_NODE_BYTES(params->type));
    if (status != AWRTC_PSA_SUCCESS) {
        goto exit;
    }

    status = awrtc_psa_hash_update(&op, right_node,
                             AWRTC_MBEDTLS_LMS_M_NODE_BYTES(params->type));
    if (status != AWRTC_PSA_SUCCESS) {
        goto exit;
    }
...
void awrtc_mbedtls_lms_public_init(awrtc_mbedtls_lms_public_t *ctx)
{
    memset(ctx, 0, sizeof(*ctx));
}

void awrtc_mbedtls_lms_public_free(awrtc_mbedtls_lms_public_t *ctx)
{
    if (ctx == NULL) {
        return;
    }
...
int awrtc_mbedtls_lms_import_public_key(awrtc_mbedtls_lms_public_t *ctx,
                                  const unsigned char *key, size_t key_size)
{
    ...
    uint32_t type = AWRTC_MBEDTLS_GET_UINT32_BE(key, PUBLIC_KEY_TYPE_OFFSET);
    if (type != (uint32_t) AWRTC_MBEDTLS_LMS_SHA256_M32_H10) {
        return AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }
    ctx->params.type = (awrtc_mbedtls_lms_algorithm_type_t) type;

    if (key_size != AWRTC_MBEDTLS_LMS_PUBLIC_KEY_LEN(ctx->params.type)) {
        return AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }

    uint32_t otstype = AWRTC_MBEDTLS_GET_UINT32_BE(key, PUBLIC_KEY_OTSTYPE_OFFSET);
    if (otstype != (uint32_t) AWRTC_MBEDTLS_LMOTS_SHA256_N32_W8) {
        return AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }
    ctx->params.otstype = (awrtc_mbedtls_lmots_algorithm_type_t) otstype;

    memcpy(ctx->params.I_key_identifier,
           key + PUBLIC_KEY_I_KEY_ID_OFFSET,
           AWRTC_MBEDTLS_LMOTS_I_KEY_ID_LEN);
    memcpy(ctx->T_1_pub_key, key + PUBLIC_KEY_ROOT_NODE_OFFSET,
           AWRTC_MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type));
...
int awrtc_mbedtls_lms_export_public_key(const awrtc_mbedtls_lms_public_t *ctx,
                                  unsigned char *key,
                                  size_t key_size, size_t *key_len)
{
    ...
    AWRTC_MBEDTLS_PUT_UINT32_BE(ctx->params.type, key, PUBLIC_KEY_TYPE_OFFSET);
    AWRTC_MBEDTLS_PUT_UINT32_BE(ctx->params.otstype, key, PUBLIC_KEY_OTSTYPE_OFFSET);
    ...
    if (key_len != NULL) {
        *key_len = AWRTC_MBEDTLS_LMS_PUBLIC_KEY_LEN(ctx->params.type);
    }

    return 0;
}

int awrtc_mbedtls_lms_verify(const awrtc_mbedtls_lms_public_t *ctx,
                       const unsigned char *msg, size_t msg_size,
                       const unsigned char *sig, size_t sig_size)
{
    unsigned int q_leaf_identifier;
    unsigned char Kc_candidate_ots_pub_key[AWRTC_MBEDTLS_LMOTS_N_HASH_LEN_MAX];
    unsigned char Tc_candidate_root_node[AWRTC_MBEDTLS_LMS_M_NODE_BYTES_MAX];
    unsigned int height;
    unsigned int curr_node_id;
    unsigned int parent_node_id;
    const unsigned char *left_node;
    const unsigned char *right_node;
    awrtc_mbedtls_lmots_parameters_t ots_params;
    ...
    if (ctx->params.type
        != AWRTC_MBEDTLS_LMS_SHA256_M32_H10) {
        return AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }
    ...
    if (sig_size != AWRTC_MBEDTLS_LMS_SIG_LEN(ctx->params.type, ctx->params.otstype)) {
        return AWRTC_MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }

    if (sig_size < SIG_OTS_SIG_OFFSET + AWRTC_MBEDTLS_LMOTS_TYPE_LEN) {
        return AWRTC_MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }

    if (AWRTC_MBEDTLS_GET_UINT32_BE(sig, SIG_OTS_SIG_OFFSET + AWRTC_MBEDTLS_LMOTS_SIG_TYPE_OFFSET)
        != AWRTC_MBEDTLS_LMOTS_SHA256_N32_W8) {
        return AWRTC_MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }

    if (sig_size < SIG_TYPE_OFFSET(ctx->params.otstype) + AWRTC_MBEDTLS_LMS_TYPE_LEN) {
        return AWRTC_MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }

    if (AWRTC_MBEDTLS_GET_UINT32_BE(sig, SIG_TYPE_OFFSET(ctx->params.otstype))
        != AWRTC_MBEDTLS_LMS_SHA256_M32_H10) {
        return AWRTC_MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }


    q_leaf_identifier = AWRTC_MBEDTLS_GET_UINT32_BE(sig, SIG_Q_LEAF_ID_OFFSET);
    ...
    memcpy(ots_params.I_key_identifier,
           ctx->params.I_key_identifier,
           AWRTC_MBEDTLS_LMOTS_I_KEY_ID_LEN);
    AWRTC_MBEDTLS_PUT_UINT32_BE(q_leaf_identifier, ots_params.q_leaf_identifier, 0);
    ots_params.type = ctx->params.otstype;

    ret = awrtc_mbedtls_lmots_calculate_public_key_candidate(&ots_params,
                                                       msg,
                                                       msg_size,
                                                       sig + SIG_OTS_SIG_OFFSET,
                                                       AWRTC_MBEDTLS_LMOTS_SIG_LEN(ctx->params.otstype),
    ...
    for (height = 0; height < AWRTC_MBEDTLS_LMS_H_TREE_HEIGHT(ctx->params.type);
         height++) {
        ...
        if (curr_node_id & 1) {
            left_node = sig + SIG_PATH_OFFSET(ctx->params.otstype) +
                        height * AWRTC_MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type);
 ...
 */
static int calculate_merkle_tree(const awrtc_mbedtls_lms_private_t *ctx,
                                 unsigned char *tree)
{
    ...
    for (priv_key_idx = 0;
        ...
        ret = create_merkle_leaf_value(&ctx->params,
                                       ...
                                       r_node_idx,
                                       &tree[r_node_idx * AWRTC_MBEDTLS_LMS_M_NODE_BYTES(
                                                 ctx->params.type)]);
        if (ret != 0) {
            return ret;
        }
 ...
 */
static int get_merkle_path(awrtc_mbedtls_lms_private_t *ctx,
                           unsigned int leaf_node_id,
                           unsigned char *path)
{
    const size_t node_bytes = AWRTC_MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type);
    ...
    tree = awrtc_mbedtls_calloc((size_t) MERKLE_TREE_NODE_AM(ctx->params.type),
                          node_bytes);
    if (tree == NULL) {
        return AWRTC_MBEDTLS_ERR_LMS_ALLOC_FAILED;
    }
...
exit:
    awrtc_mbedtls_zeroize_and_free(tree, node_bytes *
                             (size_t) MERKLE_TREE_NODE_AM(ctx->params.type));

    return ret;
}

void awrtc_mbedtls_lms_private_init(awrtc_mbedtls_lms_private_t *ctx)
{
    memset(ctx, 0, sizeof(*ctx));
}

void awrtc_mbedtls_lms_private_free(awrtc_mbedtls_lms_private_t *ctx)
{
    if (ctx == NULL) {
        return;
    }
    ...
    if (ctx->have_private_key) {
        if (ctx->ots_private_keys != NULL) {
            for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
                awrtc_mbedtls_lmots_private_free(&ctx->ots_private_keys[idx]);
            }
        }

        if (ctx->ots_public_keys != NULL) {
            for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
                awrtc_mbedtls_lmots_public_free(&ctx->ots_public_keys[idx]);
            }
        }

        awrtc_mbedtls_free(ctx->ots_private_keys);
        awrtc_mbedtls_free(ctx->ots_public_keys);
    }
...
int awrtc_mbedtls_lms_generate_private_key(awrtc_mbedtls_lms_private_t *ctx,
                                     awrtc_mbedtls_lms_algorithm_type_t type,
                                     awrtc_mbedtls_lmots_algorithm_type_t otstype,
                                     ...
                                     size_t seed_size)
{
    ...
    if (type != AWRTC_MBEDTLS_LMS_SHA256_M32_H10) {
        return AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }

    if (otstype != AWRTC_MBEDTLS_LMOTS_SHA256_N32_W8) {
        return AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }
    ...
    ctx->ots_private_keys = awrtc_mbedtls_calloc((size_t) MERKLE_TREE_LEAF_NODE_AM(ctx->params.type),
                                           sizeof(*ctx->ots_private_keys));
    if (ctx->ots_private_keys == NULL) {
        ret = AWRTC_MBEDTLS_ERR_LMS_ALLOC_FAILED;
        goto exit;
    }

    /* Requires a cast to size_t to avoid an implicit cast warning on certain
     * platforms (particularly Windows) */
    ctx->ots_public_keys = awrtc_mbedtls_calloc((size_t) MERKLE_TREE_LEAF_NODE_AM(ctx->params.type),
    ...
    for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
        awrtc_mbedtls_lmots_private_init(&ctx->ots_private_keys[idx]);
        awrtc_mbedtls_lmots_public_init(&ctx->ots_public_keys[idx]);
    }


    for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
        ret = awrtc_mbedtls_lmots_generate_private_key(&ctx->ots_private_keys[idx],
                                                 otstype,
                                                 ctx->params.I_key_identifier,
                                                 idx, seed, seed_size);
        if (ret != 0) {
        ...
        ret = awrtc_mbedtls_lmots_calculate_public_key(&ctx->ots_public_keys[idx],
...
exit:
    if (ret != 0) {
        awrtc_mbedtls_lms_private_free(ctx);
    }

    return ret;
}

int awrtc_mbedtls_lms_calculate_public_key(awrtc_mbedtls_lms_public_t *ctx,
                                     const awrtc_mbedtls_lms_private_t *priv_ctx)
{
    const size_t node_bytes = AWRTC_MBEDTLS_LMS_M_NODE_BYTES(priv_ctx->params.type);
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *tree = NULL;
    ...
    tree = awrtc_mbedtls_calloc((size_t) MERKLE_TREE_NODE_AM(priv_ctx->params.type),
                          node_bytes);
    if (tree == NULL) {
        return AWRTC_MBEDTLS_ERR_LMS_ALLOC_FAILED;
    }
    ...
    memcpy(&ctx->params, &priv_ctx->params,
           sizeof(awrtc_mbedtls_lmots_parameters_t));
...
int awrtc_mbedtls_lms_sign(awrtc_mbedtls_lms_private_t *ctx,
                     int (*f_rng)(void *, unsigned char *, size_t),
                     ...
                     size_t *sig_len)
{
    ...
    if (ctx->params.type != AWRTC_MBEDTLS_LMS_SHA256_M32_H10) {
        return AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }
    ...
    if (ctx->q_next_usable_key >= MERKLE_TREE_LEAF_NODE_AM(ctx->params.type)) {
        return AWRTC_MBEDTLS_ERR_LMS_OUT_OF_PRIVATE_KEYS;
    }
    ...
    if (AWRTC_MBEDTLS_LMS_SIG_LEN(ctx->params.type, ctx->params.otstype)
        < SIG_OTS_SIG_OFFSET) {
        return AWRTC_MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }

    ret = awrtc_mbedtls_lmots_sign(&ctx->ots_private_keys[q_leaf_identifier],
                             f_rng,
                             p_rng,
                             msg,
                             msg_size,
                             sig + SIG_OTS_SIG_OFFSET,
                             AWRTC_MBEDTLS_LMS_SIG_LEN(ctx->params.type,
    ...
    AWRTC_MBEDTLS_PUT_UINT32_BE(ctx->params.type, sig, SIG_TYPE_OFFSET(ctx->params.otstype));
    AWRTC_MBEDTLS_PUT_UINT32_BE(q_leaf_identifier, sig, SIG_Q_LEAF_ID_OFFSET);
    ...
    if (sig_len != NULL) {
        *sig_len = AWRTC_MBEDTLS_LMS_SIG_LEN(ctx->params.type, ctx->params.otstype);
    }
