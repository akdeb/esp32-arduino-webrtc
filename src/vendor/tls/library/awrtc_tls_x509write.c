 ...
 */
#include "common.h"
#if defined(AWRTC_MBEDTLS_X509_CSR_WRITE_C) || defined(AWRTC_MBEDTLS_X509_CRT_WRITE_C)
...
{
    int ret = 0;
    const awrtc_mbedtls_x509_san_list *cur;
    unsigned char *buf;
    unsigned char *p;
    size_t len;
    size_t buflen = 0;
    ...
    for (cur = san_list; cur != NULL; cur = cur->next) {
        ...
        switch (cur->node.type) {
            ...
            {
                const awrtc_mbedtls_asn1_named_data *chunk = &cur->node.san.directory_name;
                while (chunk != NULL) {
    ...
    /* Allocate buffer */
    buf = awrtc_mbedtls_calloc(1, buflen);
    if (buf == NULL) {
    ...
    while (cur != NULL) {
        ...
        switch (cur->node.type) {
            ...
            case AWRTC_MBEDTLS_X509_SAN_IP_ADDRESS:
            {
                ...
                AWRTC_MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             awrtc_mbedtls_asn1_write_raw_buffer(
                                                 &p, buf,
                                                 unstructured_name, unstructured_name_len));
                AWRTC_MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len, awrtc_mbedtls_asn1_write_len(
                                                 &p, buf, unstructured_name_len));
                AWRTC_MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             awrtc_mbedtls_asn1_write_tag(
                                                 &p, buf,
                                                 AWRTC_MBEDTLS_ASN1_CONTEXT_SPECIFIC | cur->node.type));
            }
            break;
            case AWRTC_MBEDTLS_X509_SAN_DIRECTORY_NAME:
                AWRTC_MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             awrtc_mbedtls_x509_write_names(&p, buf,
                                                                      (awrtc_mbedtls_asn1_named_data *) &
                                                                      cur->node
                                                                      .san.directory_name));
                AWRTC_MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             awrtc_mbedtls_asn1_write_len(&p, buf, single_san_len));
                AWRTC_MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             awrtc_mbedtls_asn1_write_tag(&p, buf,
                                                                    AWRTC_MBEDTLS_ASN1_CONTEXT_SPECIFIC |
                                                                    AWRTC_MBEDTLS_ASN1_CONSTRUCTED |
                                                                    AWRTC_MBEDTLS_X509_SAN_DIRECTORY_NAME));
                break;
            default:
                /* Error out on an unsupported SAN */
                ret = AWRTC_MBEDTLS_ERR_X509_FEATURE_UNAVAILABLE;
                goto cleanup;
        }
        cur = cur->next;
        /* check for overflow */
    ...
    AWRTC_MBEDTLS_ASN1_CHK_CLEANUP_ADD(len, awrtc_mbedtls_asn1_write_len(&p, buf, len));
    AWRTC_MBEDTLS_ASN1_CHK_CLEANUP_ADD(len,
                                 awrtc_mbedtls_asn1_write_tag(&p, buf,
                                                        AWRTC_MBEDTLS_ASN1_CONSTRUCTED |
                                                        AWRTC_MBEDTLS_ASN1_SEQUENCE));

    ret = awrtc_mbedtls_x509_set_extension(extensions,
                                     AWRTC_MBEDTLS_OID_SUBJECT_ALT_NAME,
                                     AWRTC_MBEDTLS_OID_SIZE(AWRTC_MBEDTLS_OID_SUBJECT_ALT_NAME),
                                     0,
                                     buf + buflen - len, len);
...
#endif /* AWRTC_MBEDTLS_X509_CSR_WRITE_C || AWRTC_MBEDTLS_X509_CRT_WRITE_C */
