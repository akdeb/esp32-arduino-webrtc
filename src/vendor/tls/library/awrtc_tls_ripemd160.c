 ...
 *  http://homes.esat.kuleuven.be/~bosselae/awrtc_mbedtls_ripemd160.html
...
{
    memset(ctx, 0, sizeof(awrtc_mbedtls_ripemd160_context));
}
...
void awrtc_mbedtls_ripemd160_free(awrtc_mbedtls_ripemd160_context *ctx)
{
    ...
    awrtc_mbedtls_platform_zeroize(ctx, sizeof(awrtc_mbedtls_ripemd160_context));
}
                                       ...
                                       const unsigned char data[64])
{
    ...
    local.X[0] = AWRTC_MBEDTLS_GET_UINT32_LE(data,  0);
    local.X[1] = AWRTC_MBEDTLS_GET_UINT32_LE(data,  4);
    local.X[2] = AWRTC_MBEDTLS_GET_UINT32_LE(data,  8);
    local.X[3] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 12);
    local.X[4] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 16);
    local.X[5] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 20);
    local.X[6] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 24);
    local.X[7] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 28);
    local.X[8] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 32);
    local.X[9] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 36);
    local.X[10] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 40);
    local.X[11] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 44);
    local.X[12] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 48);
    local.X[13] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 52);
    local.X[14] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 56);
    local.X[15] = AWRTC_MBEDTLS_GET_UINT32_LE(data, 60);
                             ...
                             size_t ilen)
{
    ...
    if (left && ilen >= fill) {
        ...
        if ((ret = awrtc_mbedtls_internal_ripemd160_process(ctx, ctx->buffer)) != 0) {
            return ret;
        }
    ...
    while (ilen >= 64) {
        if ((ret = awrtc_mbedtls_internal_ripemd160_process(ctx, input)) != 0) {
            return ret;
        }
                             ...
                             unsigned char output[20])
{
    ...
    AWRTC_MBEDTLS_PUT_UINT32_LE(low,  msglen, 0);
    AWRTC_MBEDTLS_PUT_UINT32_LE(high, msglen, 4);

    last = ctx->total[0] & 0x3F;
    padn = (last < 56) ? (56 - last) : (120 - last);

    ret = awrtc_mbedtls_ripemd160_update(ctx, ripemd160_padding, padn);
    if (ret != 0) {
        goto exit;
    }

    ret = awrtc_mbedtls_ripemd160_update(ctx, msglen, 8);
    if (ret != 0) {
        goto exit;
    }

    AWRTC_MBEDTLS_PUT_UINT32_LE(ctx->state[0], output,  0);
    AWRTC_MBEDTLS_PUT_UINT32_LE(ctx->state[1], output,  4);
    AWRTC_MBEDTLS_PUT_UINT32_LE(ctx->state[2], output,  8);
    AWRTC_MBEDTLS_PUT_UINT32_LE(ctx->state[3], output, 12);
    AWRTC_MBEDTLS_PUT_UINT32_LE(ctx->state[4], output, 16);

    ret = 0;

exit:
    awrtc_mbedtls_ripemd160_free(ctx);
    return ret;
}
...
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    awrtc_mbedtls_ripemd160_context ctx;

    awrtc_mbedtls_ripemd160_init(&ctx);

    if ((ret = awrtc_mbedtls_ripemd160_starts(&ctx)) != 0) {
        goto exit;
    }

    if ((ret = awrtc_mbedtls_ripemd160_update(&ctx, input, ilen)) != 0) {
        goto exit;
    }

    if ((ret = awrtc_mbedtls_ripemd160_finish(&ctx, output)) != 0) {
        goto exit;
    }
 ...
 * http://homes.esat.kuleuven.be/~bosselae/awrtc_mbedtls_ripemd160.html#HMAC
 */
#define TESTS   8
...
int awrtc_mbedtls_ripemd160_self_test(int verbose)
{
    ...
    for (i = 0; i < TESTS; i++) {
        if (verbose != 0) {
            awrtc_mbedtls_printf("  RIPEMD-160 test #%d: ", i + 1);
        }

        ret = awrtc_mbedtls_ripemd160(ripemd160_test_str[i],
