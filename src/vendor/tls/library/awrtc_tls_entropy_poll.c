...
#error \
    "Platform entropy sources only work on Unix and Windows, see AWRTC_MBEDTLS_NO_PLATFORM_ENTROPY in awrtc_mbedtls_config.h"
#endif
...
int awrtc_mbedtls_platform_entropy_poll(void *data, unsigned char *output, size_t len,
                                  size_t *olen)
{
    ((void) data);
    *olen = 0;
...
const char *awrtc_mbedtls_platform_dev_random = AWRTC_MBEDTLS_PLATFORM_DEV_RANDOM;
                                  ...
                                  unsigned char *output, size_t len, size_t *olen)
{
    ...
    *olen = 0;
    ...
    file = fopen(awrtc_mbedtls_platform_dev_random, "rb");
    if (file == NULL) {
        return AWRTC_MBEDTLS_ERR_ENTROPY_SOURCE_FAILED;
    }
...
int awrtc_mbedtls_nv_seed_poll(void *data,
                         unsigned char *output, size_t len, size_t *olen)
{
    unsigned char buf[AWRTC_MBEDTLS_ENTROPY_BLOCK_SIZE];
    size_t use_len = AWRTC_MBEDTLS_ENTROPY_BLOCK_SIZE;
    ((void) data);

    memset(buf, 0, AWRTC_MBEDTLS_ENTROPY_BLOCK_SIZE);

    if (awrtc_mbedtls_nv_seed_read(buf, AWRTC_MBEDTLS_ENTROPY_BLOCK_SIZE) < 0) {
        return AWRTC_MBEDTLS_ERR_ENTROPY_SOURCE_FAILED;
    }
