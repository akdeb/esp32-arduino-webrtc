...
#define AWRTC_MBEDTLS_ENABLE_ARM_SHA3_EXTENSIONS_COMPILER_FLAG
#endif

#include "common.h"

#if defined(AWRTC_MBEDTLS_SHA512_C) || defined(AWRTC_MBEDTLS_SHA384_C)
 ...
 */
#    if !defined(__ARM_FEATURE_SHA512) || defined(AWRTC_MBEDTLS_ENABLE_ARM_SHA3_EXTENSIONS_COMPILER_FLAG)
       /* Test Clang first, as it defines __GNUC__ */
#      if defined(__ARMCOMPILER_VERSION)
#        if __ARMCOMPILER_VERSION < 6090000
#          error "A more recent armclang is required for AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_*"
#        elif __ARMCOMPILER_VERSION == 6090000
#          error "Must use minimum -march=armv8.2-a+sha3 for AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_*"
#        else
...
#          error "A more recent Clang is required for AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_*"
#        else
#          pragma clang attribute push (__attribute__((target("sha3"))), apply_to=function)
#          define AWRTC_MBEDTLS_POP_TARGET_PRAGMA
#        endif
...
#        if __GNUC__ < 8
#          error "A more recent GCC is required for AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_*"
#        else
...
#        endif
#      else
#        error "Only GCC and Clang supported for AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_*"
#      endif
#    endif
/* *INDENT-ON* */
#  endif
...
#  undef AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_ONLY
#  undef AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_IF_PRESENT
#endif

#if defined(AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_IF_PRESENT)
/*
 * Capability detection code comes early, so we can disable
 * AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_IF_PRESENT if no detection mechanism found
 */
#if defined(HWCAP_SHA512)
static int awrtc_mbedtls_a64_crypto_sha512_determine_support(void)
{
...
{
    AWRTC_MBEDTLS_PUT_UINT64_BE(n, b, i);
}
#else
#define sha512_put_uint64_be    AWRTC_MBEDTLS_PUT_UINT64_BE
#endif /* AWRTC_MBEDTLS_SHA512_SMALLER */

void awrtc_mbedtls_sha512_init(awrtc_mbedtls_sha512_context *ctx)
{
    memset(ctx, 0, sizeof(awrtc_mbedtls_sha512_context));
}
...
void awrtc_mbedtls_sha512_free(awrtc_mbedtls_sha512_context *ctx)
{
    ...
    awrtc_mbedtls_platform_zeroize(ctx, sizeof(awrtc_mbedtls_sha512_context));
}
...
{
#if defined(AWRTC_MBEDTLS_SHA384_C) && defined(AWRTC_MBEDTLS_SHA512_C)
    if (is384 != 0 && is384 != 1) {
        return AWRTC_MBEDTLS_ERR_SHA512_BAD_INPUT_DATA;
    }
#elif defined(AWRTC_MBEDTLS_SHA512_C)
    if (is384 != 0) {
        return AWRTC_MBEDTLS_ERR_SHA512_BAD_INPUT_DATA;
    }
#else /* defined AWRTC_MBEDTLS_SHA384_C only */
    if (is384 == 0) {
        return AWRTC_MBEDTLS_ERR_SHA512_BAD_INPUT_DATA;
    }
#endif
...
#  define awrtc_mbedtls_internal_sha512_process_many_a64_crypto awrtc_mbedtls_internal_sha512_process_many
#  define awrtc_mbedtls_internal_sha512_process_a64_crypto      awrtc_mbedtls_internal_sha512_process
#endif
...
static size_t awrtc_mbedtls_internal_sha512_process_many_a64_crypto(
    awrtc_mbedtls_sha512_context *ctx, const uint8_t *msg, size_t len)
{
 ...
 * versions, otherwise it is renamed to be the public awrtc_mbedtls_internal_sha512_process()
 */
static
#endif
int awrtc_mbedtls_internal_sha512_process_a64_crypto(awrtc_mbedtls_sha512_context *ctx,
                                               const unsigned char data[SHA512_BLOCK_SIZE])
{
    return (awrtc_mbedtls_internal_sha512_process_many_a64_crypto(ctx, data,
                                                            SHA512_BLOCK_SIZE) ==
...
#if !defined(AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_IF_PRESENT)
#define awrtc_mbedtls_internal_sha512_process_many_c awrtc_mbedtls_internal_sha512_process_many
#define awrtc_mbedtls_internal_sha512_process_c      awrtc_mbedtls_internal_sha512_process
#endif


#if !defined(AWRTC_MBEDTLS_SHA512_PROCESS_ALT) && !defined(AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_ONLY)
...
static
#endif
int awrtc_mbedtls_internal_sha512_process_c(awrtc_mbedtls_sha512_context *ctx,
                                      const unsigned char data[SHA512_BLOCK_SIZE])
{
    int i;
    struct {
...
#define P(a, b, c, d, e, f, g, h, x, K)                                      \
    ...
    for (i = 0; i < 80; i++) {
        if (i < 16) {
            local.W[i] = AWRTC_MBEDTLS_GET_UINT64_BE(data, i << 3);
        } else {
...
#else /* AWRTC_MBEDTLS_SHA512_SMALLER */
    for (i = 0; i < 16; i++) {
        local.W[i] = AWRTC_MBEDTLS_GET_UINT64_BE(data, i << 3);
    }
...
#endif /* !AWRTC_MBEDTLS_SHA512_PROCESS_ALT && !AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_ONLY */


#if !defined(AWRTC_MBEDTLS_SHA512_USE_A64_CRYPTO_ONLY)

static size_t awrtc_mbedtls_internal_sha512_process_many_c(
    awrtc_mbedtls_sha512_context *ctx, const uint8_t *data, size_t len)
{
    size_t processed = 0;

    while (len >= SHA512_BLOCK_SIZE) {
        if (awrtc_mbedtls_internal_sha512_process_c(ctx, data) != 0) {
            return 0;
        }
...
static int awrtc_mbedtls_a64_crypto_sha512_has_support(void)
{
    static int done = 0;
    static int supported = 0;

    if (!done) {
        supported = awrtc_mbedtls_a64_crypto_sha512_determine_support();
        done = 1;
    }
...
static size_t awrtc_mbedtls_internal_sha512_process_many(awrtc_mbedtls_sha512_context *ctx,
                                                   const uint8_t *msg, size_t len)
{
    if (awrtc_mbedtls_a64_crypto_sha512_has_support()) {
        return awrtc_mbedtls_internal_sha512_process_many_a64_crypto(ctx, msg, len);
    } else {
        return awrtc_mbedtls_internal_sha512_process_many_c(ctx, msg, len);
    }
}
...
{
    if (awrtc_mbedtls_a64_crypto_sha512_has_support()) {
        return awrtc_mbedtls_internal_sha512_process_a64_crypto(ctx, data);
    } else {
        return awrtc_mbedtls_internal_sha512_process_c(ctx, data);
    }
}
                          ...
                          size_t ilen)
{
    ...
    if (left && ilen >= fill) {
        ...
        if ((ret = awrtc_mbedtls_internal_sha512_process(ctx, ctx->buffer)) != 0) {
            return ret;
        }
    ...
    while (ilen >= SHA512_BLOCK_SIZE) {
        size_t processed =
            awrtc_mbedtls_internal_sha512_process_many(ctx, input, ilen);
...
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    awrtc_mbedtls_sha512_context ctx;
    ...
    awrtc_mbedtls_sha512_init(&ctx);

    if ((ret = awrtc_mbedtls_sha512_starts(&ctx, is384)) != 0) {
        goto exit;
    }

    if ((ret = awrtc_mbedtls_sha512_update(&ctx, input, ilen)) != 0) {
        goto exit;
    }

    if ((ret = awrtc_mbedtls_sha512_finish(&ctx, output)) != 0) {
        goto exit;
    }
...
static int awrtc_mbedtls_sha512_common_self_test(int verbose, int is384)
{
    int i, buflen, ret = 0;
    unsigned char *buf;
    ...
    for (i = 0; i < 3; i++) {
        if (verbose != 0) {
            awrtc_mbedtls_printf("  SHA-%d test #%d: ", 512 - is384 * 128, i + 1);
        }
        ...
        if (i == 2) {
            ...
            for (int j = 0; j < 1000; j++) {
                ret = awrtc_mbedtls_sha512_update(&ctx, buf, buflen);
                if (ret != 0) {
                    goto fail;
                }
            }
        } else {
            ret = awrtc_mbedtls_sha512_update(&ctx, sha_test_buf[i],
                                        sha_test_buflen[i]);
            if (ret != 0) {
                goto fail;
            }
        ...
        if ((ret = awrtc_mbedtls_sha512_finish(&ctx, sha512sum)) != 0) {
            goto fail;
        }
...
{
    return awrtc_mbedtls_sha512_common_self_test(verbose, 0);
}
#endif /* AWRTC_MBEDTLS_SHA512_C */
...
{
    return awrtc_mbedtls_sha512_common_self_test(verbose, 1);
}
#endif /* AWRTC_MBEDTLS_SHA384_C */
...
#endif /* AWRTC_MBEDTLS_SHA512_C || AWRTC_MBEDTLS_SHA384_C */
