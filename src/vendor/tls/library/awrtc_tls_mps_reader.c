...
static int awrtc_mbedtls_mps_trace_id = AWRTC_MBEDTLS_MPS_TRACE_BIT_READER;
#endif /* AWRTC_MBEDTLS_MPS_ENABLE_TRACE */
...
static inline int mps_reader_is_accumulating(
    awrtc_mbedtls_mps_reader const *rd)
{
    awrtc_mbedtls_mps_size_t acc_remaining;
    if (rd->acc == NULL) {
        return 0;
    }
...
static inline awrtc_mbedtls_mps_size_t mps_reader_get_fragment_offset(
    awrtc_mbedtls_mps_reader const *rd)
{
    unsigned char *acc = rd->acc;
    awrtc_mbedtls_mps_size_t frag_offset;
...
static inline awrtc_mbedtls_mps_size_t mps_reader_serving_from_accumulator(
    awrtc_mbedtls_mps_reader const *rd)
{
    awrtc_mbedtls_mps_size_t frag_offset, end;
...
static inline void mps_reader_zero(awrtc_mbedtls_mps_reader *rd)
{
    ...
    static awrtc_mbedtls_mps_reader const zero =
    { .frag          = NULL,
      .frag_len      = 0,
      .commit        = 0,
      .end           = 0,
...
int awrtc_mbedtls_mps_reader_init(awrtc_mbedtls_mps_reader *rd,
                            unsigned char *acc,
                            awrtc_mbedtls_mps_size_t acc_len)
{
    AWRTC_MBEDTLS_MPS_TRACE_INIT("awrtc_mbedtls_mps_reader_init");
    AWRTC_MBEDTLS_MPS_TRACE(AWRTC_MBEDTLS_MPS_TRACE_TYPE_COMMENT,
                      "* Accumulator size: %u bytes", (unsigned) acc_len);
    mps_reader_zero(rd);
    rd->acc = acc;
    rd->acc_len = acc_len;
    AWRTC_MBEDTLS_MPS_TRACE_RETURN(0);
}

int awrtc_mbedtls_mps_reader_free(awrtc_mbedtls_mps_reader *rd)
{
    AWRTC_MBEDTLS_MPS_TRACE_INIT("awrtc_mbedtls_mps_reader_free");
    mps_reader_zero(rd);
    AWRTC_MBEDTLS_MPS_TRACE_RETURN(0);
}

int awrtc_mbedtls_mps_reader_feed(awrtc_mbedtls_mps_reader *rd,
                            unsigned char *new_frag,
                            awrtc_mbedtls_mps_size_t new_frag_len)
{
    awrtc_mbedtls_mps_size_t copy_to_acc;
    AWRTC_MBEDTLS_MPS_TRACE_INIT("awrtc_mbedtls_mps_reader_feed");
    AWRTC_MBEDTLS_MPS_TRACE(AWRTC_MBEDTLS_MPS_TRACE_TYPE_COMMENT,
                      "* Fragment length: %u bytes", (unsigned) new_frag_len);

    if (new_frag == NULL) {
        AWRTC_MBEDTLS_MPS_TRACE_RETURN(AWRTC_MBEDTLS_ERR_MPS_READER_INVALID_ARG);
    }

    AWRTC_MBEDTLS_MPS_STATE_VALIDATE_RAW(mps_reader_is_producing(
                                       rd),
                                   "awrtc_mbedtls_mps_reader_feed() requires reader to be in producing mode");

    if (mps_reader_is_accumulating(rd)) {
        unsigned char *acc    = rd->acc;
        awrtc_mbedtls_mps_size_t acc_remaining = rd->acc_share.acc_remaining;
        awrtc_mbedtls_mps_size_t acc_available = rd->acc_available;
        ...
        if (acc_remaining > 0) {
            ...
            AWRTC_MBEDTLS_MPS_TRACE_RETURN(AWRTC_MBEDTLS_ERR_MPS_READER_NEED_MORE);
        }
...
int awrtc_mbedtls_mps_reader_get(awrtc_mbedtls_mps_reader *rd,
                           awrtc_mbedtls_mps_size_t desired,
                           unsigned char **buffer,
                           awrtc_mbedtls_mps_size_t *buflen)
{
    unsigned char *frag;
    awrtc_mbedtls_mps_size_t frag_len, frag_offset, end, frag_fetched, frag_remaining;
    AWRTC_MBEDTLS_MPS_TRACE_INIT("awrtc_mbedtls_mps_reader_get");
    AWRTC_MBEDTLS_MPS_TRACE(AWRTC_MBEDTLS_MPS_TRACE_TYPE_COMMENT,
                      "* Bytes requested: %u", (unsigned) desired);

    AWRTC_MBEDTLS_MPS_STATE_VALIDATE_RAW(mps_reader_is_consuming(
                                       rd),
                                   "awrtc_mbedtls_mps_reader_get() requires reader to be in consuming mode");
    ...
    if (mps_reader_serving_from_accumulator(rd)) {
         ...
         * `frag` starting from the next call to awrtc_mbedtls_mps_reader_get().
         */
        ...
        if (frag_offset - end < desired) {
            awrtc_mbedtls_mps_size_t acc_available;
            ...
            if (acc_available - end != desired) {
                 ...
                 * validate this. */
                AWRTC_MBEDTLS_MPS_TRACE_RETURN(
                    AWRTC_MBEDTLS_ERR_MPS_READER_INCONSISTENT_REQUESTS);
            }
        }
    ...
    if (frag_remaining < desired) {
        ...
        if (buflen == NULL) {
            ...
            AWRTC_MBEDTLS_MPS_TRACE_RETURN(AWRTC_MBEDTLS_ERR_MPS_READER_OUT_OF_DATA);
        }
...
int awrtc_mbedtls_mps_reader_commit(awrtc_mbedtls_mps_reader *rd)
{
    awrtc_mbedtls_mps_size_t end;
    AWRTC_MBEDTLS_MPS_TRACE_INIT("awrtc_mbedtls_mps_reader_commit");
    AWRTC_MBEDTLS_MPS_STATE_VALIDATE_RAW(mps_reader_is_consuming(
                                       rd),
                                   "awrtc_mbedtls_mps_reader_commit() requires reader to be in consuming mode");
...
int awrtc_mbedtls_mps_reader_reclaim(awrtc_mbedtls_mps_reader *rd,
                               int *paused)
{
    unsigned char *frag, *acc;
    awrtc_mbedtls_mps_size_t pending, commit;
    awrtc_mbedtls_mps_size_t acc_len, frag_offset, frag_len;
    AWRTC_MBEDTLS_MPS_TRACE_INIT("awrtc_mbedtls_mps_reader_reclaim");
    ...
    AWRTC_MBEDTLS_MPS_STATE_VALIDATE_RAW(mps_reader_is_consuming(
                                       rd),
                                   "awrtc_mbedtls_mps_reader_reclaim() requires reader to be in consuming mode");
    ...
    if (pending == 0) {
        ...
        if (commit < frag_offset || commit - frag_offset < frag_len) {
            ...
            rd->end = commit;
            AWRTC_MBEDTLS_MPS_TRACE_RETURN(AWRTC_MBEDTLS_ERR_MPS_READER_DATA_LEFT);
        }
        ...
        awrtc_mbedtls_mps_size_t acc_backup_offset;
        awrtc_mbedtls_mps_size_t acc_backup_len;
        awrtc_mbedtls_mps_size_t frag_backup_offset;
        awrtc_mbedtls_mps_size_t frag_backup_len;

        awrtc_mbedtls_mps_size_t backup_len;
        awrtc_mbedtls_mps_size_t acc_len_needed;
        ...
        if (acc == NULL) {
            ...
            AWRTC_MBEDTLS_MPS_TRACE_RETURN(
                AWRTC_MBEDTLS_ERR_MPS_READER_NEED_ACCUMULATOR);
        }
        acc_len = rd->acc_len;
        ...
        if (overflow || acc_len < acc_len_needed) {
             ...
             * there hadn't been a call to awrtc_mbedtls_mps_reader_get()
             * since the last commit. */
            rd->end = commit;
            rd->pending = 0;
            AWRTC_MBEDTLS_MPS_TRACE(AWRTC_MBEDTLS_MPS_TRACE_TYPE_ERROR,
            ...
            AWRTC_MBEDTLS_MPS_TRACE_RETURN(
                AWRTC_MBEDTLS_ERR_MPS_READER_ACCUMULATOR_TOO_SMALL);
        }
