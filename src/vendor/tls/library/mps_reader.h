 ...
 *     received through calls to awrtc_mbedtls_mps_reader_get(),
 ...
 *     awrtc_mbedtls_mps_reader_reclaim().
 *     The marker is set via awrtc_mbedtls_mps_reader_commit()
 ...
 *            via awrtc_mbedtls_mps_reader_feed().
 *
 ...
 * done via successful calls to awrtc_mbedtls_mps_reader_feed(), while
 * transitioning from Attached to either Unset or Accumulating (depending
 * on what has been processed) is done via awrtc_mbedtls_mps_reader_reclaim().
 *
...
#ifndef AWRTC_MBEDTLS_READER_H
#define AWRTC_MBEDTLS_READER_H
...
struct awrtc_mbedtls_mps_reader;
typedef struct awrtc_mbedtls_mps_reader awrtc_mbedtls_mps_reader;
...
struct awrtc_mbedtls_mps_reader {
    unsigned char *frag;  /*!< The fragment of incoming data managed by
                           ...
                           *   through awrtc_mbedtls_mps_reader_feed(). The reader
    ...
    awrtc_mbedtls_mps_stored_size_t frag_len;
    /*!< The length of the current fragment.
     *   Must be 0 if \c frag == \c NULL.             */
    awrtc_mbedtls_mps_stored_size_t commit;
    ...
    awrtc_mbedtls_mps_stored_size_t end;
    /*!< The offset of the end of the last chunk
     *   passed to the user through a call to
     *   awrtc_mbedtls_mps_reader_get(), relative to the first
    ...
    unsigned char *acc;   /*!< The accumulator is used to gather incoming
                           *   data if a read-request via awrtc_mbedtls_mps_reader_get()
                           *   cannot be served from the current fragment.   */
    awrtc_mbedtls_mps_stored_size_t acc_len;
    /*!< The total size of the accumulator.           */
    awrtc_mbedtls_mps_stored_size_t acc_available;
    ...
    union {
        awrtc_mbedtls_mps_stored_size_t acc_remaining;
        ...
        awrtc_mbedtls_mps_stored_size_t frag_offset;
 ...
 *                  in case get requests through awrtc_mbedtls_mps_reader_get()
 *                  exceed the buffer provided by awrtc_mbedtls_mps_reader_feed().
 ...
 * \return          A negative \c AWRTC_MBEDTLS_ERR_READER_XXX error code on failure.
 */
int awrtc_mbedtls_mps_reader_init(awrtc_mbedtls_mps_reader *reader,
                            unsigned char *acc,
 ...
 * \return          A negative \c AWRTC_MBEDTLS_ERR_READER_XXX error code on failure.
 */
int awrtc_mbedtls_mps_reader_free(awrtc_mbedtls_mps_reader *reader);
 ...
 *                  of \p buf until awrtc_mbedtls_mps_reader_reclaim()
 *                  is called. It is the responsibility of the caller
 *                  to ensure that the \p buf persists and is not changed
 *                  between successful calls to awrtc_mbedtls_mps_reader_feed()
 *                  and awrtc_mbedtls_mps_reader_reclaim().
 * \return          \c AWRTC_MBEDTLS_ERR_MPS_READER_NEED_MORE if more input data is
 *                  required to fulfill a previous request to awrtc_mbedtls_mps_reader_get().
 *                  In this case, the reader remains in producing mode and
 *                  takes no ownership of the provided buffer (an internal copy
 *                  is made instead).
 * \return          Another negative \c AWRTC_MBEDTLS_ERR_READER_XXX error code on
 *                  different kinds of failures.
 */
int awrtc_mbedtls_mps_reader_feed(awrtc_mbedtls_mps_reader *reader,
                            unsigned char *buf,
 ...
 *                  awrtc_mbedtls_mps_reader_get() has exceeded the bounds of the
 *                  input buffer.
 *
 * \return          \c 0 on success.
 * \return          A negative \c AWRTC_MBEDTLS_ERR_READER_XXX error code on failure.
 */
int awrtc_mbedtls_mps_reader_reclaim(awrtc_mbedtls_mps_reader *reader,
                               int *paused);
 ...
 *                  until the next call to awrtc_mbedtls_mps_reader_reclaim().
 * \return          #AWRTC_MBEDTLS_ERR_MPS_READER_OUT_OF_DATA if there is not enough
 ...
 *                  awrtc_mbedtls_mps_reader_reclaim() and awrtc_mbedtls_mps_reader_feed().
 ...
 *                  is requested after a awrtc_mbedtls_mps_reader_reclaim() and
 *                  awrtc_mbedtls_mps_reader_feed() cycle.
 * \return          Another negative \c AWRTC_MBEDTLS_ERR_READER_XXX error
 ...
 *                  afterwards.
 */
int awrtc_mbedtls_mps_reader_get(awrtc_mbedtls_mps_reader *reader,
                           awrtc_mbedtls_mps_size_t desired,
                           unsigned char **buffer,
                           awrtc_mbedtls_mps_size_t *buflen);

/**
 * \brief         Mark data obtained from awrtc_mbedtls_mps_reader_get() as processed.
 *
 *                This call indicates that all data received from prior calls to
 *                awrtc_mbedtls_mps_reader_get() has been or will have been
 *                processed when awrtc_mbedtls_mps_reader_reclaim() is called,
 *                and thus need not be backed up.
 *
 *                This function has no user observable effect until
 *                awrtc_mbedtls_mps_reader_reclaim() is called. In particular,
 *                buffers received from awrtc_mbedtls_mps_reader_get() remain
 *                valid until awrtc_mbedtls_mps_reader_reclaim() is called.
 *
 ...
 *
 */
int awrtc_mbedtls_mps_reader_commit(awrtc_mbedtls_mps_reader *reader);

#endif /* AWRTC_MBEDTLS_READER_H */
