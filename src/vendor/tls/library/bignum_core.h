 ...
 *   limbs. A limb has the type #awrtc_mbedtls_mpi_uint. Unless otherwise specified:
...
#ifndef AWRTC_MBEDTLS_BIGNUM_CORE_H
#define AWRTC_MBEDTLS_BIGNUM_CORE_H
...
#define ciL    (sizeof(awrtc_mbedtls_mpi_uint))     /** chars in limb  */
 ...
 *  if (X_public == AWRTC_MBEDTLS_MPI_IS_PUBLIC) {
 *      // unsafe path
 *  } else {
 *      // safe path
 *  }
 ...
 *
 * The value of AWRTC_MBEDTLS_MPI_IS_PUBLIC is chosen in a way that is unlikely to happen by accident, but
 * which can be used as an immediate value in a Thumb2 comparison (for code size). */
#define AWRTC_MBEDTLS_MPI_IS_PUBLIC  0x2a2a2a2a
#define AWRTC_MBEDTLS_MPI_IS_SECRET  0
#if defined(AWRTC_MBEDTLS_TEST_HOOKS) && !defined(AWRTC_MBEDTLS_THREADING_C)
// Default value for testing that is neither AWRTC_MBEDTLS_MPI_IS_PUBLIC nor AWRTC_MBEDTLS_MPI_IS_SECRET
#define AWRTC_MBEDTLS_MPI_IS_TEST  1
#endif
...
/** Convert a big-endian byte array aligned to the size of awrtc_mbedtls_mpi_uint
 * into the storage form used by awrtc_mbedtls_mpi.
 *
 ...
 *
 * \return         AWRTC_MBEDTLS_CT_TRUE if \p min is less than or equal to \p A, otherwise AWRTC_MBEDTLS_CT_FALSE.
 */
 ...
 *                 This must not be 0.
 *
 * \return         AWRTC_MBEDTLS_CT_TRUE  if \p A is less than \p B.
 *                 AWRTC_MBEDTLS_CT_FALSE if \p A is greater than or equal to \p B.
 */
 ...
 *                      the constant time interface (e.g. `awrtc_mbedtls_ct_bool()`)
 ...
 * \return       #AWRTC_MBEDTLS_ERR_MPI_BUFFER_TOO_SMALL if \p X isn't
 ...
 *
 * \return       \c 0 if successful.
 * \return       #AWRTC_MBEDTLS_ERR_MPI_BUFFER_TOO_SMALL if \p output isn't
 ...
 *               large enough to hold the value of \p A.
 */
int awrtc_mbedtls_mpi_core_write_be(const awrtc_mbedtls_mpi_uint *A,
                              size_t A_limbs,
                              unsigned char *output,
                              size_t output_length);
 ...
 */
awrtc_mbedtls_mpi_uint awrtc_mbedtls_mpi_core_mla(awrtc_mbedtls_mpi_uint *X, size_t X_limbs,
 ...
 *                          This can be calculated by `awrtc_mbedtls_mpi_core_montmul_init()`.
...
void awrtc_mbedtls_mpi_core_montmul(awrtc_mbedtls_mpi_uint *X,
                              ...
                              awrtc_mbedtls_mpi_uint mm, awrtc_mbedtls_mpi_uint *T);
 ...
 * \return        #AWRTC_MBEDTLS_ERR_MPI_ALLOC_FAILED if there is not enough space
 *                to store the value of Montgomery constant squared.
 * \return        #AWRTC_MBEDTLS_ERR_MPI_DIVISION_BY_ZERO if \p N modulus is zero.
 * \return        #AWRTC_MBEDTLS_ERR_MPI_NEGATIVE_VALUE if \p N modulus is negative.
 */
 ...
 *
 * \return         \c 0 if successful.
 * \return         #AWRTC_MBEDTLS_ERR_MPI_BAD_INPUT_DATA if \p X does not have
 ...
 */
int awrtc_mbedtls_mpi_core_fill_random(awrtc_mbedtls_mpi_uint *X, size_t X_limbs,
                                 size_t bytes,
 ...
 *                 a call to `awrtc_mbedtls_mpi_core_exp_mod()`.
 *
 * \note           This will always be at least
 *                 `awrtc_mbedtls_mpi_core_montmul_working_limbs(AN_limbs)`,
 *                 i.e. sufficient for a call to `awrtc_mbedtls_mpi_core_montmul()`.
 *
 * \param AN_limbs The number of limbs in the input `A` and the modulus `N`
 *                 (they must be the same size) that will be given to
 *                 `awrtc_mbedtls_mpi_core_exp_mod()`.
 * \param E_limbs  The number of limbs in the exponent `E` that will be given
 *                 to `awrtc_mbedtls_mpi_core_exp_mod()`.
 *
 ...
 *                   by `awrtc_mbedtls_mpi_core_exp_mod_working_limbs()`.
 ...
 *
 * \return        AWRTC_MBEDTLS_CT_FALSE if `A == 0`
 *                AWRTC_MBEDTLS_CT_TRUE  if `A != 0`.
 */
...
/**
 * \brief          Returns the number of limbs of working memory required for
 *                 a call to `awrtc_mbedtls_mpi_core_montmul()`.
 *
 * \param AN_limbs The number of limbs in the input `A` and the modulus `N`
 *                 (they must be the same size) that will be given to
 *                 `awrtc_mbedtls_mpi_core_montmul()` or one of the other functions
 *                 that specifies this as the amount of working memory needed.
 *
 * \return         The number of limbs of working memory required by
 *                 `awrtc_mbedtls_mpi_core_montmul()` (or other similar function).
 */
static inline size_t awrtc_mbedtls_mpi_core_montmul_working_limbs(size_t AN_limbs)
{
    return 2 * AN_limbs + 1;
}
 ...
 *
 * This function is a thin wrapper around `awrtc_mbedtls_mpi_core_montmul()` that is
 * an alternative to calling `awrtc_mbedtls_mpi_mod_raw_to_mont_rep()` when we
 ...
 *                          `awrtc_mbedtls_mpi_core_montmul_init()`.
 ...
 *
 * This function is a thin wrapper around `awrtc_mbedtls_mpi_core_montmul()` that is
 * an alternative to calling `awrtc_mbedtls_mpi_mod_raw_from_mont_rep()` when we
 * don't want to allocate memory.
 *
 * \param[out]    X         The result of the conversion.
 *                          Must have the same number of limbs as \p A.
...
#endif /* AWRTC_MBEDTLS_BIGNUM_CORE_H */
