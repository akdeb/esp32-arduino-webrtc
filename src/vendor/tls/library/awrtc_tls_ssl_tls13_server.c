...
#if defined(AWRTC_MBEDTLS_SSL_SRV_C) && defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_3)
...
static const awrtc_mbedtls_ssl_ciphersuite_t *ssl_tls13_validate_peer_ciphersuite(
    awrtc_mbedtls_ssl_context *ssl,
    unsigned int cipher_suite)
{
    const awrtc_mbedtls_ssl_ciphersuite_t *ciphersuite_info;
    if (!awrtc_mbedtls_ssl_tls13_cipher_suite_is_offered(ssl, cipher_suite)) {
        return NULL;
    }

    ciphersuite_info = awrtc_mbedtls_ssl_ciphersuite_from_id(cipher_suite);
    if ((awrtc_mbedtls_ssl_validate_ciphersuite(ssl, ciphersuite_info,
                                          ssl->tls_version,
                                          ssl->tls_version) != 0)) {
        return NULL;
    }
...
static void ssl_tls13_select_ciphersuite(
    ...
    awrtc_psa_algorithm_t psk_hash_alg,
    const awrtc_mbedtls_ssl_ciphersuite_t **selected_ciphersuite_info)
{
    *selected_ciphersuite_info = NULL;
    ...
    for (const unsigned char *p = cipher_suites;
         ...
         */
        uint16_t id = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
        ...
        } else if (psk_hash_alg != AWRTC_PSA_ALG_NONE) {
            if (awrtc_mbedtls_md_psa_alg_from_type((awrtc_mbedtls_md_type_t) info->mac) !=
                psk_hash_alg) {
                continue;
            }
        }
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("No matched ciphersuite, psk_ciphersuite_id=%x, psk_hash_alg=%lx",
...
static int ssl_tls13_parse_key_exchange_modes_ext(awrtc_mbedtls_ssl_context *ssl,
                                                  ...
                                                  const unsigned char *end)
{
    ...
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, 1);
    ke_modes_len = *p++;
    ...
    if (ke_modes_len > 2) {
        AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER,
                                     AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
        return AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }

    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, ke_modes_len);

    while (ke_modes_len-- != 0) {
        switch (*p++) {
            case AWRTC_MBEDTLS_SSL_TLS1_3_PSK_MODE_PURE:
                ke_modes |= AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK;
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("Found PSK KEX MODE"));
                break;
            case AWRTC_MBEDTLS_SSL_TLS1_3_PSK_MODE_ECDHE:
                ke_modes |= AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_EPHEMERAL;
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("Found PSK_EPHEMERAL KEX MODE"));
                break;
            default:
                AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER,
                                             AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_key_exchange_is_psk_available(awrtc_mbedtls_ssl_context *ssl);
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_key_exchange_is_psk_ephemeral_available(awrtc_mbedtls_ssl_context *ssl);
    ...
    awrtc_mbedtls_ssl_session *session)
{
    ...
    awrtc_mbedtls_ms_time_t server_age;
    uint32_t client_age;
    awrtc_mbedtls_ms_time_t age_diff;
#endif

    ((void) obfuscated_ticket_age);

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> check_identity_match_ticket"));
     ...
     */
    ticket_buffer = awrtc_mbedtls_calloc(1, identity_len);
    ...
    switch (ret) {
        ...
        default:
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ticket_parse", ret);
            ret = SSL_TLS1_3_PSK_IDENTITY_DOES_NOT_MATCH;
    }

    /* We delete the temporary buffer */
    awrtc_mbedtls_free(ticket_buffer);
    ...
    if (session->tls_version != AWRTC_MBEDTLS_SSL_VERSION_TLS1_3) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("Ticket TLS version is not 1.3."));
        goto exit;
    }

#if defined(AWRTC_MBEDTLS_HAVE_TIME)
    now = awrtc_mbedtls_ms_time();

    if (now < session->ticket_creation_time) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(
            3, ("Invalid ticket creation time ( now = %" AWRTC_MBEDTLS_PRINTF_MS_TIME
                ", creation_time = %" AWRTC_MBEDTLS_PRINTF_MS_TIME " )",
     ...
     *
     */
    if (server_age > AWRTC_MBEDTLS_SSL_TLS1_3_MAX_ALLOWED_TICKET_LIFETIME * 1000) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(
            3, ("Ticket age exceeds limitation ticket_age = %" AWRTC_MBEDTLS_PRINTF_MS_TIME,
                server_age));
        goto exit;
    }
    ...
    age_diff = server_age - (awrtc_mbedtls_ms_time_t) client_age;
    if (age_diff < -AWRTC_MBEDTLS_SSL_TLS1_3_TICKET_AGE_TOLERANCE ||
        age_diff > AWRTC_MBEDTLS_SSL_TLS1_3_TICKET_AGE_TOLERANCE) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(
            3, ("Ticket age outside tolerance window ( diff = %"
                AWRTC_MBEDTLS_PRINTF_MS_TIME ")",
                age_diff));
        goto exit;
    }
    ...
    awrtc_mbedtls_ssl_session *session)
{
    ...
    ((void) session);
    ((void) obfuscated_ticket_age);
    *psk_type = AWRTC_MBEDTLS_SSL_TLS1_3_PSK_EXTERNAL;

    AWRTC_MBEDTLS_SSL_DEBUG_BUF(4, "identity", identity, identity_len);

#if defined(AWRTC_MBEDTLS_SSL_SESSION_TICKETS)
    ret = ssl_tls13_offered_psks_check_identity_match_ticket(
        ssl, identity, identity_len, obfuscated_ticket_age, session);
    if (ret == SSL_TLS1_3_PSK_IDENTITY_MATCH) {
        *psk_type = AWRTC_MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION;
        ret = awrtc_mbedtls_ssl_set_hs_psk(ssl,
                                     session->resumption_key,
                                     session->resumption_key_len);
        if (ret != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "awrtc_mbedtls_ssl_set_hs_psk", ret);
            return ret;
        }

        AWRTC_MBEDTLS_SSL_DEBUG_BUF(4, "Ticket-resumed PSK:",
                              session->resumption_key,
                              session->resumption_key_len);
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(4, ("ticket: obfuscated_ticket_age: %u",
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(5, "identity", identity, identity_len);
    /* Check identity with pre-configured psk */
    if (ssl->conf->psk_identity != NULL &&
        identity_len == ssl->conf->psk_identity_len &&
        awrtc_mbedtls_ct_memcmp(ssl->conf->psk_identity,
                          identity, identity_len) == 0) {
        ret = awrtc_mbedtls_ssl_set_hs_psk(ssl, ssl->conf->psk, ssl->conf->psk_len);
        if (ret != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "awrtc_mbedtls_ssl_set_hs_psk", ret);
            return ret;
        }
...
static int ssl_tls13_offered_psks_check_binder_match(
    ...
    int psk_type, awrtc_psa_algorithm_t psk_hash_alg)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    unsigned char transcript[AWRTC_PSA_HASH_MAX_SIZE];
    size_t transcript_len;
    unsigned char *psk;
    size_t psk_len;
    unsigned char server_computed_binder[AWRTC_PSA_HASH_MAX_SIZE];

    if (binder_len != AWRTC_PSA_HASH_LENGTH(psk_hash_alg)) {
        return SSL_TLS1_3_BINDER_DOES_NOT_MATCH;
    }

    /* Get current state of handshake transcript. */
    ret = awrtc_mbedtls_ssl_get_handshake_transcript(
        ssl, awrtc_mbedtls_md_type_from_psa_alg(psk_hash_alg),
    ...
    ret = awrtc_mbedtls_ssl_tls13_create_psk_binder(ssl, psk_hash_alg,
...
#endif
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("PSK binder calculation failed."));
        return AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }

    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "psk binder ( computed ): ",
                          server_computed_binder, transcript_len);
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "psk binder ( received ): ", binder, binder_len);

    if (awrtc_mbedtls_ct_memcmp(server_computed_binder,
                          binder,
                          AWRTC_PSA_HASH_LENGTH(psk_hash_alg)) == 0) {
        return SSL_TLS1_3_BINDER_MATCH;
    }

    awrtc_mbedtls_platform_zeroize(server_computed_binder,
...
#if defined(AWRTC_MBEDTLS_SSL_SESSION_TICKETS)
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_session_copy_ticket(awrtc_mbedtls_ssl_session *dst,
    ...
    struct psk_attributes *psk)
{
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "pre_shared_key extension",
                          pre_shared_key_ext,
     ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(identities, pre_shared_key_ext_end, 7 + 2);
    identities_len = AWRTC_MBEDTLS_GET_UINT16_BE(identities, 0);
    p_identity_len = identities + 2;
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p_identity_len, pre_shared_key_ext_end,
                                 identities_len);
    ...
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(binders, pre_shared_key_ext_end, 33 + 2);
    binders_len = AWRTC_MBEDTLS_GET_UINT16_BE(binders, 0);
    p_binder_len = binders + 2;
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p_binder_len, pre_shared_key_ext_end, binders_len);
    ...
    while (p_identity_len < identities_end && p_binder_len < binders_end) {
        ...
        AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p_identity_len, identities_end, 2 + 1 + 4);
        identity_len = AWRTC_MBEDTLS_GET_UINT16_BE(p_identity_len, 0);
        identity = p_identity_len + 2;
        AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(identity, identities_end, identity_len + 4);
        obfuscated_ticket_age = AWRTC_MBEDTLS_GET_UINT32_BE(identity, identity_len);
        p_identity_len += identity_len + 6;

        AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p_binder_len, binders_end, 1 + 32);
        binder_len = *p_binder_len;
        binder = p_binder_len + 1;
        AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(binder, binders_end, binder_len);
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(4, ("found matched identity"));

        switch (psk->type) {
            case AWRTC_MBEDTLS_SSL_TLS1_3_PSK_EXTERNAL:
                psk_ciphersuite_id = 0;
                psk_hash_alg = AWRTC_PSA_ALG_SHA_256;
                allowed_key_exchange_modes =
                    AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_ALL;
                break;
#if defined(AWRTC_MBEDTLS_SSL_SESSION_TICKETS)
            case AWRTC_MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION:
                psk_ciphersuite_id = session.ciphersuite;
                psk_hash_alg = AWRTC_PSA_ALG_NONE;
        ...
        psk->key_exchange_mode = AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_NONE;
        ...
        if ((allowed_key_exchange_modes &
            ...
            psk->key_exchange_mode = AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_EPHEMERAL;
            ...
            psk->key_exchange_mode = AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK;
        }

        if (psk->key_exchange_mode == AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_NONE) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("No suitable PSK key exchange mode"));
            continue;
        }
        ...
        ret = ssl_tls13_offered_psks_check_binder_match(
            ...
            awrtc_mbedtls_md_psa_alg_from_type((awrtc_mbedtls_md_type_t) psk->ciphersuite_info->mac));
        if (ret != SSL_TLS1_3_BINDER_MATCH) {
            ...
            awrtc_mbedtls_ssl_session_free(&session);
#endif
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("Invalid binder."));
...
#if defined(AWRTC_MBEDTLS_SSL_SESSION_TICKETS)
        if (psk->type == AWRTC_MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION) {
    ...
    if (p_identity_len != identities_end || p_binder_len != binders_end) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("pre_shared_key extension decode error"));
    ...
    if (matched_identity == -1) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("No usable PSK or ticket."));
        return AWRTC_MBEDTLS_ERR_SSL_UNKNOWN_IDENTITY;
    }

    ssl->handshake->selected_identity = (uint16_t) matched_identity;
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("Pre shared key found"));
 ...
 * } PreSharedKeyExtension;
 */
static int ssl_tls13_write_server_pre_shared_key_ext(awrtc_mbedtls_ssl_context *ssl,
                                                     unsigned char *buf,
                                                     unsigned char *end,
                                                     size_t *olen)
{
    ...
    *olen = 0;
    ...
    int not_using_psk = 0;
#if defined(AWRTC_MBEDTLS_USE_PSA_CRYPTO)
    not_using_psk = (awrtc_mbedtls_svc_key_id_is_null(ssl->handshake->psk_opaque));
#else
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, adding pre_shared_key extension"));
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 6);

    AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_TLS_EXT_PRE_SHARED_KEY, p, 0);
    AWRTC_MBEDTLS_PUT_UINT16_BE(2, p, 2);

    AWRTC_MBEDTLS_PUT_UINT16_BE(ssl->handshake->selected_identity, p, 4);

    *olen = 6;

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(4, ("sent selected_identity: %u",
                              ssl->handshake->selected_identity));

    awrtc_mbedtls_ssl_tls13_set_hs_sent_ext_mask(ssl, AWRTC_MBEDTLS_TLS_EXT_PRE_SHARED_KEY);
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_supported_versions_ext(awrtc_mbedtls_ssl_context *ssl,
                                                  const unsigned char *buf,
                                                  const unsigned char *end)
{
    const unsigned char *p = buf;
    ...
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, versions_len);
    versions_end = p + versions_len;
    while (p < versions_end) {
        AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, versions_end, 2);
        tls_version = awrtc_mbedtls_ssl_read_version(p, ssl->conf->transport);
        p += 2;

        if (AWRTC_MBEDTLS_SSL_VERSION_TLS1_3 == tls_version) {
            found_supported_version = 1;
            break;
        }

        if ((AWRTC_MBEDTLS_SSL_VERSION_TLS1_2 == tls_version) &&
            awrtc_mbedtls_ssl_conf_is_tls12_enabled(ssl->conf)) {
            found_supported_version = 1;
            break;
        }
    }
    ...
    if (!found_supported_version) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("No supported version found."));

        AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_PROTOCOL_VERSION,
                                     AWRTC_MBEDTLS_ERR_SSL_BAD_PROTOCOL_VERSION);
        return AWRTC_MBEDTLS_ERR_SSL_BAD_PROTOCOL_VERSION;
    }

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("Negotiated version: [%04x]",
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_supported_groups_ext(awrtc_mbedtls_ssl_context *ssl,
                                                const unsigned char *buf,
                                                const unsigned char *end)
{
    const unsigned char *p = buf;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "supported_groups extension", p, end - buf);
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, 2);
    named_group_list_len = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
    p += 2;
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, named_group_list_len);
    named_group_list_end = p + named_group_list_len;
    ssl->handshake->hrr_selected_group = 0;

    while (p < named_group_list_end) {
        uint16_t named_group;
        AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, named_group_list_end, 2);
        named_group = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
        p += 2;

        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2,
                              ("got named group: %s(%04x)",
                               awrtc_mbedtls_ssl_named_group_to_str(named_group),
                               named_group));

        if (!awrtc_mbedtls_ssl_named_group_is_offered(ssl, named_group) ||
            !awrtc_mbedtls_ssl_named_group_is_supported(named_group) ||
            ssl->handshake->hrr_selected_group != 0) {
            continue;
        }
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_key_shares_ext(awrtc_mbedtls_ssl_context *ssl,
                                          const unsigned char *buf,
                                          const unsigned char *end)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, 2);
    client_shares_len = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
    p += 2;
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, client_shares_len);
    ...
    while (p < client_shares_end) {
         ...
         * } KeyShareEntry;
         */
        AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, client_shares_end, 4);
        group = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
        key_exchange_len = AWRTC_MBEDTLS_GET_UINT16_BE(p, 2);
        p += 4;
        key_exchange = p;
        AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, client_shares_end, key_exchange_len);
        p += key_exchange_len;

        /* Continue parsing even if we have already found a match,
         * for input validation purposes.
         */
        if (!awrtc_mbedtls_ssl_named_group_is_offered(ssl, group) ||
            !awrtc_mbedtls_ssl_named_group_is_supported(group) ||
            ssl->handshake->offered_group_id != 0) {
            continue;
        }
         ...
         */
        if (awrtc_mbedtls_ssl_tls13_named_group_is_ecdhe(group) ||
            awrtc_mbedtls_ssl_tls13_named_group_is_ffdh(group)) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("ECDH/FFDH group: %s (%04x)",
                                      awrtc_mbedtls_ssl_named_group_to_str(group),
                                      group));
            ret = awrtc_mbedtls_ssl_tls13_read_public_xxdhe_share(
            ...
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(4, ("Unrecognized NamedGroup %u",
                                      (unsigned) group));
            continue;
        }
    ...
    if (ssl->handshake->offered_group_id == 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("no matching key share"));
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_client_hello_has_exts(awrtc_mbedtls_ssl_context *ssl,
                                           int exts_mask)
{
...
{
#if defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_ENABLED)
    return awrtc_mbedtls_ssl_conf_tls13_is_psk_enabled(ssl) &&
           awrtc_mbedtls_ssl_tls13_is_psk_supported(ssl) &&
...
{
#if defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_EPHEMERAL_ENABLED)
    return awrtc_mbedtls_ssl_conf_tls13_is_psk_ephemeral_enabled(ssl) &&
           awrtc_mbedtls_ssl_tls13_is_psk_ephemeral_supported(ssl) &&
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_key_exchange_is_ephemeral_available(awrtc_mbedtls_ssl_context *ssl)
{
#if defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED)
    return awrtc_mbedtls_ssl_conf_tls13_is_ephemeral_enabled(ssl) &&
    ...
    defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED)

#if defined(AWRTC_MBEDTLS_USE_PSA_CRYPTO)
static awrtc_psa_algorithm_t ssl_tls13_iana_sig_alg_to_psa_alg(uint16_t sig_alg)
{
    switch (sig_alg) {
        case AWRTC_MBEDTLS_TLS1_3_SIG_ECDSA_SECP256R1_SHA256:
            return AWRTC_PSA_ALG_ECDSA(AWRTC_PSA_ALG_SHA_256);
        case AWRTC_MBEDTLS_TLS1_3_SIG_ECDSA_SECP384R1_SHA384:
            return AWRTC_PSA_ALG_ECDSA(AWRTC_PSA_ALG_SHA_384);
        case AWRTC_MBEDTLS_TLS1_3_SIG_ECDSA_SECP521R1_SHA512:
            return AWRTC_PSA_ALG_ECDSA(AWRTC_PSA_ALG_SHA_512);
        case AWRTC_MBEDTLS_TLS1_3_SIG_RSA_PSS_RSAE_SHA256:
            return AWRTC_PSA_ALG_RSA_PSS(AWRTC_PSA_ALG_SHA_256);
        case AWRTC_MBEDTLS_TLS1_3_SIG_RSA_PSS_RSAE_SHA384:
            return AWRTC_PSA_ALG_RSA_PSS(AWRTC_PSA_ALG_SHA_384);
        case AWRTC_MBEDTLS_TLS1_3_SIG_RSA_PSS_RSAE_SHA512:
            return AWRTC_PSA_ALG_RSA_PSS(AWRTC_PSA_ALG_SHA_512);
        case AWRTC_MBEDTLS_TLS1_3_SIG_RSA_PKCS1_SHA256:
            return AWRTC_PSA_ALG_RSA_PKCS1V15_SIGN(AWRTC_PSA_ALG_SHA_256);
        case AWRTC_MBEDTLS_TLS1_3_SIG_RSA_PKCS1_SHA384:
            return AWRTC_PSA_ALG_RSA_PKCS1V15_SIGN(AWRTC_PSA_ALG_SHA_384);
        case AWRTC_MBEDTLS_TLS1_3_SIG_RSA_PKCS1_SHA512:
            return AWRTC_PSA_ALG_RSA_PKCS1V15_SIGN(AWRTC_PSA_ALG_SHA_512);
        default:
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_pick_key_cert(awrtc_mbedtls_ssl_context *ssl)
{
    awrtc_mbedtls_ssl_key_cert *key_cert, *key_cert_list;
    ...
    for (; *sig_alg != AWRTC_MBEDTLS_TLS1_3_SIG_NONE; sig_alg++) {
        ...
        for (key_cert = key_cert_list; key_cert != NULL;
...
#if defined(AWRTC_MBEDTLS_USE_PSA_CRYPTO)
            awrtc_psa_algorithm_t awrtc_psa_alg = AWRTC_PSA_ALG_NONE;
#endif /* AWRTC_MBEDTLS_USE_PSA_CRYPTO */

            AWRTC_MBEDTLS_SSL_DEBUG_CRT(3, "certificate (chain) candidate",
                                  key_cert->cert);
             ...
             */
            if (awrtc_mbedtls_x509_crt_check_key_usage(
                    key_cert->cert, AWRTC_MBEDTLS_X509_KU_DIGITAL_SIGNATURE) != 0 ||
                awrtc_mbedtls_x509_crt_check_extended_key_usage(
                    key_cert->cert, AWRTC_MBEDTLS_OID_SERVER_AUTH,
                    AWRTC_MBEDTLS_OID_SIZE(AWRTC_MBEDTLS_OID_SERVER_AUTH)) != 0) {
            ...
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(3,
                                   ...
                                   *sig_alg));
#if defined(AWRTC_MBEDTLS_USE_PSA_CRYPTO)
            awrtc_psa_alg = ssl_tls13_iana_sig_alg_to_psa_alg(*sig_alg);
#endif /* AWRTC_MBEDTLS_USE_PSA_CRYPTO */

            if (awrtc_mbedtls_ssl_tls13_check_sig_alg_cert_key_match(
                    *sig_alg, &key_cert->cert->pk)
#if defined(AWRTC_MBEDTLS_USE_PSA_CRYPTO)
                && awrtc_psa_alg != AWRTC_PSA_ALG_NONE &&
                awrtc_mbedtls_pk_can_do_ext(&key_cert->cert->pk, awrtc_psa_alg,
                                      AWRTC_PSA_KEY_USAGE_SIGN_HASH) == 1
#endif /* AWRTC_MBEDTLS_USE_PSA_CRYPTO */
                ) {
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("ssl_tls13_pick_key_cert:"
                              "no suitable certificate found"));
    return -1;
}
#endif /* AWRTC_MBEDTLS_X509_CRT_PARSE_C &&
          AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED */
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_client_hello(awrtc_mbedtls_ssl_context *ssl,
                                        const unsigned char *buf,
                                        const unsigned char *end)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
     ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, 38);
     ...
     * uint16 ProtocolVersion;
     */
    if (awrtc_mbedtls_ssl_read_version(p, ssl->conf->transport) !=
        AWRTC_MBEDTLS_SSL_VERSION_TLS1_2) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("Unsupported version of TLS."));
        AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_PROTOCOL_VERSION,
                                     AWRTC_MBEDTLS_ERR_SSL_BAD_PROTOCOL_VERSION);
        return AWRTC_MBEDTLS_ERR_SSL_BAD_PROTOCOL_VERSION;
    }
     ...
     */
    random = p;
    p += AWRTC_MBEDTLS_CLIENT_HELLO_RANDOM_LEN;
     ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, legacy_session_id_len + 2);
     ...
     * uint8 CipherSuite[2];
     */
    cipher_suites_len = AWRTC_MBEDTLS_GET_UINT16_BE(p, 0);
    p += 2;
    cipher_suites = p;
     ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p, end, cipher_suites_len + 1);
    p += cipher_suites_len;
    cipher_suites_end = p;
     ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_READ_PTR(p + 1, end, p[0] + 2);

    /*
     * Search for the supported versions extension and parse it to determine
     * if the client supports TLS 1.3.
     */
    ret = awrtc_mbedtls_ssl_tls13_is_supported_versions_ext_present_in_exts(
        ssl, p + 1 + p[0], end,
        &supported_versions_data, &supported_versions_data_end);
    if (ret < 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1,
                              ("awrtc_mbedtls_ssl_tls13_is_supported_versions_ext_present_in_exts"), ret);
        return ret;
    }

    if (ret == 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("no supported_versions extension"));
    ...
    if (ret == 1) {
         ...
         * AWRTC_MBEDTLS_SSL_VERSION_TLS1_2 or AWRTC_MBEDTLS_SSL_VERSION_TLS1_3, defining
         * the TLS version to negotiate.
         */
        if (AWRTC_MBEDTLS_SSL_VERSION_TLS1_2 == ret) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("supported_versions without 1.3"));
            return SSL_CLIENT_HELLO_TLS1_2;
        }
    }
     ...
     */
    ssl->tls_version = AWRTC_MBEDTLS_SSL_VERSION_TLS1_3;
    ssl->session_negotiate->tls_version = AWRTC_MBEDTLS_SSL_VERSION_TLS1_3;
    ssl->session_negotiate->endpoint = ssl->conf->endpoint;

    /* Before doing any crypto, make sure we can. */
    ret = awrtc_mbedtls_ssl_tls13_crypto_init(ssl);
    if (ret != 0) {
        return ret;
    }
     ...
     */
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "client hello, random bytes",
                          random, AWRTC_MBEDTLS_CLIENT_HELLO_RANDOM_LEN);
    memcpy(&handshake->randbytes[0], random, AWRTC_MBEDTLS_CLIENT_HELLO_RANDOM_LEN);
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "client hello, session id",
     ...
     */
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "client hello, list of cipher suites",
                          cipher_suites, cipher_suites_len);

    ssl_tls13_select_ciphersuite(ssl, cipher_suites, cipher_suites_end,
                                 0, AWRTC_PSA_ALG_NONE, &handshake->ciphersuite_info);
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("selected ciphersuite: %04x - %s",
     ...
     * ...
     */
    if (p[0] != 1 || p[1] != AWRTC_MBEDTLS_SSL_COMPRESS_NULL) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("bad legacy compression method"));
        AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER,
                                     AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
        return AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
    }
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "client hello extensions", p, extensions_len);
    ...
    while (p < extensions_end) {
        ...
        uint32_t allowed_exts = AWRTC_MBEDTLS_SSL_TLS1_3_ALLOWED_EXTS_OF_CH;

        if (ssl->handshake->hello_retry_request_flag) {
            /* Do not accept early data extension in 2nd ClientHello */
            allowed_exts &= ~AWRTC_MBEDTLS_SSL_EXT_MASK(EARLY_DATA);
        }
         ...
         */
        if (handshake->received_extensions & AWRTC_MBEDTLS_SSL_EXT_MASK(PRE_SHARED_KEY)) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(
        ...
        ret = awrtc_mbedtls_ssl_tls13_check_received_extension(
            ssl, AWRTC_MBEDTLS_SSL_HS_CLIENT_HELLO, extension_type,
            allowed_exts);
        if (ret != 0) {
            return ret;
        }
        ...
        switch (extension_type) {
            ...
            case AWRTC_MBEDTLS_TLS_EXT_SERVERNAME:
                ...
                ret = awrtc_mbedtls_ssl_parse_server_name_ext(ssl, p,
                                                        extension_data_end);
                if (ret != 0) {
                    AWRTC_MBEDTLS_SSL_DEBUG_RET(
                        1, "awrtc_mbedtls_ssl_parse_servername_ext", ret);
                    return ret;
                }
                break;
#endif /* AWRTC_MBEDTLS_SSL_SERVER_NAME_INDICATION */
            ...
            case AWRTC_MBEDTLS_TLS_EXT_SUPPORTED_GROUPS:
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("found supported group extension"));
            ...
            case AWRTC_MBEDTLS_TLS_EXT_KEY_SHARE:
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("found key share extension"));
                ...
                if (ret == SSL_TLS1_3_PARSE_KEY_SHARES_EXT_NO_MATCH) {
                    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("No usable share for key agreement."));
            ...
            case AWRTC_MBEDTLS_TLS_EXT_PRE_SHARED_KEY:
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("found pre_shared_key extension"));
                if ((handshake->received_extensions &
                     AWRTC_MBEDTLS_SSL_EXT_MASK(PSK_KEY_EXCHANGE_MODES)) == 0) {
                    AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(
                        AWRTC_MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER,
                        AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
                    return AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
            ...
            case AWRTC_MBEDTLS_TLS_EXT_ALPN:
                ...
                ret = awrtc_mbedtls_ssl_parse_alpn_ext(ssl, p, extension_data_end);
                if (ret != 0) {
                    AWRTC_MBEDTLS_SSL_DEBUG_RET(
                        1, ("awrtc_mbedtls_ssl_parse_alpn_ext"), ret);
                    return ret;
                }
                break;
#endif /* AWRTC_MBEDTLS_SSL_ALPN */
            ...
            case AWRTC_MBEDTLS_TLS_EXT_SIG_ALG:
                ...
                ret = awrtc_mbedtls_ssl_parse_sig_alg_ext(
                    ssl, p, extension_data_end);
                if (ret != 0) {
                    AWRTC_MBEDTLS_SSL_DEBUG_RET(
                        1, "awrtc_mbedtls_ssl_parse_sig_alg_ext", ret);
                    return ret;
                }
                break;
#endif /* AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED */
            ...
            case AWRTC_MBEDTLS_TLS_EXT_RECORD_SIZE_LIMIT:
                AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("found record_size_limit extension"));

                ret = awrtc_mbedtls_ssl_tls13_parse_record_size_limit_ext(
                    ssl, p, extension_data_end);
                if (ret != 0) {
                    AWRTC_MBEDTLS_SSL_DEBUG_RET(
                        1, ("awrtc_mbedtls_ssl_tls13_parse_record_size_limit_ext"), ret);
                    return ret;
                }
                break;
            ...
            default:
                AWRTC_MBEDTLS_SSL_PRINT_EXT(
                    3, AWRTC_MBEDTLS_SSL_HS_CLIENT_HELLO,
    ...
    AWRTC_MBEDTLS_SSL_PRINT_EXTS(3, AWRTC_MBEDTLS_SSL_HS_CLIENT_HELLO,
                           handshake->received_extensions);

    ret = awrtc_mbedtls_ssl_add_hs_hdr_to_checksum(ssl,
                                             AWRTC_MBEDTLS_SSL_HS_CLIENT_HELLO,
                                             p - buf);
    if (0 != ret) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, ("awrtc_mbedtls_ssl_add_hs_hdr_to_checksum"), ret);
        return ret;
    }
    ...
    if (handshake->received_extensions & AWRTC_MBEDTLS_SSL_EXT_MASK(PRE_SHARED_KEY)) {
        ...
        if (ret == 0) {
            got_psk = 1;
        } else if (ret != AWRTC_MBEDTLS_ERR_SSL_UNKNOWN_IDENTITY) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(
    ...
    if (got_psk && (psk.key_exchange_mode ==
        ...
        handshake->key_exchange_mode =
            AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_EPHEMERAL;
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("key exchange mode: psk_ephemeral"));
    ...
    if (ssl_tls13_key_exchange_is_ephemeral_available(ssl)) {
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("key exchange mode: ephemeral"));

    }
#if defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
    else if (got_psk && (psk.key_exchange_mode ==
                         AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK)) {
        handshake->key_exchange_mode = AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK;
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("key exchange mode: psk"));
    }
#endif
    else {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(
        ...
        AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_MISSING_EXTENSION,
                                     AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
        return AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
    }
    ...
    if (handshake->key_exchange_mode &
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("Select PSK ciphersuite: %04x - %s",
                                  ((unsigned) psk.ciphersuite_info->id),
                                  psk.ciphersuite_info->name));

        if (psk.type == AWRTC_MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION && (!hrr_required)) {
            handshake->resume = 1;
        }
    }
#endif

    awrtc_mbedtls_ssl_optimize_checksum(ssl, handshake->ciphersuite_info);

    return hrr_required ? SSL_CLIENT_HELLO_HRR_REQUIRED : SSL_CLIENT_HELLO_OK;
}

#if defined(AWRTC_MBEDTLS_SSL_EARLY_DATA)
static int ssl_tls13_check_early_data_requirements(awrtc_mbedtls_ssl_context *ssl)
{
    awrtc_mbedtls_ssl_handshake_params *handshake = ssl->handshake;

    if (ssl->conf->early_data_enabled == AWRTC_MBEDTLS_SSL_EARLY_DATA_DISABLED) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(
            1,
    ...
    if (!awrtc_mbedtls_ssl_tls13_session_ticket_allow_early_data(ssl->session_negotiate)) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(
            1,
...
#if defined(AWRTC_MBEDTLS_SSL_ALPN)
    const char *alpn = awrtc_mbedtls_ssl_get_alpn_protocol(ssl);
    size_t alpn_len;
    ...
    if (alpn == NULL ||
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("EarlyData: rejected, the selected ALPN is different "
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_postprocess_client_hello(awrtc_mbedtls_ssl_context *ssl,
                                              int hrr_required)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = awrtc_mbedtls_ssl_tls13_key_schedule_stage_early(ssl);
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1,
                              "awrtc_mbedtls_ssl_tls1_3_key_schedule_stage_early", ret);
        return ret;
    }

#if defined(AWRTC_MBEDTLS_SSL_EARLY_DATA)
    if (ssl->handshake->received_extensions & AWRTC_MBEDTLS_SSL_EXT_MASK(EARLY_DATA)) {
        ...
        if (ssl->handshake->early_data_accepted) {
            ret = awrtc_mbedtls_ssl_tls13_compute_early_transform(ssl);
            if (ret != 0) {
                AWRTC_MBEDTLS_SSL_DEBUG_RET(
                    1, "awrtc_mbedtls_ssl_tls13_compute_early_transform", ret);
                return ret;
            }
        } else {
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_process_client_hello(awrtc_mbedtls_ssl_context *ssl)
{
    ...
    AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_tls13_fetch_handshake_msg(
                             ssl, AWRTC_MBEDTLS_SSL_HS_CLIENT_HELLO,
                             &buf, &buflen));

    AWRTC_MBEDTLS_SSL_PROC_CHK_NEG(ssl_tls13_parse_client_hello(ssl, buf,
                                                          buf + buflen));
    parse_client_hello_ret = ret; /* Store positive return value of
                                   * parse_client_hello,
                                   * as negative error codes are handled
                                   * by AWRTC_MBEDTLS_SSL_PROC_CHK_NEG. */
     ...
     * AWRTC_MBEDTLS_SSL_VERSION_TLS1_2 thus from now on awrtc_mbedtls_ssl_handshake_step()
     * will dispatch to the TLS 1.2 state machine.
     */
    if (SSL_CLIENT_HELLO_TLS1_2 == parse_client_hello_ret) {
        if (ssl->handshake->hello_retry_request_flag) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("Non compliant 2nd ClientHello, TLS 1.2 version"));
            AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER,
                                         AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
            return AWRTC_MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
        }
        if (!awrtc_mbedtls_ssl_conf_is_tls12_enabled(ssl->conf)) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("non-1.3 ClientHello left for later processing"));
        return 0;
    }
    ...
    if (SSL_CLIENT_HELLO_OK == parse_client_hello_ret) {
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_SERVER_HELLO);
    } else {
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_HELLO_RETRY_REQUEST);
    }
...
/*
 * Handler for AWRTC_MBEDTLS_SSL_SERVER_HELLO
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_prepare_server_hello(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *server_randbytes =
        ssl->handshake->randbytes + AWRTC_MBEDTLS_CLIENT_HELLO_RANDOM_LEN;

    if ((ret = ssl->conf->f_rng(ssl->conf->p_rng, server_randbytes,
                                AWRTC_MBEDTLS_SERVER_HELLO_RANDOM_LEN)) != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "f_rng", ret);
        return ret;
    }

    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "server hello, random bytes", server_randbytes,
    ...
    size_t *out_len)
{
    *out_len = 0;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, write selected version"));
     ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(buf, end, 6);

    AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_TLS_EXT_SUPPORTED_VERSIONS, buf, 0);

    AWRTC_MBEDTLS_PUT_UINT16_BE(2, buf, 2);

    awrtc_mbedtls_ssl_write_version(buf + 4,
                              ssl->conf->transport,
                              ssl->tls_version);

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("supported version: [%04x]",
                              ssl->tls_version));

    *out_len = 6;

    awrtc_mbedtls_ssl_tls13_set_hs_sent_ext_mask(
        ssl, AWRTC_MBEDTLS_TLS_EXT_SUPPORTED_VERSIONS);
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_generate_and_write_key_share(awrtc_mbedtls_ssl_context *ssl,
                                                  uint16_t named_group,
                                                  unsigned char *buf,
                                                  unsigned char *end,
                                                  size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    if (awrtc_mbedtls_ssl_tls13_named_group_is_ecdhe(named_group) ||
        awrtc_mbedtls_ssl_tls13_named_group_is_ffdh(named_group)) {
        ret = awrtc_mbedtls_ssl_tls13_generate_and_write_xxdh_key_exchange(
            ssl, named_group, buf, end, out_len);
        if (ret != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(
                1, "awrtc_mbedtls_ssl_tls13_generate_and_write_xxdh_key_exchange",
                ret);
            return ret;
        }
    } else
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_key_share_ext(awrtc_mbedtls_ssl_context *ssl,
                                         unsigned char *buf,
                                         unsigned char *end,
                                         size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, adding key share extension"));

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("server hello, write selected_group: %s (%04x)",
                              awrtc_mbedtls_ssl_named_group_to_str(group),
                              group));
     ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 8);
    AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_TLS_EXT_KEY_SHARE, p, 0);
    AWRTC_MBEDTLS_PUT_UINT16_BE(group, server_share, 0);
    p += 8;
    ...
    AWRTC_MBEDTLS_PUT_UINT16_BE(key_exchange_length, server_share + 2, 0);

    AWRTC_MBEDTLS_PUT_UINT16_BE(p - server_share, buf, 2);

    *out_len = p - buf;

    awrtc_mbedtls_ssl_tls13_set_hs_sent_ext_mask(ssl, AWRTC_MBEDTLS_TLS_EXT_KEY_SHARE);

    return 0;
}

AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_hrr_key_share_ext(awrtc_mbedtls_ssl_context *ssl,
                                             unsigned char *buf,
                                             unsigned char *end,
                                             size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    if (ssl->handshake->offered_group_id != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(4, ("Skip key_share extension in HRR"));
        return 0;
    }

    if (selected_group == 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("no matching named group found"));
        return AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }
    ...
    AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_TLS_EXT_KEY_SHARE, buf, 0);
    AWRTC_MBEDTLS_PUT_UINT16_BE(2, buf, 2);
    AWRTC_MBEDTLS_PUT_UINT16_BE(selected_group, buf, 4);

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3,
                          ("HRR selected_group: %s (%x)",
                           awrtc_mbedtls_ssl_named_group_to_str(selected_group),
                           selected_group));
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_server_hello_body(awrtc_mbedtls_ssl_context *ssl,
                                             unsigned char *buf,
                                             unsigned char *end,
                                             size_t *out_len,
                                             int is_hrr)
{
    ...
    *out_len = 0;
    ssl->handshake->sent_extensions = AWRTC_MBEDTLS_SSL_EXT_MASK_NONE;
    ...
    AWRTC_MBEDTLS_PUT_UINT16_BE(0x0303, p, 0);
    p += 2;
     ...
     * ...
     * with Random defined as:
     * opaque Random[AWRTC_MBEDTLS_SERVER_HELLO_RANDOM_LEN];
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, AWRTC_MBEDTLS_SERVER_HELLO_RANDOM_LEN);
    if (is_hrr) {
        memcpy(p, awrtc_mbedtls_ssl_tls13_hello_retry_request_magic,
               AWRTC_MBEDTLS_SERVER_HELLO_RANDOM_LEN);
    } else {
        memcpy(p, &ssl->handshake->randbytes[AWRTC_MBEDTLS_CLIENT_HELLO_RANDOM_LEN],
               AWRTC_MBEDTLS_SERVER_HELLO_RANDOM_LEN);
    }
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "server hello, random bytes",
                          p, AWRTC_MBEDTLS_SERVER_HELLO_RANDOM_LEN);
    p += AWRTC_MBEDTLS_SERVER_HELLO_RANDOM_LEN;
     ...
     * ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 1 + ssl->session_negotiate->id_len);
    *p++ = (unsigned char) ssl->session_negotiate->id_len;
    if (ssl->session_negotiate->id_len > 0) {
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "session id", ssl->session_negotiate->id,
                              ssl->session_negotiate->id_len);
    }
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3,
                          ("server hello, chosen ciphersuite: %s ( id=%d )",
                           awrtc_mbedtls_ssl_get_ciphersuite_name(
                               ssl->session_negotiate->ciphersuite),
                           ssl->session_negotiate->ciphersuite));
     ...
     * ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 1);
    *p++ = AWRTC_MBEDTLS_SSL_COMPRESS_NULL;
...
#if defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
    if (!is_hrr && awrtc_mbedtls_ssl_tls13_key_exchange_mode_with_psk(ssl)) {
        ret = ssl_tls13_write_server_pre_shared_key_ext(ssl, p, end, &output_len);
        if (ret != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_tls13_write_server_pre_shared_key_ext",
                                  ret);
            return ret;
        }
        p += output_len;
    ...
    AWRTC_MBEDTLS_PUT_UINT16_BE(p - p_extensions_len - 2, p_extensions_len, 0);

    AWRTC_MBEDTLS_SSL_DEBUG_BUF(4, "server hello extensions",
                          p_extensions_len, p - p_extensions_len);

    *out_len = p - buf;

    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "server hello", buf, *out_len);

    AWRTC_MBEDTLS_SSL_PRINT_EXTS(
        3, is_hrr ? AWRTC_MBEDTLS_SSL_TLS1_3_HS_HELLO_RETRY_REQUEST :
        AWRTC_MBEDTLS_SSL_HS_SERVER_HELLO,
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_finalize_server_hello(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ret = awrtc_mbedtls_ssl_tls13_compute_handshake_transform(ssl);
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1,
                              "awrtc_mbedtls_ssl_tls13_compute_handshake_transform",
                              ret);
        return ret;
    }
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_server_hello(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf;
    ...
    AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_prepare_server_hello(ssl));

    AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_start_handshake_msg(
                             ssl, AWRTC_MBEDTLS_SSL_HS_SERVER_HELLO, &buf, &buf_len));

    AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_server_hello_body(ssl, buf,
                                                           buf + buf_len,
                                                           &msg_len,
                                                           0));

    AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_add_hs_msg_to_checksum(
                             ssl, AWRTC_MBEDTLS_SSL_HS_SERVER_HELLO, buf, msg_len));

    AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_finish_handshake_msg(
                             ssl, buf_len, msg_len));

    AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_finalize_server_hello(ssl));
     ...
     */
    awrtc_mbedtls_ssl_handshake_set_state(
        ssl, AWRTC_MBEDTLS_SSL_SERVER_CCS_AFTER_SERVER_HELLO);
#else
    awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_ENCRYPTED_EXTENSIONS);
...
/*
 * Handler for AWRTC_MBEDTLS_SSL_HELLO_RETRY_REQUEST
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_prepare_hello_retry_request(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    if (ssl->handshake->hello_retry_request_flag) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("Too many HRRs"));
        AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE,
                                     AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE);
        return AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }
     ...
     */
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(4, ("Reset transcript for HRR"));
    ret = awrtc_mbedtls_ssl_reset_transcript_for_hrr(ssl);
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "awrtc_mbedtls_ssl_reset_transcript_for_hrr", ret);
        return ret;
    }
    awrtc_mbedtls_ssl_session_reset_msg_layer(ssl, 0);

    return 0;
}

AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_hello_retry_request(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf;
    size_t buf_len, msg_len;

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> write hello retry request"));

    AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_prepare_hello_retry_request(ssl));

    AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_start_handshake_msg(
                             ssl, AWRTC_MBEDTLS_SSL_HS_SERVER_HELLO,
                             &buf, &buf_len));
    ...
    AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_finish_handshake_msg(ssl, buf_len,
                                                          msg_len));
     ...
     */
    awrtc_mbedtls_ssl_handshake_set_state(
        ssl, AWRTC_MBEDTLS_SSL_SERVER_CCS_AFTER_HELLO_RETRY_REQUEST);
#else
...
/*
 * Handler for AWRTC_MBEDTLS_SSL_ENCRYPTED_EXTENSIONS
 */
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_encrypted_extensions_body(awrtc_mbedtls_ssl_context *ssl,
                                                     unsigned char *buf,
                                                     unsigned char *end,
                                                     size_t *out_len)
{
    ...
    *out_len = 0;
...
#if defined(AWRTC_MBEDTLS_SSL_ALPN)
    ret = awrtc_mbedtls_ssl_write_alpn_ext(ssl, p, end, &output_len);
    if (ret != 0) {
        return ret;
    }
    p += output_len;
...
#if defined(AWRTC_MBEDTLS_SSL_EARLY_DATA)
    if (ssl->handshake->early_data_accepted) {
        ret = awrtc_mbedtls_ssl_tls13_write_early_data_ext(
...
#if defined(AWRTC_MBEDTLS_SSL_RECORD_SIZE_LIMIT)
    if (ssl->handshake->received_extensions & AWRTC_MBEDTLS_SSL_EXT_MASK(RECORD_SIZE_LIMIT)) {
        ret = awrtc_mbedtls_ssl_tls13_write_record_size_limit_ext(
    ...
    AWRTC_MBEDTLS_PUT_UINT16_BE(extensions_len, p_extensions_len, 0);

    *out_len = p - buf;

    AWRTC_MBEDTLS_SSL_DEBUG_BUF(4, "encrypted extensions", buf, *out_len);

    AWRTC_MBEDTLS_SSL_PRINT_EXTS(
        3, AWRTC_MBEDTLS_SSL_HS_ENCRYPTED_EXTENSIONS, ssl->handshake->sent_extensions);

    return 0;
}

AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_encrypted_extensions(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf;
    size_t buf_len, msg_len;

    awrtc_mbedtls_ssl_set_outbound_transform(ssl,
                                       ssl->handshake->transform_handshake);
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(
        3, ("switching to handshake transform for outbound data"));

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> write encrypted extensions"));

    AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_start_handshake_msg(
                             ssl, AWRTC_MBEDTLS_SSL_HS_ENCRYPTED_EXTENSIONS,
                             &buf, &buf_len));

    AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_encrypted_extensions_body(
    ...
    if (awrtc_mbedtls_ssl_tls13_key_exchange_mode_with_psk(ssl)) {
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_SERVER_FINISHED);

        /* Since we're not using a certificate, set verify_result to success */
        ssl->session_negotiate->verify_result = 0;
    } else {
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_CERTIFICATE_REQUEST);
    }
#else
    awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_SERVER_FINISHED);
#endif
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_certificate_request_coordinate(awrtc_mbedtls_ssl_context *ssl)
{
    int authmode;
    ...
    if (authmode == AWRTC_MBEDTLS_SSL_VERIFY_NONE) {
        ssl->session_negotiate->verify_result = AWRTC_MBEDTLS_X509_BADCERT_SKIP_VERIFY;
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_certificate_request_body(awrtc_mbedtls_ssl_context *ssl,
                                                    unsigned char *buf,
                                                    const unsigned char *end,
                                                    size_t *out_len)
{
    ...
    *p++ = 0x0;
    ...
    p_extensions_len = p;
    p += 2;
    ret = awrtc_mbedtls_ssl_write_sig_alg_ext(ssl, p, end, &output_len);
    if (ret != 0) {
        return ret;
    }
    ...
    *out_len = p - buf;
    ...
    AWRTC_MBEDTLS_SSL_PRINT_EXTS(
        3, AWRTC_MBEDTLS_SSL_HS_CERTIFICATE_REQUEST, ssl->handshake->sent_extensions);

    return 0;
}

AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_certificate_request(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> write certificate request"));

    AWRTC_MBEDTLS_SSL_PROC_CHK_NEG(ssl_tls13_certificate_request_coordinate(ssl));
    ...
    if (ret == SSL_CERTIFICATE_REQUEST_SEND_REQUEST) {
        ...
        AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_start_handshake_msg(
                                 ssl, AWRTC_MBEDTLS_SSL_HS_CERTIFICATE_REQUEST,
                                 &buf, &buf_len));

        AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_certificate_request_body(
                                 ssl, buf, buf + buf_len, &msg_len));
    ...
    awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_SERVER_CERTIFICATE);
cleanup:
...
/*
 * Handler for AWRTC_MBEDTLS_SSL_SERVER_CERTIFICATE
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_server_certificate(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

#if defined(AWRTC_MBEDTLS_X509_CRT_PARSE_C)
    if ((ssl_tls13_pick_key_cert(ssl) != 0) ||
        awrtc_mbedtls_ssl_own_cert(ssl) == NULL) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("No certificate available."));
        AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(AWRTC_MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE,
                                     AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE);
        return AWRTC_MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }
    ...
    ret = awrtc_mbedtls_ssl_tls13_write_certificate(ssl);
    if (ret != 0) {
        return ret;
    }
    awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_CERTIFICATE_VERIFY);
    return 0;
}

/*
 * Handler for AWRTC_MBEDTLS_SSL_CERTIFICATE_VERIFY
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_certificate_verify(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = awrtc_mbedtls_ssl_tls13_write_certificate_verify(ssl);
    if (ret != 0) {
        return ret;
    }
    awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_SERVER_FINISHED);
    ...
    awrtc_mbedtls_ssl_context *ssl)
{
    if (ssl->handshake->certificate_request_sent) {
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_CLIENT_CERTIFICATE);
    } else {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("skip parse certificate"));
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("skip parse certificate verify"));

        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_CLIENT_FINISHED);
    }
}

/*
 * Handler for AWRTC_MBEDTLS_SSL_SERVER_FINISHED
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_server_finished(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    ret = awrtc_mbedtls_ssl_tls13_write_finished_message(ssl);
    if (ret != 0) {
        return ret;
    }

    ret = awrtc_mbedtls_ssl_tls13_compute_application_transform(ssl);
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_PEND_FATAL_ALERT(
    ...
    if (ssl->handshake->early_data_accepted) {
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(
            1, ("Switch to early keys for inbound traffic. "
                "( K_recv = early data )"));
        awrtc_mbedtls_ssl_set_inbound_transform(
            ssl, ssl->handshake->transform_earlydata);
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_END_OF_EARLY_DATA);
        return 0;
    }
#endif /* AWRTC_MBEDTLS_SSL_EARLY_DATA */
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(
        1, ("Switch to handshake keys for inbound traffic "
            "( K_recv = handshake )"));
    awrtc_mbedtls_ssl_set_inbound_transform(ssl, ssl->handshake->transform_handshake);
...
#if defined(AWRTC_MBEDTLS_SSL_EARLY_DATA)
/*
 * Handler for AWRTC_MBEDTLS_SSL_END_OF_EARLY_DATA
 */
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_end_of_early_data_coordinate(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (ssl->in_msgtype == AWRTC_MBEDTLS_SSL_MSG_HANDSHAKE        &&
        ssl->in_msg[0]  == AWRTC_MBEDTLS_SSL_HS_END_OF_EARLY_DATA) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("Received an end_of_early_data message."));
        return SSL_GOT_END_OF_EARLY_DATA;
    }

    if (ssl->in_msgtype == AWRTC_MBEDTLS_SSL_MSG_APPLICATION_DATA) {
        if (ssl->in_offt == NULL) {
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("Received early data"));
            /* Set the reading pointer */
            ssl->in_offt = ssl->in_msg;
            ret = awrtc_mbedtls_ssl_tls13_check_early_data_len(ssl, ssl->in_msglen);
            if (ret != 0) {
                return ret;
            }
        }
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_end_of_early_data(awrtc_mbedtls_ssl_context *ssl,
                                             const unsigned char *buf,
                                             const unsigned char *end)
{
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_process_end_of_early_data(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> ssl_tls13_process_end_of_early_data"));

    AWRTC_MBEDTLS_SSL_PROC_CHK_NEG(ssl_tls13_end_of_early_data_coordinate(ssl));
    ...
    if (ret == SSL_GOT_END_OF_EARLY_DATA) {
        ...
        AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_tls13_fetch_handshake_msg(
                                 ssl, AWRTC_MBEDTLS_SSL_HS_END_OF_EARLY_DATA,
                                 &buf, &buf_len));

        AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_parse_end_of_early_data(
                                 ssl, buf, buf + buf_len));
        ...
        ret = AWRTC_MBEDTLS_ERR_SSL_RECEIVED_EARLY_DATA;
        goto cleanup;
    } else {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
        ret = AWRTC_MBEDTLS_ERR_SSL_INTERNAL_ERROR;
...
/*
 * Handler for AWRTC_MBEDTLS_SSL_CLIENT_FINISHED
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_process_client_finished(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    ret = awrtc_mbedtls_ssl_tls13_process_finished_message(ssl);
    if (ret != 0) {
        return ret;
    }

    ret = awrtc_mbedtls_ssl_tls13_compute_resumption_master_secret(ssl);
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(
            1, "awrtc_mbedtls_ssl_tls13_compute_resumption_master_secret", ret);
    }
...
/*
 * Handler for AWRTC_MBEDTLS_SSL_HANDSHAKE_WRAPUP
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_handshake_wrapup(awrtc_mbedtls_ssl_context *ssl)
{
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("handshake: done"));

    awrtc_mbedtls_ssl_tls13_handshake_wrapup(ssl);
 ...
 */
    ...
    if (awrtc_mbedtls_ssl_tls13_is_some_psk_supported(ssl)) {
        awrtc_mbedtls_ssl_handshake_set_state(
            ssl, AWRTC_MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET);
    } else
#endif
    {
...
#if defined(AWRTC_MBEDTLS_SSL_SESSION_TICKETS)
/*
 * Handler for AWRTC_MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET
 */
#define SSL_NEW_SESSION_TICKET_SKIP  0
#define SSL_NEW_SESSION_TICKET_WRITE 1
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_new_session_ticket_coordinate(awrtc_mbedtls_ssl_context *ssl)
{
    /* Check whether the use of session tickets is enabled */
    if (ssl->conf->f_ticket_write == NULL) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("NewSessionTicket: disabled,"
                                  " callback is not set"));
    ...
    if (ssl->handshake->new_session_tickets_count == 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("NewSessionTicket: all tickets have "
                                  "been sent."));
        return SSL_NEW_SESSION_TICKET_SKIP;
    }
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_prepare_new_session_ticket(awrtc_mbedtls_ssl_context *ssl,
                                                unsigned char *ticket_nonce,
                                                size_t ticket_nonce_size)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    awrtc_mbedtls_ssl_session *session = ssl->session;
    awrtc_mbedtls_ssl_ciphersuite_t *ciphersuite_info;
    awrtc_psa_algorithm_t awrtc_psa_hash_alg;
    int hash_length;

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> prepare NewSessionTicket msg"));

    /* Set ticket_flags depends on the advertised psk key exchange mode */
    awrtc_mbedtls_ssl_tls13_session_clear_ticket_flags(
        session, AWRTC_MBEDTLS_SSL_TLS1_3_TICKET_FLAGS_MASK);
#if defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
    awrtc_mbedtls_ssl_tls13_session_set_ticket_flags(
        session, ssl->handshake->tls13_kex_modes);
#endif

#if defined(AWRTC_MBEDTLS_SSL_EARLY_DATA)
    if (ssl->conf->early_data_enabled == AWRTC_MBEDTLS_SSL_EARLY_DATA_ENABLED &&
    ...
    AWRTC_MBEDTLS_SSL_PRINT_TICKET_FLAGS(4, session->ticket_flags);

#if defined(AWRTC_MBEDTLS_SSL_EARLY_DATA) && defined(AWRTC_MBEDTLS_SSL_ALPN)
    if (session->ticket_alpn == NULL) {
        ret = awrtc_mbedtls_ssl_session_set_ticket_alpn(session, ssl->alpn_chosen);
        if (ret != 0) {
            return ret;
        }
    }
    ...
    if ((ret = ssl->conf->f_rng(ssl->conf->p_rng,
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "generate_ticket_age_add", ret);
        return ret;
    }
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("ticket_age_add: %u",
                              (unsigned int) session->ticket_age_add));

    /* Generate ticket_nonce */
    ret = ssl->conf->f_rng(ssl->conf->p_rng, ticket_nonce, ticket_nonce_size);
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "generate_ticket_nonce", ret);
        return ret;
    }
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "ticket_nonce:",
                          ticket_nonce, ticket_nonce_size);

    ciphersuite_info =
        (awrtc_mbedtls_ssl_ciphersuite_t *) ssl->handshake->ciphersuite_info;
    awrtc_psa_hash_alg = awrtc_mbedtls_md_psa_alg_from_type((awrtc_mbedtls_md_type_t) ciphersuite_info->mac);
    hash_length = AWRTC_PSA_HASH_LENGTH(awrtc_psa_hash_alg);
    if (hash_length == -1 ||
    ...
    ret = awrtc_mbedtls_ssl_tls13_hkdf_expand_label(
        ...
        hash_length,
        AWRTC_MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(resumption),
        ticket_nonce,
        ticket_nonce_size,
        session->resumption_key,
        hash_length);
    ...
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(2,
                              "Creating the ticket-resumed PSK failed",
                              ret);
        return ret;
    }
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "Ticket-resumed PSK",
                          session->resumption_key,
                          session->resumption_key_len);

    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "resumption_master_secret",
                          session->app_secrets.resumption_master_secret,
                          hash_length);
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_new_session_ticket_body(awrtc_mbedtls_ssl_context *ssl,
                                                   unsigned char *buf,
                                                   unsigned char *end,
                                                   size_t *out_len,
                                                   unsigned char *ticket_nonce,
                                                   size_t ticket_nonce_size)
{
    ...
    *out_len = 0;
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> write NewSessionTicket msg"));
     ...
     *    ticket            >=2 bytes
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 4 + 4 + 1 + ticket_nonce_size + 2);

    /* Generate ticket and ticket_lifetime */
#if defined(AWRTC_MBEDTLS_HAVE_TIME)
    session->ticket_creation_time = awrtc_mbedtls_ms_time();
#endif
    ret = ssl->conf->f_ticket_write(ssl->conf->p_ticket,
                                    ...
                                    &ticket_lifetime);
    if (ret != 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "write_ticket", ret);
        return ret;
    }
     ...
     */
    if (ticket_lifetime > AWRTC_MBEDTLS_SSL_TLS1_3_MAX_ALLOWED_TICKET_LIFETIME) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(
    ...
    AWRTC_MBEDTLS_PUT_UINT32_BE(ticket_lifetime, p, 0);
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("ticket_lifetime: %u",
                              (unsigned int) ticket_lifetime));

    /* Write ticket_age_add */
    AWRTC_MBEDTLS_PUT_UINT32_BE(session->ticket_age_add, p, 4);
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("ticket_age_add: %u",
                              (unsigned int) session->ticket_age_add));
    ...
    /* Write ticket */
    AWRTC_MBEDTLS_PUT_UINT16_BE(ticket_len, p, 0);
    p += 2;
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(4, "ticket", p, ticket_len);
    p += ticket_len;
...
#if defined(AWRTC_MBEDTLS_SSL_EARLY_DATA)
    if (awrtc_mbedtls_ssl_tls13_session_ticket_allow_early_data(session)) {
        size_t output_len;

        if ((ret = awrtc_mbedtls_ssl_tls13_write_early_data_ext(
                 ssl, 1, p, end, &output_len)) != 0) {
            AWRTC_MBEDTLS_SSL_DEBUG_RET(
                1, "awrtc_mbedtls_ssl_tls13_write_early_data_ext", ret);
            return ret;
        }
        p += output_len;
    } else {
    ...
    *out_len = p - buf;
    AWRTC_MBEDTLS_SSL_DEBUG_BUF(4, "ticket", buf, *out_len);
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("<= write new session ticket"));

    AWRTC_MBEDTLS_SSL_PRINT_EXTS(
        3, AWRTC_MBEDTLS_SSL_HS_NEW_SESSION_TICKET, ssl->handshake->sent_extensions);
 ...
 * Handler for AWRTC_MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET
 */
static int ssl_tls13_write_new_session_ticket(awrtc_mbedtls_ssl_context *ssl)
{
    int ret = AWRTC_MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    AWRTC_MBEDTLS_SSL_PROC_CHK_NEG(ssl_tls13_write_new_session_ticket_coordinate(ssl));

    if (ret == SSL_NEW_SESSION_TICKET_WRITE) {
        unsigned char ticket_nonce[AWRTC_MBEDTLS_SSL_TLS1_3_TICKET_NONCE_LENGTH];
        unsigned char *buf;
        size_t buf_len, msg_len;

        AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_prepare_new_session_ticket(
                                 ssl, ticket_nonce, sizeof(ticket_nonce)));

        AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_start_handshake_msg(
                                 ssl, AWRTC_MBEDTLS_SSL_HS_NEW_SESSION_TICKET,
                                 &buf, &buf_len));

        AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_new_session_ticket_body(
                                 ssl, buf, buf + buf_len, &msg_len,
         ...
         *
         * See document of awrtc_mbedtls_ssl_conf_new_session_tickets.
         */
        ...
        awrtc_mbedtls_ssl_handshake_set_state(
            ssl, AWRTC_MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET_FLUSH);
    } else {
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_HANDSHAKE_OVER);
    }
...
int awrtc_mbedtls_ssl_tls13_handshake_server_step(awrtc_mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->state == AWRTC_MBEDTLS_SSL_HANDSHAKE_OVER || ssl->handshake == NULL) {
        return AWRTC_MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }

    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("tls13 server state: %s(%d)",
    ...
    switch (ssl->state) {
        ...
        case AWRTC_MBEDTLS_SSL_CLIENT_HELLO:
            ...
            if (ret != 0) {
                AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_tls13_process_client_hello", ret);
            }
            break;
        ...
        case AWRTC_MBEDTLS_SSL_HELLO_RETRY_REQUEST:
            ...
            if (ret != 0) {
                AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_tls13_write_hello_retry_request", ret);
                return ret;
            }
            break;
        ...
        case AWRTC_MBEDTLS_SSL_ENCRYPTED_EXTENSIONS:
            ...
            if (ret != 0) {
                AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "ssl_tls13_write_encrypted_extensions", ret);
                return ret;
            }
            break;
        ...
        case AWRTC_MBEDTLS_SSL_SERVER_CCS_AFTER_HELLO_RETRY_REQUEST:
            ret = awrtc_mbedtls_ssl_tls13_write_change_cipher_spec(ssl);
            if (ret == 0) {
                awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_CLIENT_HELLO);
            }
            break;
        ...
        case AWRTC_MBEDTLS_SSL_CLIENT_CERTIFICATE:
            ret = awrtc_mbedtls_ssl_tls13_process_certificate(ssl);
            if (ret == 0) {
                if (ssl->session_negotiate->peer_cert != NULL) {
                    awrtc_mbedtls_ssl_handshake_set_state(
                        ssl, AWRTC_MBEDTLS_SSL_CLIENT_CERTIFICATE_VERIFY);
                } else {
                    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("skip parse certificate verify"));
                    awrtc_mbedtls_ssl_handshake_set_state(
                        ssl, AWRTC_MBEDTLS_SSL_CLIENT_FINISHED);
                }
            }
            break;

        case AWRTC_MBEDTLS_SSL_CLIENT_CERTIFICATE_VERIFY:
            ret = awrtc_mbedtls_ssl_tls13_process_certificate_verify(ssl);
            if (ret == 0) {
                awrtc_mbedtls_ssl_handshake_set_state(
                    ssl, AWRTC_MBEDTLS_SSL_CLIENT_FINISHED);
            }
        ...
        case AWRTC_MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET_FLUSH:
             ...
             * Ticket message written in AWRTC_MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET
...
#endif /* AWRTC_MBEDTLS_SSL_SRV_C && AWRTC_MBEDTLS_SSL_PROTO_TLS1_3 */
