...
{
    memset(ctx, 0, sizeof(awrtc_mbedtls_des_context));
}
...
void awrtc_mbedtls_des_free(awrtc_mbedtls_des_context *ctx)
{
    ...
    awrtc_mbedtls_platform_zeroize(ctx, sizeof(awrtc_mbedtls_des_context));
}

void awrtc_mbedtls_des3_init(awrtc_mbedtls_des3_context *ctx)
{
    memset(ctx, 0, sizeof(awrtc_mbedtls_des3_context));
}
...
void awrtc_mbedtls_des3_free(awrtc_mbedtls_des3_context *ctx)
{
    ...
    awrtc_mbedtls_platform_zeroize(ctx, sizeof(awrtc_mbedtls_des3_context));
}
...
void awrtc_mbedtls_des_key_set_parity(unsigned char key[AWRTC_MBEDTLS_DES_KEY_SIZE])
{
    ...
    for (i = 0; i < AWRTC_MBEDTLS_DES_KEY_SIZE; i++) {
...
static const unsigned char weak_key_table[WEAK_KEY_COUNT][AWRTC_MBEDTLS_DES_KEY_SIZE] =
{
...
int awrtc_mbedtls_des_key_check_weak(const unsigned char key[AWRTC_MBEDTLS_DES_KEY_SIZE])
{
    ...
    for (i = 0; i < WEAK_KEY_COUNT; i++) {
        if (memcmp(weak_key_table[i], key, AWRTC_MBEDTLS_DES_KEY_SIZE) == 0) {
            return 1;
        }
    }
...
void awrtc_mbedtls_des_setkey(uint32_t SK[32], const unsigned char key[AWRTC_MBEDTLS_DES_KEY_SIZE])
{
    int i;
    uint32_t X, Y, T;

    X = AWRTC_MBEDTLS_GET_UINT32_BE(key, 0);
    Y = AWRTC_MBEDTLS_GET_UINT32_BE(key, 4);
...
{
    awrtc_mbedtls_des_setkey(ctx->sk, key);
                         ...
                         const unsigned char key[AWRTC_MBEDTLS_DES_KEY_SIZE*2])
{
    ...
    awrtc_mbedtls_des_setkey(esk, key);
    awrtc_mbedtls_des_setkey(dsk + 32, key + 8);
                             ...
                             const unsigned char key[AWRTC_MBEDTLS_DES_KEY_SIZE * 2])
{
    ...
    awrtc_mbedtls_platform_zeroize(sk,  sizeof(sk));
                         ...
                         const unsigned char key[24])
{
    ...
    awrtc_mbedtls_des_setkey(esk, key);
    awrtc_mbedtls_des_setkey(dsk + 32, key +  8);
    awrtc_mbedtls_des_setkey(esk + 64, key + 16);
                          ...
                          unsigned char output[8])
{
    ...
    X = AWRTC_MBEDTLS_GET_UINT32_BE(input, 0);
    Y = AWRTC_MBEDTLS_GET_UINT32_BE(input, 4);
    ...
    AWRTC_MBEDTLS_PUT_UINT32_BE(Y, output, 0);
    AWRTC_MBEDTLS_PUT_UINT32_BE(X, output, 4);
                          ...
                          unsigned char *output)
{
    ...
    if (length % 8) {
        return AWRTC_MBEDTLS_ERR_DES_INVALID_INPUT_LENGTH;
    }

    if (mode == AWRTC_MBEDTLS_DES_ENCRYPT) {
        while (length > 0) {
            awrtc_mbedtls_xor(output, input, iv, 8);

            ret = awrtc_mbedtls_des_crypt_ecb(ctx, output, output);
            if (ret != 0) {
                goto exit;
            }
            memcpy(iv, output, 8);
    ...
    } else { /* AWRTC_MBEDTLS_DES_DECRYPT */
        while (length > 0) {
            memcpy(temp, input, 8);
            ret = awrtc_mbedtls_des_crypt_ecb(ctx, input, output);
            if (ret != 0) {
                goto exit;
            }

            awrtc_mbedtls_xor(output, output, iv, 8);
                           ...
                           unsigned char *output)
{
    ...
    if (mode == AWRTC_MBEDTLS_DES_ENCRYPT) {
        while (length > 0) {
            ...
            ret = awrtc_mbedtls_des3_crypt_ecb(ctx, output, output);
            if (ret != 0) {
                goto exit;
            }
            memcpy(iv, output, 8);
        ...
        while (length > 0) {
            memcpy(temp, input, 8);
            ret = awrtc_mbedtls_des3_crypt_ecb(ctx, input, output);
            if (ret != 0) {
                goto exit;
            }
...
{
    int i, j, u, v, ret = 0;
    awrtc_mbedtls_des_context ctx;
    awrtc_mbedtls_des3_context ctx3;
    unsigned char buf[8];
#if defined(AWRTC_MBEDTLS_CIPHER_MODE_CBC)
    unsigned char prv[8];
    unsigned char iv[8];
    ...
    awrtc_mbedtls_des_init(&ctx);
    awrtc_mbedtls_des3_init(&ctx3);
    /*
     * ECB mode
     */
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0) {
            awrtc_mbedtls_printf("  DES%c-ECB-%3d (%s): ",
                           (u == 0) ? ' ' : '3', 56 + u * 56,
                           (v == AWRTC_MBEDTLS_DES_DECRYPT) ? "dec" : "enc");
        }
        ...
        switch (i) {
            case 0:
                ret = awrtc_mbedtls_des_setkey_dec(&ctx, des3_test_keys);
                break;

            case 1:
                ret = awrtc_mbedtls_des_setkey_enc(&ctx, des3_test_keys);
                break;

            case 2:
                ret = awrtc_mbedtls_des3_set2key_dec(&ctx3, des3_test_keys);
                break;

            case 3:
                ret = awrtc_mbedtls_des3_set2key_enc(&ctx3, des3_test_keys);
                break;

            case 4:
                ret = awrtc_mbedtls_des3_set3key_dec(&ctx3, des3_test_keys);
                break;

            case 5:
                ret = awrtc_mbedtls_des3_set3key_enc(&ctx3, des3_test_keys);
                break;
        ...
        for (j = 0; j < 100; j++) {
            if (u == 0) {
                ret = awrtc_mbedtls_des_crypt_ecb(&ctx, buf, buf);
            } else {
                ret = awrtc_mbedtls_des3_crypt_ecb(&ctx3, buf, buf);
            }
            if (ret != 0) {
                goto exit;
            }
        ...
        if ((v == AWRTC_MBEDTLS_DES_DECRYPT &&
             memcmp(buf, des3_test_ecb_dec[u], 8) != 0) ||
            (v != AWRTC_MBEDTLS_DES_DECRYPT &&
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0) {
            awrtc_mbedtls_printf("  DES%c-CBC-%3d (%s): ",
                           (u == 0) ? ' ' : '3', 56 + u * 56,
                           (v == AWRTC_MBEDTLS_DES_DECRYPT) ? "dec" : "enc");
        }
        ...
        if (v == AWRTC_MBEDTLS_DES_DECRYPT) {
            for (j = 0; j < 100; j++) {
                if (u == 0) {
                    ret = awrtc_mbedtls_des_crypt_cbc(&ctx, v, 8, iv, buf, buf);
                } else {
                    ret = awrtc_mbedtls_des3_crypt_cbc(&ctx3, v, 8, iv, buf, buf);
                }
                if (ret != 0) {
                    goto exit;
                }
...
exit:
    awrtc_mbedtls_des_free(&ctx);
    awrtc_mbedtls_des3_free(&ctx3);
