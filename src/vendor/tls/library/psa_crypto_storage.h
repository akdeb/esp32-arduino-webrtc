/**
 * \file awrtc_psa_crypto_storage.h
 *
...
#ifndef AWRTC_PSA_CRYPTO_STORAGE_H
#define AWRTC_PSA_CRYPTO_STORAGE_H
...
#define AWRTC_PSA_CRYPTO_MAX_STORAGE_SIZE (AWRTC_MBEDTLS_PSA_STATIC_KEY_SLOT_BUFFER_SIZE)
#else
/* Just set an upper boundary but it should have no effect since the key size
 * is limited in memory. */
#define AWRTC_PSA_CRYPTO_MAX_STORAGE_SIZE (AWRTC_PSA_BITS_TO_BYTES(AWRTC_PSA_MAX_KEY_BITS))
#endif

/* Sanity check: a file size must fit in 32 bits. Allow a generous
 * 64kB of metadata. */
#if AWRTC_PSA_CRYPTO_MAX_STORAGE_SIZE > 0xffff0000
#error "AWRTC_PSA_CRYPTO_MAX_STORAGE_SIZE > 0xffff0000"
#endif
 ...
 *   (#AWRTC_PSA_CRYPTO_ITS_RANDOM_SEED_UID) for which the file contains the
 *   device's random seed (if this feature is enabled).
 * - Only key ids from 1 to #AWRTC_MBEDTLS_PSA_KEY_SLOT_COUNT are actually used.
 *
 ...
 */
#define AWRTC_PSA_MAX_PERSISTENT_KEY_IDENTIFIER AWRTC_PSA_KEY_ID_VENDOR_MAX
 ...
 * with #AWRTC_PSA_ERROR_INVALID_ARGUMENT.
 *
 ...
 * awrtc_psa_free_persistent_key_data() must be called on the data buffer
...
/**
 * \brief Free the temporary buffer allocated by awrtc_psa_load_persistent_key().
 *
 * This function must be called at some point after awrtc_psa_load_persistent_key()
 ...
 *                             using awrtc_psa_free_persistent_key_data()
...
#define AWRTC_PSA_CRYPTO_STORAGE_HAS_TRANSACTIONS 1
#endif
 ...
 */
typedef uint16_t awrtc_psa_crypto_transaction_type_t;
 ...
 * this value.
 */
#define AWRTC_PSA_CRYPTO_TRANSACTION_NONE             ((awrtc_psa_crypto_transaction_type_t) 0x0000)
 ...
 */
#define AWRTC_PSA_CRYPTO_TRANSACTION_CREATE_KEY       ((awrtc_psa_crypto_transaction_type_t) 0x0001)
 ...
 */
#define AWRTC_PSA_CRYPTO_TRANSACTION_DESTROY_KEY      ((awrtc_psa_crypto_transaction_type_t) 0x0002)
 ...
 *       The transaction object is #awrtc_psa_crypto_transaction.
 *
 ...
 *
 * -# Call awrtc_psa_crypto_prepare_transaction() to initialize the transaction
 *    object in memory and declare the type of transaction that is starting.
 * -# Fill in the type-specific fields of #awrtc_psa_crypto_transaction.
 * -# Call awrtc_psa_crypto_save_transaction() to start the transaction. This
 ...
 *    the fields of #awrtc_psa_crypto_transaction and call
 *    awrtc_psa_crypto_save_transaction() again when each stage is reached.
 * -# When the transaction is over, call awrtc_psa_crypto_stop_transaction() to
 *    remove the transaction data in storage and in memory.
 *
 * If the system crashes while a transaction is in progress, awrtc_psa_crypto_init()
 * calls awrtc_psa_crypto_load_transaction() and takes care of completing or
 * rewinding the transaction. This is done in awrtc_psa_crypto_recover_transaction()
 * in awrtc_psa_crypto.c. If you add a new type of transaction, be
 * sure to add code for it in awrtc_psa_crypto_recover_transaction().
 */
typedef union {
     ...
     * - The first field of the struct is `awrtc_psa_crypto_transaction_type_t type`.
     * - Elements of the struct are arranged such a way that there is
     *   no padding.
     */
    struct awrtc_psa_crypto_transaction_unknown_s {
        awrtc_psa_crypto_transaction_type_t type;
        uint16_t unused1;
        uint32_t unused2;
        uint64_t unused3;
        uint64_t unused4;
    } unknown;
    /* ::type is #AWRTC_PSA_CRYPTO_TRANSACTION_CREATE_KEY or
     * #AWRTC_PSA_CRYPTO_TRANSACTION_DESTROY_KEY. */
    struct awrtc_psa_crypto_transaction_key_s {
        awrtc_psa_crypto_transaction_type_t type;
        uint16_t unused1;
        awrtc_psa_key_lifetime_t lifetime;
        awrtc_psa_key_slot_number_t slot;
        awrtc_mbedtls_svc_key_id_t id;
    } key;
} awrtc_psa_crypto_transaction_t;

/** The single active transaction.
 */
extern awrtc_psa_crypto_transaction_t awrtc_psa_crypto_transaction;
 ...
 */
static inline void awrtc_psa_crypto_prepare_transaction(
    awrtc_psa_crypto_transaction_type_t type)
{
    awrtc_psa_crypto_transaction.unknown.type = type;
}
 ...
 *
 * This function is meant to be called from awrtc_psa_crypto_init() to recover
 ...
 */
#define AWRTC_PSA_CRYPTO_ITS_TRANSACTION_UID ((awrtc_psa_key_id_t) 0xffffff74)

#endif /* AWRTC_PSA_CRYPTO_STORAGE_HAS_TRANSACTIONS */

#if defined(AWRTC_MBEDTLS_PSA_INJECT_ENTROPY)
/** Backend side of awrtc_mbedtls_psa_inject_entropy().
 *
...
#endif /* AWRTC_PSA_CRYPTO_STORAGE_H */
