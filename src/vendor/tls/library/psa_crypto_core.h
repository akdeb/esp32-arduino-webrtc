...
#ifndef AWRTC_PSA_CRYPTO_CORE_H
#define AWRTC_PSA_CRYPTO_CORE_H
...
typedef struct {
     ...
     * awrtc_psa_key_slot_state_transition.
     *
     * The state variable is used to help determine whether library functions
     * which operate on the slot succeed. For example, awrtc_psa_finish_key_creation,
     * which transfers the state of a slot from AWRTC_PSA_SLOT_FILLING to
     * AWRTC_PSA_SLOT_FULL, must fail with error code AWRTC_PSA_ERROR_CORRUPTION_DETECTED
     * if the state of the slot is not AWRTC_PSA_SLOT_FILLING.
     *
     * Library functions which traverse the array of key slots only consider
     * slots that are in a suitable state for the function.
     * For example, awrtc_psa_get_and_lock_key_slot_in_memory, which finds a slot
     * containing a given key ID, will only check slots whose state variable is
     * AWRTC_PSA_SLOT_FULL.
     */
    awrtc_psa_key_slot_state_t state;
    ...
    union {
        ...
        struct {
             ...
             *
             * A function must call awrtc_psa_register_read(slot) before reading
             * the current contents of the slot for an operation.
             * They then must call awrtc_psa_unregister_read(slot) once they have
             ...
             * A function must call awrtc_psa_key_slot_has_readers(slot) to check if
     ...
     * Format as specified in awrtc_psa_export_key(). */
    struct key_data {
#if defined(AWRTC_MBEDTLS_PSA_STATIC_KEY_SLOTS)
        uint8_t data[AWRTC_MBEDTLS_PSA_STATIC_KEY_SLOT_BUFFER_SIZE];
#else
        uint8_t *data;
#endif
        size_t bytes;
    } key;
} awrtc_psa_key_slot_t;

#if defined(AWRTC_MBEDTLS_THREADING_C)

/** Perform a mutex operation and return immediately upon failure.
 *
 * Returns AWRTC_PSA_ERROR_SERVICE_FAILURE if the operation fails
 * and status was AWRTC_PSA_SUCCESS.
 *
 * Assumptions:
 *  awrtc_psa_status_t status exists.
 *  f is a mutex operation which returns 0 upon success.
 */
#define AWRTC_PSA_THREADING_CHK_RET(f)                       \
    do                                                 \
    {                                                  \
        if ((f) != 0) {                                \
 ...
 *
 * Sets status to AWRTC_PSA_ERROR_SERVICE_FAILURE if status was AWRTC_PSA_SUCCESS.
 *
 * Assumptions:
 *  awrtc_psa_status_t status exists.
 *  Label exit: exists.
 *  f is a mutex operation which returns 0 upon success.
 */
#define AWRTC_PSA_THREADING_CHK_GOTO_EXIT(f)                 \
    do                                                 \
    {                                                  \
        if ((f) != 0) {                                \
            if (status == AWRTC_PSA_SUCCESS) {               \
                status = AWRTC_PSA_ERROR_SERVICE_FAILURE;    \
            }                                          \
            goto exit;                                 \
        }                                              \
    } while (0);
 ...
 */
static inline int awrtc_psa_key_slot_has_readers(const awrtc_psa_key_slot_t *slot)
{
 ...
 */
static inline awrtc_psa_key_slot_number_t awrtc_psa_key_slot_get_slot_number(
    const awrtc_psa_key_slot_t *slot)
{
    return *((awrtc_psa_key_slot_number_t *) (slot->key.data));
}
#endif
 ...
 * Sets the slot's state to AWRTC_PSA_SLOT_EMPTY.
 * If multi-threading is enabled, the caller must hold the
 * global key slot mutex.
 *
 ...
 *         The slot's state was AWRTC_PSA_SLOT_FULL or AWRTC_PSA_SLOT_PENDING_DELETION, and
 *         the amount of registered readers was not equal to 1. Or,
 *         the slot's state was AWRTC_PSA_SLOT_EMPTY. Or,
 *         the slot's state was AWRTC_PSA_SLOT_FILLING, and the amount
 ...
 *
 * \retval #AWRTC_PSA_SUCCESS  The key was imported successfully.
 ...
 *                              \p data
 *
 * \retval #AWRTC_PSA_SUCCESS  The key was exported successfully.
 * \retval #AWRTC_PSA_ERROR_NOT_SUPPORTED \emptydescription
 ...
 *                              \p data
 *
 * \retval #AWRTC_PSA_SUCCESS  The public key was exported successfully.
 * \retval #AWRTC_PSA_ERROR_NOT_SUPPORTED \emptydescription
 * \retval #AWRTC_PSA_ERROR_COMMUNICATION_FAILURE \emptydescription
 * \retval #AWRTC_PSA_ERROR_HARDWARE_FAILURE \emptydescription
 * \retval #AWRTC_PSA_ERROR_CORRUPTION_DETECTED \emptydescription
 ...
 */
awrtc_psa_status_t awrtc_psa_generate_key_internal(const awrtc_psa_key_attributes_t *attributes,
                                       const awrtc_psa_custom_key_parameters_t *custom,
 ...
 *       transparent drivers.
 *
 * \note This function will call the driver for awrtc_psa_sign_hash
...
awrtc_psa_status_t awrtc_psa_sign_message_builtin(
    const awrtc_psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    awrtc_psa_algorithm_t alg, const uint8_t *input, size_t input_length,
 ...
 *       transparent drivers.
 *
 * \note This function will call the driver for awrtc_psa_verify_hash
 *       and go through driver dispatch again.
 *
 * \param[in]  attributes       The attributes of the key to use for the
 *                              operation.
 ...
 */
awrtc_psa_status_t awrtc_psa_key_agreement_raw_builtin(
    const awrtc_psa_key_attributes_t *attributes,
 ...
 */
uint32_t awrtc_mbedtls_psa_interruptible_get_max_ops(void);
 ...
 *                              awrtc_mbedtls_psa_sign_hash_complete().
 */
 ...
 * \return                      Number of ops that were completed
 *                              in the last call to \c
 *                              awrtc_mbedtls_psa_verify_hash_complete().
 */
 ...
 *         used.
 * \retval #AWRTC_PSA_ERROR_NOT_SUPPORTED Either no internal interruptible operations
...
typedef struct awrtc_psa_crypto_local_input_s {
    uint8_t *buffer;
    size_t length;
} awrtc_psa_crypto_local_input_t;

#define AWRTC_PSA_CRYPTO_LOCAL_INPUT_INIT ((awrtc_psa_crypto_local_input_t) { NULL, 0 })
 ...
 * \param[out] local_input      Pointer to a awrtc_psa_crypto_local_input_t struct
 *                              containing a local input copy.
 * \return                      #AWRTC_PSA_SUCCESS, if the buffer was successfully
 *                              copied.
 * \return                      #AWRTC_PSA_ERROR_INSUFFICIENT_MEMORY, if a copy of
 ...
 *
 * \param[in] local_input       Pointer to a awrtc_psa_crypto_local_input_t struct
 *                              populated by a previous call to
 *                              awrtc_psa_crypto_local_input_alloc().
 */
void awrtc_psa_crypto_local_input_free(awrtc_psa_crypto_local_input_t *local_input);

typedef struct awrtc_psa_crypto_local_output_s {
    uint8_t *original;
    uint8_t *buffer;
    size_t length;
} awrtc_psa_crypto_local_output_t;

#define AWRTC_PSA_CRYPTO_LOCAL_OUTPUT_INIT ((awrtc_psa_crypto_local_output_t) { NULL, NULL, 0 })
 ...
 *                              awrtc_psa_crypto_local_output_free().
 *
 * \param[in] output            Pointer to output buffer.
 * \param[in] output_len        Length of the output buffer.
 * \param[out] local_output     Pointer to a awrtc_psa_crypto_local_output_t struct to
 ...
 *  free the local copy.
 *
 * \param[in] local_output      Pointer to a awrtc_psa_crypto_local_output_t struct
 *                              populated by a previous call to
 *                              awrtc_psa_crypto_local_output_alloc().
 * \return                      #AWRTC_PSA_SUCCESS, if the local output was
 *                              successfully copied back to the original.
 * \return                      #AWRTC_PSA_ERROR_CORRUPTION_DETECTED, if the output
