...
#ifndef AWRTC_MBEDTLS_CONSTANT_TIME_INTERNAL_H
#define AWRTC_MBEDTLS_CONSTANT_TIME_INTERNAL_H
 ...
 *
 * - boolean operations
 *   These are all named awrtc_mbedtls_ct_<type>_<operation>.
 *   They operate over <type> and return awrtc_mbedtls_ct_condition_t.
 *   All arguments are considered secret.
 *   example: bool x = y | z          =>    x = awrtc_mbedtls_ct_bool_or(y, z)
 *   example: bool x = y == z         =>    x = awrtc_mbedtls_ct_uint_eq(y, z)
 *
 * - conditional data selection
 *   These are all named awrtc_mbedtls_ct_<type>_if and awrtc_mbedtls_ct_<type>_if_else_0
 *   All arguments are considered secret.
 *   example: size_t a = x ? b : c    =>    a = awrtc_mbedtls_ct_size_if(x, b, c)
 *   example: unsigned a = x ? b : 0  =>    a = awrtc_mbedtls_ct_uint_if_else_0(x, b)
 *
 * - block memory operations
 *   Only some arguments are considered secret, as documented for each
 *   function.
 *   example: if (x) memcpy(...)      =>    awrtc_mbedtls_ct_memcpy_if(x, ...)
 *
 * awrtc_mbedtls_ct_condition_t must be treated as opaque and only created and
 ...
 *
 * awrtc_mbedtls_ct_uint_t is an unsigned integer type over which constant time
 * operations may be performed via the functions in this header. It is as big
 * as the larger of size_t and awrtc_mbedtls_mpi_uint, i.e. it is safe to cast
 * to/from "unsigned int", "size_t", and "awrtc_mbedtls_mpi_uint" (and any other
...
/* Pointer size > 64-bit */
typedef size_t    awrtc_mbedtls_ct_condition_t;
typedef size_t    awrtc_mbedtls_ct_uint_t;
typedef ptrdiff_t awrtc_mbedtls_ct_int_t;
#define AWRTC_MBEDTLS_CT_TRUE  ((awrtc_mbedtls_ct_condition_t) awrtc_mbedtls_ct_compiler_opaque(SIZE_MAX))
#elif (SIZE_MAX > 0xffffffff) || defined(AWRTC_MBEDTLS_HAVE_INT64)
/* 32-bit < pointer size <= 64-bit, or 64-bit MPI */
typedef uint64_t  awrtc_mbedtls_ct_condition_t;
typedef uint64_t  awrtc_mbedtls_ct_uint_t;
typedef int64_t   awrtc_mbedtls_ct_int_t;
#define AWRTC_MBEDTLS_CT_SIZE_64
#define AWRTC_MBEDTLS_CT_TRUE  ((awrtc_mbedtls_ct_condition_t) awrtc_mbedtls_ct_compiler_opaque(UINT64_MAX))
#else
/* Pointer size <= 32-bit, and no 64-bit MPIs */
typedef uint32_t  awrtc_mbedtls_ct_condition_t;
typedef uint32_t  awrtc_mbedtls_ct_uint_t;
typedef int32_t   awrtc_mbedtls_ct_int_t;
#define AWRTC_MBEDTLS_CT_SIZE_32
#define AWRTC_MBEDTLS_CT_TRUE  ((awrtc_mbedtls_ct_condition_t) awrtc_mbedtls_ct_compiler_opaque(UINT32_MAX))
#endif
#define AWRTC_MBEDTLS_CT_FALSE ((awrtc_mbedtls_ct_condition_t) awrtc_mbedtls_ct_compiler_opaque(0))

/* ============================================================================
 * Boolean operations
 */

/** Convert a number into a awrtc_mbedtls_ct_condition_t.
 *
 * \param x Number to convert.
 *
 * \return AWRTC_MBEDTLS_CT_TRUE if \p x != 0, or AWRTC_MBEDTLS_CT_FALSE if \p x == 0
 *
 */
 ...
 *
 * \return      AWRTC_MBEDTLS_CT_TRUE if \p x != \p y, otherwise AWRTC_MBEDTLS_CT_FALSE.
 */
 ...
 * \param y     The second value to analyze.
 *
 * \return      AWRTC_MBEDTLS_CT_TRUE if \p x >= \p y,
 *              otherwise AWRTC_MBEDTLS_CT_FALSE.
 */
 ...
 * \param y     The second value to analyze.
 *
 * \note        This is more efficient than awrtc_mbedtls_ct_uint_ne if both arguments are
 *              awrtc_mbedtls_ct_condition_t.
 *
 * \return      AWRTC_MBEDTLS_CT_TRUE if \p x != \p y,
 *              otherwise AWRTC_MBEDTLS_CT_FALSE.
 */
 ...
 *
 * \return      AWRTC_MBEDTLS_CT_FALSE if \p x, otherwise AWRTC_MBEDTLS_CT_TRUE.
 */
 ...
 * \param if1           Value to use if \p condition == AWRTC_MBEDTLS_CT_TRUE.
 * \param if0           Value to use if \p condition == AWRTC_MBEDTLS_CT_FALSE.
 *
 * \return  \c if1 if \p condition == AWRTC_MBEDTLS_CT_TRUE, otherwise \c if0.
 */
...
/** Choose between two awrtc_mbedtls_ct_condition_t values.
 *
 * Functionally equivalent to:
 *
 * condition ? if1 : if0.
...
/** Choose between two awrtc_mbedtls_mpi_uint values.
 *
 * Functionally equivalent to:
 *
 * condition ? if1 : if0.
 ...
 * condition ? if1 : 0.
 *
 * Functionally equivalent to awrtc_mbedtls_ct_uint_if(condition, if1, 0) but
 * results in smaller code size.
 *
 * \param condition     Condition to test.
 * \param if1           Value to use if \p condition == AWRTC_MBEDTLS_CT_TRUE.
 *
 * \return  \c if1 if \p condition == AWRTC_MBEDTLS_CT_TRUE, otherwise 0.
 */
static inline unsigned awrtc_mbedtls_ct_uint_if_else_0(awrtc_mbedtls_ct_condition_t condition, unsigned if1);

/** Choose between an awrtc_mbedtls_ct_condition_t and 0.
 *
 * Functionally equivalent to:
 *
 * condition ? if1 : 0.
 *
 * Functionally equivalent to awrtc_mbedtls_ct_bool_if(condition, if1, 0) but
 * results in smaller code size.
 *
 * \param condition     Condition to test.
 * \param if1           Value to use if \p condition == AWRTC_MBEDTLS_CT_TRUE.
 ...
 * condition ? if1 : 0.
 *
 * Functionally equivalent to awrtc_mbedtls_ct_size_if(condition, if1, 0) but
 * results in smaller code size.
 *
 * \param condition     Condition to test.
 * \param if1           Value to use if \p condition == AWRTC_MBEDTLS_CT_TRUE.
...
/** Choose between an awrtc_mbedtls_mpi_uint value and 0.
 *
 * Functionally equivalent to:
 *
 * condition ? if1 : 0.
 *
 * Functionally equivalent to awrtc_mbedtls_ct_mpi_uint_if(condition, if1, 0) but
 * results in smaller code size.
 *
 * \param condition     Condition to test.
 * \param if1           Value to use if \p condition == AWRTC_MBEDTLS_CT_TRUE.
 ...
 * condition ? if1 : 0.
 *
 * Functionally equivalent to awrtc_mbedtls_ct_error_if(condition, if1, 0) but
 * results in smaller code size.
 *
 * \param condition     Condition to test.
 * \param if1           Value to use if \p condition == AWRTC_MBEDTLS_CT_TRUE.
 ...
 *
 * \warning Unlike awrtc_mbedtls_platform_zeroize, this does not have the same guarantees
 * about not being optimised away if the memory is never read again.
 */
void awrtc_mbedtls_ct_zeroize_if(awrtc_mbedtls_ct_condition_t condition, void *buf, size_t len);
 ...
 */
void awrtc_mbedtls_ct_memmove_left(void *start,
                             size_t total,
                             size_t offset);

#endif /* defined(AWRTC_MBEDTLS_PKCS1_V15) && defined(AWRTC_MBEDTLS_RSA_C) && !defined(AWRTC_MBEDTLS_RSA_ALT) */
 ...
 * \param src1      Secret. Pointer to copy from (if \p condition == AWRTC_MBEDTLS_CT_TRUE).
 ...
 *                  Pointer to copy from (if \p condition == AWRTC_MBEDTLS_CT_FALSE and \p src2 is not NULL). May be NULL.
 *                  This may be equal to \p dest, but may not overlap it in other ways. It may overlap with \p src1.
 * \param len       Number of bytes to copy.
 */
void awrtc_mbedtls_ct_memcpy_if(awrtc_mbedtls_ct_condition_t condition,
                          unsigned char *dest,
 ...
 */
void awrtc_mbedtls_ct_memcpy_offset(unsigned char *dest,
...
/* Documented in include/mbedtls/constant_time.h. a and b are secret.
   ...
   int awrtc_mbedtls_ct_memcmp(const void *a,
                         const void *b,
                         size_t n);
 */
 ...
 *
 * Similar to awrtc_mbedtls_ct_memcmp, except that the result only depends on part of
 ...
 */
int awrtc_mbedtls_ct_memcmp_partial(const void *a,
                              const void *b,
                              size_t n,
                              size_t skip_head,
                              size_t skip_tail);
...
#endif /* AWRTC_MBEDTLS_CONSTANT_TIME_INTERNAL_H */
