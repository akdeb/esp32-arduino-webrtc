...
{
    memset(ctx, 0, sizeof(awrtc_mbedtls_nist_kw_context));
}
                           ...
                           const int is_wrap)
{
    ...
    cipher_info = awrtc_mbedtls_cipher_info_from_values(cipher,
                                                  keybits,
     ...
     */
    if (cipher != AWRTC_MBEDTLS_CIPHER_ID_AES) {
    ...
    if ((ret = awrtc_mbedtls_cipher_setkey(&ctx->cipher_ctx, key, keybits,
                                     is_wrap ? AWRTC_MBEDTLS_ENCRYPT :
                                     AWRTC_MBEDTLS_DECRYPT)
         ) != 0) {
        return ret;
    }
...
void awrtc_mbedtls_nist_kw_free(awrtc_mbedtls_nist_kw_context *ctx)
{
    ...
    awrtc_mbedtls_cipher_free(&ctx->cipher_ctx);
    awrtc_mbedtls_platform_zeroize(ctx, sizeof(awrtc_mbedtls_nist_kw_context));
}
 ...
 */
int awrtc_mbedtls_nist_kw_wrap(awrtc_mbedtls_nist_kw_context *ctx,
                         ...
                         unsigned char *output, size_t *out_len, size_t out_size)
{
    ...
    *out_len = 0;
     ...
     */
    if (mode == AWRTC_MBEDTLS_KW_MODE_KW) {
        ...
        AWRTC_MBEDTLS_PUT_UINT32_BE((in_len & 0xffffffff), output,
    ...
    if (mode == AWRTC_MBEDTLS_KW_MODE_KWP
        && in_len <= KW_SEMIBLOCK_LENGTH) {
        memcpy(inbuff, output, 16);
        ret = awrtc_mbedtls_cipher_update(&ctx->cipher_ctx,
        ...
        if (semiblocks < MIN_SEMIBLOCKS_COUNT) {
            ret = AWRTC_MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
            goto cleanup;
        }
...
cleanup:
    ...
    awrtc_mbedtls_platform_zeroize(inbuff, KW_SEMIBLOCK_LENGTH * 2);
    awrtc_mbedtls_platform_zeroize(outbuff, KW_SEMIBLOCK_LENGTH * 2);
 ...
 */
static int unwrap(awrtc_mbedtls_nist_kw_context *ctx,
...
cleanup:
    ...
    awrtc_mbedtls_platform_zeroize(inbuff, sizeof(inbuff));
    awrtc_mbedtls_platform_zeroize(outbuff, sizeof(outbuff));
 ...
 */
int awrtc_mbedtls_nist_kw_unwrap(awrtc_mbedtls_nist_kw_context *ctx,
                           awrtc_mbedtls_nist_kw_mode_t mode,
                           const unsigned char *input, size_t in_len,
                           unsigned char *output, size_t *out_len, size_t out_size)
{
    ...
    *out_len = 0;
    ...
    if (mode == AWRTC_MBEDTLS_KW_MODE_KW) {
        ...
        diff = awrtc_mbedtls_ct_memcmp(NIST_KW_ICV1, A, KW_SEMIBLOCK_LENGTH);
    ...
    } else if (mode == AWRTC_MBEDTLS_KW_MODE_KWP) {
        size_t padlen = 0;
        uint32_t Plen;
        /*
        ...
        /* Check ICV in "constant-time" */
        diff = awrtc_mbedtls_ct_memcmp(NIST_KW_ICV2, A, KW_SEMIBLOCK_LENGTH / 2);

        if (diff != 0) {
            ret = AWRTC_MBEDTLS_ERR_CIPHER_AUTH_FAILED;
        }

        Plen = AWRTC_MBEDTLS_GET_UINT32_BE(A, KW_SEMIBLOCK_LENGTH / 2);
        ...
        ret = awrtc_mbedtls_ct_error_if(awrtc_mbedtls_ct_uint_gt(padlen, 7),
                                  AWRTC_MBEDTLS_ERR_CIPHER_AUTH_FAILED, ret);
        padlen &= 7;

        /* Check padding in "constant-time" */
        const uint8_t zero[KW_SEMIBLOCK_LENGTH] = { 0 };
        diff = awrtc_mbedtls_ct_memcmp_partial(
...
cleanup:
    ...
    awrtc_mbedtls_platform_zeroize(&diff, sizeof(diff));
    awrtc_mbedtls_platform_zeroize(A, sizeof(A));
...
{
    awrtc_mbedtls_nist_kw_context ctx;
    unsigned char out[48];
    size_t olen;
    int i;
    int ret = 0;
    awrtc_mbedtls_nist_kw_init(&ctx);
     ...
     */
    {
        ...
        for (i = 0; i < num_tests; i++) {
            if (verbose != 0) {
                awrtc_mbedtls_printf("  KW-AES-%u ", (unsigned int) key_len[i] * 8);
            }

            ret = awrtc_mbedtls_nist_kw_setkey(&ctx, AWRTC_MBEDTLS_CIPHER_ID_AES,
                                         kw_key[i], key_len[i] * 8, 1);
            if (ret != 0) {
                if (verbose != 0) {
                    awrtc_mbedtls_printf("  KW: setup failed ");
                }

                goto end;
            }

            ret = awrtc_mbedtls_nist_kw_wrap(&ctx, AWRTC_MBEDTLS_KW_MODE_KW, kw_msg[i],
            ...
            if ((ret = awrtc_mbedtls_nist_kw_setkey(&ctx, AWRTC_MBEDTLS_CIPHER_ID_AES,
            ...
            ret = awrtc_mbedtls_nist_kw_unwrap(&ctx, AWRTC_MBEDTLS_KW_MODE_KW,
     ...
     */
    {
        ...
        for (i = 0; i < num_tests; i++) {
            olen = sizeof(out);
            if (verbose != 0) {
                awrtc_mbedtls_printf("  KWP-AES-%u ", (unsigned int) key_len[i] * 8);
            }

            ret = awrtc_mbedtls_nist_kw_setkey(&ctx, AWRTC_MBEDTLS_CIPHER_ID_AES, kwp_key[i],
                                         key_len[i] * 8, 1);
            if (ret  != 0) {
                if (verbose != 0) {
                    awrtc_mbedtls_printf("  KWP: setup failed ");
                }

                goto end;
            }
            ret = awrtc_mbedtls_nist_kw_wrap(&ctx, AWRTC_MBEDTLS_KW_MODE_KWP, kwp_msg[i],
            ...
            ret = awrtc_mbedtls_nist_kw_unwrap(&ctx, AWRTC_MBEDTLS_KW_MODE_KWP, out,
