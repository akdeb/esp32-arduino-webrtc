...
#if __ARM_ARCH >= 8
#define AWRTC_MBEDTLS_AESCE_ARCH_IS_ARMV8_A
#endif
#endif

#if defined(AWRTC_MBEDTLS_AESCE_ARCH_IS_ARMV8_A) && !defined(__ARM_FEATURE_CRYPTO)
...
#   if defined(AWRTC_MBEDTLS_ARCH_IS_ARM32) && (__clang_major__ < 11)
#       error "Minimum version of Clang for AWRTC_MBEDTLS_AESCE_C on 32-bit Arm or Thumb is 11.0."
#   elif defined(AWRTC_MBEDTLS_ARCH_IS_ARM64) && (__clang_major__ < 4)
#       error "Minimum version of Clang for AWRTC_MBEDTLS_AESCE_C on aarch64 is 4.0."
#   endif
#elif defined(__GNUC__)
#   if __GNUC__ < 6
#       error "Minimum version of GCC for AWRTC_MBEDTLS_AESCE_C is 6.0."
#   endif
#elif defined(_MSC_VER)
/* TODO: We haven't verified MSVC from 1920 to 1928. If someone verified that,
 *       please update this and document of `AWRTC_MBEDTLS_AESCE_C` in
 *       `awrtc_mbedtls_config.h`. */
#   if _MSC_VER < 1929
#       error "Minimum version of MSVC for AWRTC_MBEDTLS_AESCE_C is 2019 version 16.11.2."
#   endif
#elif defined(__ARMCC_VERSION)
#    if defined(AWRTC_MBEDTLS_ARCH_IS_ARM32) && (__ARMCC_VERSION < 6200002)
 ...
 * `AWRTC_MBEDTLS_AESCE_C` in `awrtc_mbedtls_config.h`. */
#         error "Minimum version of armclang for AWRTC_MBEDTLS_AESCE_C on 32-bit Arm is 6.20."
#    elif defined(AWRTC_MBEDTLS_ARCH_IS_ARM64) && (__ARMCC_VERSION < 6060000)
#         error "Minimum version of armclang for AWRTC_MBEDTLS_AESCE_C on aarch64 is 6.6."
#    endif
#endif

#if !(defined(__ARM_FEATURE_CRYPTO) || defined(__ARM_FEATURE_AES)) || \
    defined(AWRTC_MBEDTLS_ENABLE_ARM_CRYPTO_EXTENSIONS_COMPILER_FLAG)
#   if defined(__ARMCOMPILER_VERSION)
#       if __ARMCOMPILER_VERSION <= 6090000
#           error "Must use minimum -march=armv8-a+crypto for AWRTC_MBEDTLS_AESCE_C"
#       else
...
#   endif
          ...
          AWRTC_MBEDTLS_ENABLE_ARM_CRYPTO_EXTENSIONS_COMPILER_FLAG */
...
signed char awrtc_mbedtls_aesce_has_support_result = -1;
...
int awrtc_mbedtls_aesce_has_support_impl(void)
{
     ...
     */
    if (awrtc_mbedtls_aesce_has_support_result == -1) {
        ...
        if (((auxval  & HWCAP_NEON) == HWCAP_NEON) &&
            ...
            awrtc_mbedtls_aesce_has_support_result = 1;
        } else {
            awrtc_mbedtls_aesce_has_support_result = 0;
        }
#else
        unsigned long auxval = getauxval(AT_HWCAP);
    ...
    return awrtc_mbedtls_aesce_has_support_result;
}
#endif
                             ...
                             size_t bits)
{
    switch (bits) {
        ...
        case 256:
            ...
            break;
        default:
            return AWRTC_MBEDTLS_ERR_AES_INVALID_KEY_LENGTH;
    }
...
#endif /* defined(AWRTC_MBEDTLS_ARCH_IS_ARM32) */

#if defined(AWRTC_MBEDTLS_COMPILER_IS_GCC) && __GNUC__ == 5
/* Some intrinsics are not available for GCC 5.X. */
#define COMMON_MISSING_INTRINSICS
#endif /* AWRTC_MBEDTLS_COMPILER_IS_GCC && __GNUC__ == 5 */
 ...
 */
#if defined(AWRTC_MBEDTLS_COMPILER_IS_GCC)
...
#define AWRTC_MBEDTLS_VMULL_P64(a, b) vmull_p64((poly64_t) a, (poly64_t) b)
#else
 ...
 * cast for clang also. */
#define AWRTC_MBEDTLS_VMULL_P64(a, b) vmull_p64(a, b)
#endif /* AWRTC_MBEDTLS_COMPILER_IS_GCC */

static inline uint8x16_t pmull_low(uint8x16_t a, uint8x16_t b)
{

    return vreinterpretq_u8_p128(
        AWRTC_MBEDTLS_VMULL_P64(
...
#endif /* AWRTC_MBEDTLS_AESCE_HAVE_CODE */
