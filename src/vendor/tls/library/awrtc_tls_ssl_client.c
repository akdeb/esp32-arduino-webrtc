...
#if defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_3) || defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_2)
...
static int ssl_write_hostname_ext(awrtc_mbedtls_ssl_context *ssl,
                                  unsigned char *buf,
                                  const unsigned char *end,
                                  size_t *olen)
{
    unsigned char *p = buf;
    const char *hostname = awrtc_mbedtls_ssl_get_hostname_pointer(ssl);
    size_t hostname_len;
    ...
    *olen = 0;
    ...
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, hostname_len + 9);
     ...
     *
     */
    AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_TLS_EXT_SERVERNAME, p, 0);
    p += 2;

    AWRTC_MBEDTLS_PUT_UINT16_BE(hostname_len + 5, p, 0);
    p += 2;

    AWRTC_MBEDTLS_PUT_UINT16_BE(hostname_len + 3, p, 0);
    p += 2;

    *p++ = AWRTC_MBEDTLS_BYTE_0(AWRTC_MBEDTLS_TLS_EXT_SERVERNAME_HOSTNAME);
    ...
    *olen = hostname_len + 9;
    ...
    awrtc_mbedtls_ssl_tls13_set_hs_sent_ext_mask(ssl, AWRTC_MBEDTLS_TLS_EXT_SERVERNAME);
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_alpn_ext(awrtc_mbedtls_ssl_context *ssl,
                              unsigned char *buf,
                              const unsigned char *end,
                              size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, adding alpn extension"));
    ...
    for (const char **cur = ssl->conf->alpn_list; *cur != NULL; cur++) {
        /*
         * awrtc_mbedtls_ssl_conf_set_alpn_protocols() checked that the length of
         * protocol names is less than 255.
         */
        size_t protocol_name_len = strlen(*cur);

        AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 1 + protocol_name_len);
    ...
    *out_len = (size_t) (p - buf);
    ...
    AWRTC_MBEDTLS_PUT_UINT16_BE(*out_len - 6, buf, 4);

    /* Extension length = *out_len - 2 (ext_type) - 2 (ext_len) */
    AWRTC_MBEDTLS_PUT_UINT16_BE(*out_len - 4, buf, 2);
...
#if defined(AWRTC_MBEDTLS_SSL_TLS1_2_SOME_ECC) || \
    defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
/*
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_supported_groups_ext(awrtc_mbedtls_ssl_context *ssl,
                                          unsigned char *buf,
                                          const unsigned char *end,
                                          int flags,
                                          size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, adding supported_groups extension"));
    ...
    for (; *group_list != 0; group_list++) {
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("got supported group(%04x)", *group_list));
        ...
        if (flags & SSL_WRITE_SUPPORTED_GROUPS_EXT_TLS1_3_FLAG) {
            ...
            if (awrtc_mbedtls_ssl_tls13_named_group_is_ecdhe(*group_list) &&
                (awrtc_mbedtls_ssl_get_ecp_group_id_from_tls_id(*group_list) !=
        ...
        if ((flags & SSL_WRITE_SUPPORTED_GROUPS_EXT_TLS1_2_FLAG) &&
            awrtc_mbedtls_ssl_tls12_named_group_is_ecdhe(*group_list) &&
            (awrtc_mbedtls_ssl_get_ecp_group_id_from_tls_id(*group_list) !=
             AWRTC_MBEDTLS_ECP_DP_NONE)) {
            propose_group = 1;
        }
#endif /* AWRTC_MBEDTLS_SSL_TLS1_2_SOME_ECC */

        if (propose_group) {
            AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 2);
            AWRTC_MBEDTLS_PUT_UINT16_BE(*group_list, p, 0);
            p += 2;
            AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("NamedGroup: %s ( %x )",
                                      awrtc_mbedtls_ssl_named_group_to_str(*group_list),
                                      *group_list));
        }
    }
    ...
    if (named_group_list_len == 0) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(1, ("No group available."));
        return AWRTC_MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }

    /* Write extension_type */
    AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_TLS_EXT_SUPPORTED_GROUPS, buf, 0);
    /* Write extension_data_length */
    AWRTC_MBEDTLS_PUT_UINT16_BE(named_group_list_len + 2, buf, 2);
    /* Write length of named_group_list */
    AWRTC_MBEDTLS_PUT_UINT16_BE(named_group_list_len, buf, 4);

    AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "Supported groups extension",
    ...
    *out_len = (size_t) (p - buf);
    ...
    awrtc_mbedtls_ssl_tls13_set_hs_sent_ext_mask(
        ssl, AWRTC_MBEDTLS_TLS_EXT_SUPPORTED_GROUPS);
#endif /* AWRTC_MBEDTLS_SSL_PROTO_TLS1_3 */

    return 0;
}
#endif /* AWRTC_MBEDTLS_SSL_TLS1_2_SOME_ECC ||
          AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED */
    ...
    size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    for (size_t i = 0; ciphersuite_list[i] != 0; i++) {
        ...
        *tls12_uses_ec |= awrtc_mbedtls_ssl_ciphersuite_uses_ec(ciphersuite_info);
#endif

        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, add ciphersuite: %04x, %s",
                                  (unsigned int) cipher_suite,
                                  ciphersuite_info->name));

        /* Check there is space for the cipher suite identifier (2 bytes). */
        AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 2);
        AWRTC_MBEDTLS_PUT_UINT16_BE(cipher_suite, p, 0);
        p += 2;
    }
    ...
    renegotiating = (ssl->renego_status != AWRTC_MBEDTLS_SSL_INITIAL_HANDSHAKE);
#endif
    if (!renegotiating) {
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("adding EMPTY_RENEGOTIATION_INFO_SCSV"));
        AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, 2);
        AWRTC_MBEDTLS_PUT_UINT16_BE(AWRTC_MBEDTLS_SSL_EMPTY_RENEGOTIATION_INFO, p, 0);
        p += 2;
    }

    /* Write the cipher_suites length in number of bytes */
    cipher_suites_len = (size_t) (p - cipher_suites);
    AWRTC_MBEDTLS_PUT_UINT16_BE(cipher_suites_len, buf, 0);
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(3,
                          ("client hello, got %" AWRTC_MBEDTLS_PRINTF_SIZET " cipher suites",
                           cipher_suites_len/2));
 ...
 */
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_client_hello_body(awrtc_mbedtls_ssl_context *ssl,
                                       unsigned char *buf,
                                       unsigned char *end,
                                       size_t *out_len,
                                       size_t *binders_len)
{
    ...
    *binders_len = 0;
    ...
    unsigned char propose_tls12 =
        (handshake->min_tls_version <= AWRTC_MBEDTLS_SSL_VERSION_TLS1_2)
        &&
        (AWRTC_MBEDTLS_SSL_VERSION_TLS1_2 <= ssl->tls_version);
#endif
#if defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_3)
    unsigned char propose_tls13 =
        (handshake->min_tls_version <= AWRTC_MBEDTLS_SSL_VERSION_TLS1_3)
        &&
        (AWRTC_MBEDTLS_SSL_VERSION_TLS1_3 <= ssl->tls_version);
#endif
     ...
     * buffer.
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, AWRTC_MBEDTLS_CLIENT_HELLO_RANDOM_LEN);
    memcpy(p, handshake->randbytes, AWRTC_MBEDTLS_CLIENT_HELLO_RANDOM_LEN);
     ...
     */
    AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, ssl->session_negotiate->id_len + 1);
    *p++ = (unsigned char) ssl->session_negotiate->id_len;
     ...
     * ...
     */
#if defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_2) && defined(AWRTC_MBEDTLS_SSL_PROTO_DTLS)
    if (ssl->conf->transport == AWRTC_MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        AWRTC_MBEDTLS_SSL_CHK_BUF_PTR(p, end, cookie_len + 1);
...
#endif /* AWRTC_MBEDTLS_SSL_PROTO_TLS1_2 && AWRTC_MBEDTLS_SSL_PROTO_DTLS */
    ...
    *p++ = AWRTC_MBEDTLS_SSL_COMPRESS_NULL;
    ...
    handshake->sent_extensions = AWRTC_MBEDTLS_SSL_EXT_MASK_NONE;
#endif
...
#if defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_3)
    if (propose_tls13) {
        ret = awrtc_mbedtls_ssl_tls13_write_client_hello_exts(ssl, p, end,
                                                        &output_len);
        if (ret != 0) {
            return ret;
        }
...
#if defined(AWRTC_MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
        if (propose_tls13 && awrtc_mbedtls_ssl_conf_tls13_is_some_ephemeral_enabled(ssl)) {
    ...
    write_sig_alg_ext = write_sig_alg_ext ||
                        (propose_tls13 && awrtc_mbedtls_ssl_conf_tls13_is_ephemeral_enabled(ssl));
#endif
#if defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_2)
...
#if defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_2)
    if (propose_tls12) {
        ret = awrtc_mbedtls_ssl_tls12_write_client_hello_exts(ssl, p, end,
                                                        tls12_uses_ec,
                                                        &output_len);
        if (ret != 0) {
            return ret;
     ...
     */
    if (propose_tls13 && awrtc_mbedtls_ssl_conf_tls13_is_some_psk_enabled(ssl)) {
        ret = awrtc_mbedtls_ssl_tls13_write_identities_of_pre_shared_key_ext(
    ...
    if (extensions_len == 0) {
        ...
        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, total extension length: %" \
                                  AWRTC_MBEDTLS_PRINTF_SIZET, extensions_len));
        AWRTC_MBEDTLS_SSL_DEBUG_BUF(3, "client hello extensions",
...
AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_generate_random(awrtc_mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->tls_version == AWRTC_MBEDTLS_SSL_VERSION_TLS1_2) {
        ...
        awrtc_mbedtls_time_t gmt_unix_time = awrtc_mbedtls_time(NULL);
        AWRTC_MBEDTLS_PUT_UINT32_BE(gmt_unix_time, randbytes, 0);
        gmt_unix_time_len = 4;

        AWRTC_MBEDTLS_SSL_DEBUG_MSG(3,
                              ("client hello, current time: %" AWRTC_MBEDTLS_PRINTF_LONGLONG,
    ...
    ret = ssl->conf->f_rng(ssl->conf->p_rng,
                           ...
                           AWRTC_MBEDTLS_CLIENT_HELLO_RANDOM_LEN - gmt_unix_time_len);
    return ret;
}

AWRTC_MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_prepare_client_hello(awrtc_mbedtls_ssl_context *ssl)
{
    int ret;
    size_t session_id_len;
    awrtc_mbedtls_ssl_session *session_negotiate = ssl->session_negotiate;
    ...
    if (ssl->handshake->resume != 0 &&
        session_negotiate->tls_version == AWRTC_MBEDTLS_SSL_VERSION_TLS1_3 &&
        session_negotiate->ticket != NULL) {
        awrtc_mbedtls_ms_time_t now = awrtc_mbedtls_ms_time();
        awrtc_mbedtls_ms_time_t age = now - session_negotiate->ticket_reception_time;
        if (age < 0 ||
            age > (awrtc_mbedtls_ms_time_t) session_negotiate->ticket_lifetime * 1000) {
...
#endif
    {
...
#endif
        {
            ...
            if (ret != 0) {
                AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "Random bytes generation failed", ret);
                return ret;
            }
        }
    }
    ...
    if (session_id_len != session_negotiate->id_len) {
        ...
        if (session_id_len > 0) {
            ret = ssl->conf->f_rng(ssl->conf->p_rng,
                                   ...
                                   session_id_len);
            if (ret != 0) {
                AWRTC_MBEDTLS_SSL_DEBUG_RET(1, "creating session id failed", ret);
                return ret;
            }
        }
    }
    ...
    const char *context_hostname = awrtc_mbedtls_ssl_get_hostname_pointer(ssl);
    if (ssl->tls_version == AWRTC_MBEDTLS_SSL_VERSION_TLS1_3  &&
        ...
        return awrtc_mbedtls_ssl_session_set_hostname(session_negotiate,
                                                context_hostname);
    }
#endif /* AWRTC_MBEDTLS_SSL_PROTO_TLS1_3 &&
          AWRTC_MBEDTLS_SSL_SESSION_TICKETS &&
 ...
 * Handler for AWRTC_MBEDTLS_SSL_CLIENT_HELLO
 */
int awrtc_mbedtls_ssl_write_client_hello(awrtc_mbedtls_ssl_context *ssl)
{
    ...
    AWRTC_MBEDTLS_SSL_DEBUG_MSG(2, ("=> write client hello"));

    AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_prepare_client_hello(ssl));

    AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_start_handshake_msg(
                             ssl, AWRTC_MBEDTLS_SSL_HS_CLIENT_HELLO,
                             &buf, &buf_len));

    AWRTC_MBEDTLS_SSL_PROC_CHK(ssl_write_client_hello_body(ssl, buf,
                                                     buf + buf_len,
                                                     &msg_len,
                                                     &binders_len));
    ...
    if (ssl->conf->transport == AWRTC_MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
         ...
         * can return with the AWRTC_MBEDTLS_ERR_SSL_WANT_READ error code when they
...
#endif /* AWRTC_MBEDTLS_SSL_PROTO_TLS1_2 && AWRTC_MBEDTLS_SSL_PROTO_DTLS */
    {
        ...
        if (binders_len > 0) {
            AWRTC_MBEDTLS_SSL_PROC_CHK(
                awrtc_mbedtls_ssl_tls13_write_binders_of_pre_shared_key_ext(
        ...
        AWRTC_MBEDTLS_SSL_PROC_CHK(awrtc_mbedtls_ssl_finish_handshake_msg(ssl,
                                                              buf_len,
                                                              msg_len));

        /*
         * Set next state. Note that if TLS 1.3 is proposed, this may be
         * overwritten by awrtc_mbedtls_ssl_tls13_finalize_client_hello().
         */
        awrtc_mbedtls_ssl_handshake_set_state(ssl, AWRTC_MBEDTLS_SSL_SERVER_HELLO);

#if defined(AWRTC_MBEDTLS_SSL_PROTO_TLS1_3)
        if (ssl->handshake->min_tls_version <=  AWRTC_MBEDTLS_SSL_VERSION_TLS1_3 &&
            AWRTC_MBEDTLS_SSL_VERSION_TLS1_3 <= ssl->tls_version) {
            ret = awrtc_mbedtls_ssl_tls13_finalize_client_hello(ssl);
        }
#endif
    }
    ...
    AWRTC_MBEDTLS_SSL_PRINT_EXTS(
        3, AWRTC_MBEDTLS_SSL_HS_CLIENT_HELLO, ssl->handshake->sent_extensions);
#endif
...
#endif /* AWRTC_MBEDTLS_SSL_PROTO_TLS1_3 || AWRTC_MBEDTLS_SSL_PROTO_TLS1_2 */
