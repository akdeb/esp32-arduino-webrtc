...
{
    awrtc_psa_key_location_t location = AWRTC_PSA_KEY_LIFETIME_GET_LOCATION( awrtc_psa_get_key_lifetime(attributes) );
    ...
    switch( location )
    {
        ...
        case AWRTC_PSA_CRYPTO_TEST_DRIVER_LOCATION:
            ...
            if( awrtc_psa_key_id_is_builtin(
                    AWRTC_MBEDTLS_SVC_KEY_ID_GET_KEY_ID(
                        awrtc_psa_get_key_id( attributes ) ) ) )
            {
                *key_buffer_size = sizeof( awrtc_psa_drv_slot_number_t );
...

{
    ...
    switch( location )
    {
        case AWRTC_PSA_KEY_LOCATION_LOCAL_STORAGE:
...
#if (defined(AWRTC_PSA_CRYPTO_DRIVER_TEST) )
            status = awrtc_mbedtls_test_transparent_export_public_key
                (attributes,
                                key_buffer,
                                key_buffer_size,
                                data,
            ...
            return( awrtc_psa_export_public_key_internal( attributes,
                                                    key_buffer,
                                                    key_buffer_size,
                                                    data,
                                                    data_size,
...
#if (defined(AWRTC_PSA_CRYPTO_DRIVER_TEST) )
        case 0x7fffff:
            return( awrtc_mbedtls_test_opaque_export_public_key
            (attributes,
                            key_buffer,
                            key_buffer_size,
                            data,
...
#if (defined(AWRTC_PSA_CRYPTO_DRIVER_TEST) )
        case 0x7fffff:
            return( awrtc_mbedtls_test_opaque_get_builtin_key
            (slot_number,
                            attributes,
                            key_buffer,
                            key_buffer_size,
