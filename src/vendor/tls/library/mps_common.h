...
#ifndef AWRTC_MBEDTLS_MPS_COMMON_H
#define AWRTC_MBEDTLS_MPS_COMMON_H
 ...
 *  with error #AWRTC_MBEDTLS_ERR_MPS_OPERATION_UNEXPECTED.
 ...
 */
#define AWRTC_MBEDTLS_MPS_STATE_VALIDATION
 ...
 */
#define AWRTC_MBEDTLS_MPS_ENABLE_ASSERTIONS

/*! This flag controls whether tracing for MPS should be enabled. */
//#define AWRTC_MBEDTLS_MPS_ENABLE_TRACE

#if defined(AWRTC_MBEDTLS_MPS_STATE_VALIDATION)

#define AWRTC_MBEDTLS_MPS_STATE_VALIDATE_RAW(cond, string)                         \
    do                                                                         \
    {                                                                          \
        if (!(cond))                                                          \
        {                                                                      \
            AWRTC_MBEDTLS_MPS_TRACE(AWRTC_MBEDTLS_MPS_TRACE_TYPE_ERROR, string);         \
            AWRTC_MBEDTLS_MPS_TRACE_RETURN(AWRTC_MBEDTLS_ERR_MPS_OPERATION_UNEXPECTED);  \
        }                                                                      \
    } while (0)

#else /* AWRTC_MBEDTLS_MPS_STATE_VALIDATION */
...
#endif /* AWRTC_MBEDTLS_MPS_STATE_VALIDATION */

#if defined(AWRTC_MBEDTLS_MPS_ENABLE_ASSERTIONS)

#define AWRTC_MBEDTLS_MPS_ASSERT_RAW(cond, string)                          \
    do                                                                  \
    {                                                                   \
        if (!(cond))                                                   \
        {                                                               \
            AWRTC_MBEDTLS_MPS_TRACE(AWRTC_MBEDTLS_MPS_TRACE_TYPE_ERROR, string);  \
            AWRTC_MBEDTLS_MPS_TRACE_RETURN(AWRTC_MBEDTLS_ERR_MPS_INTERNAL_ERROR); \
        }                                                               \
    } while (0)

#else /* AWRTC_MBEDTLS_MPS_ENABLE_ASSERTIONS */

#define AWRTC_MBEDTLS_MPS_ASSERT_RAW(cond, string) do {} while (0)

#endif /* AWRTC_MBEDTLS_MPS_ENABLE_ASSERTIONS */
 ...
 *           than ::awrtc_mbedtls_mps_size_t here because of
 *           potential truncation during conversion.
 *
 * \warning  Handshake messages in TLS may be up to 2^24 ~ 16Mb in size.
 *           If awrtc_mbedtls_mps_[opt_]stored_size_t is smaller than that, the
 *           maximum handshake message is restricted accordingly.
 *
 * For now, we use the default type of size_t throughout, and the use of
 * smaller types or different types for ::awrtc_mbedtls_mps_size_t and
 * ::awrtc_mbedtls_mps_stored_size_t is not yet supported.
 *
 */
typedef size_t awrtc_mbedtls_mps_stored_size_t;
#define AWRTC_MBEDTLS_MPS_STORED_SIZE_MAX  (SIZE_MAX)

/** \brief The type of buffer sizes and offsets used in the MPS API
 *         and implementation.
 *
 *         This must be at least as wide as ::awrtc_mbedtls_stored_size_t but
 ...
 *         so almost 10%.
 */
typedef size_t awrtc_mbedtls_mps_size_t;
#define AWRTC_MBEDTLS_MPS_SIZE_MAX  (SIZE_MAX)

#if AWRTC_MBEDTLS_MPS_STORED_SIZE_MAX > AWRTC_MBEDTLS_MPS_SIZE_MAX
#error "Misconfiguration of awrtc_mbedtls_mps_size_t and awrtc_mbedtls_mps_stored_size_t."
#endif

/* \} SECTION: Common types */


#endif /* AWRTC_MBEDTLS_MPS_COMMON_H */
